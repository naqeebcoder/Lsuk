...
function myFunction() {
	 ...
	 var q= document.getElementById("srcdbs_checked").value;if(!q){q="<?php echo $srcdbs_checked; ?>";}
	 var tp = document.getElementById("tp").value;if(!tp){tp="<?php echo $tp; ?>";}
	 window.location.href="reg_interp_list.php" + '?name=' + x + '&srchgender=' + y + '&city=' + z + '&lang=' + p + '&srcdbs_checked=' + q+ '&tp=' + tp;	 
}
</script>
...
<style>.table>tbody>tr>td, .table>tbody>tr>th, .table>tfoot>tr>td, .table>tfoot>tr>th, .table>thead>tr>td, .table>thead>tr>th {
    padding: 8px!important;cursor:pointer;}html, body {background: #fff !important;}.div_actions{position: absolute;margin-top: -37px;background: #ffffff;border: 1px solid lightgrey;}.alert{padding: 6px;}.div_actions .fa {font-size: 18px;}.w3-btn, .w3-button {padding: 4px 6px!important;}</style>
<section class="container-fluid" style="overflow-x:auto">
<div class="col-md-12">
		<header>
		    <center><div class="alert <?php echo $class; ?> col-sm-3">
...
{ ?>
    ...
    <div class="form-group col-md-3 col-sm-4">
        <select id="name" onChange="myFunction()" name="name" class="form-control">
		      <?php
$sql_opt = "SELECT distinct interpreter_reg.name,interpreter_reg.id,interpreter_reg.gender, interpreter_reg.city FROM interpreter_reg WHERE $put_delete $put_active AND isAdhoc=1 ORDER BY interpreter_reg.name ASC";
...
if ($_SESSION["prv"] == "Management" || $_SESSION["prv"] == "Finance")
{
    if ($adLang = "adLang" && $lang == "")
    {
        $query = "SELECT distinct $table.* FROM $table 		
         ...
         and ( interpreter_reg.isAdhoc != '' OR interpreter_reg.isAdhoc =1) and city like '$city%' $put_active	  
    ...
    else
    {
        $query = "SELECT distinct $table.* FROM $table
         ...
         and ( interpreter_reg.isAdhoc != '' OR interpreter_reg.isAdhoc =1) and gender like '$srchgender%' and city like '$city%' $put_active group by email  LIMIT {$startpoint} , {$limit}";
    }
}
if ($_SESSION["prv"] == "Operator")
{
    if ($adLang = "adLang" && $lang == "")
    {
        ...
        where $table.$put_delete and $table.dbs_checked like '$srcdbs_checked%' $put_active and  name like '$name%' and gender like '$srchgender%' 	  
         and ( interpreter_reg.isAdhoc != '' OR interpreter_reg.isAdhoc =1) and city like '$city%'	  
        LIMIT {$startpoint} , {$limit}";
    }
    else
    {
...
?>
				...
				<tr <?php if ($row["is_temp"] == 1)
    { ?>title="This interpreter is registered by Temporary Role. Kindly confirm to process." style="background-color:#cbda78;"<?php
    } ?> <?php if ($row["active"] == 1)
    { ?> class="bg-danger tr_data" title="<?php echo ucwords($row["name"]); ?> is De Activated. Click on row to see actions" <?php
    }
    else
    { ?> title="Click on row to see actions" class="tr_data" <?php
    } ?>> 
				  <td id="emtpyclr">
...
<a title="Trash Record" href="javascript:void(0)" class="w3-button w3-small w3-circle w3-white w3-border w3-border-red" onclick="popupwindow('del_trash.php?del_id=<?php echo $row["id"]; ?>&table=<?php echo $table; ?>','Delete interpreter record', 500,350);"><i class="fa fa-trash"></i></a> 
            ...
            <?php if ($row["is_temp"] == 0)
        {
            ...
            { ?>
                ...
                { ?>
                      <a href="reg_interp_list.php?active=<?php echo $row["active"]; ?>&&act_id=<?php echo $row["id"]; ?>" 
                        title="Status">
                      <?php
                    $status = $row["active"];
                    if ($status == 0)
