...
$source=$row['source'];$target=$row['target'];$docType=$row['docType'];$transType=$row['transType'];$trans_detail=$row['trans_detail'];
  $deliveryType=$row['deliveryType'];$inchContact=$row['inchContact'];$inchEmail=$row['inchEmail'];$inchEmail2=$row['inchEmail2'];
  $file=$row['file'];$orgName=$row['orgName'];$orgRef=$row['orgRef'];$orgContact=$row['orgContact'];
  $asignDate=$row['asignDate'];$deliverDate=$row['deliverDate'];$deliverDate2=$row['deliverDate2'];$deliveryType=$row['deliveryType'];
  $remrks=$row['remrks'];$intrpName=$row['intrpName'];$dated=$row['dated'];$invoiceNo=$row['invoiceNo'];
  $jobStatus=$row['jobStatus'];$bookinType=$row['bookinType'];$nameRef=$row['nameRef'];
  $I_Comments=$row['I_Comments'];$snote=$row['snote'];$jobDisp=$row['jobDisp'];
  $bookedVia=$row['bookedVia'];$inchNo=$row['inchNo'];$line1=$row['line1'];$line2=$row['line2'];
...
.checkbox-inline+.checkbox-inline, .radio-inline+.radio-inline {
    margin-top: 4px;}
    .multiselect {min-width: 250px;}.multiselect-container {max-height: 400px;overflow-y: auto;max-width: 380px;}
    .sky-form select{-webkit-appearance: auto !important;}
    .multiselect-container li.active label.checkbox{color:white;}
...
?>
    ...
    <div class="form-group col-md-6 col-sm-6" id="div_tt">
    ...
    <div class="form-group col-md-6 col-sm-6" id="div_td">
    ...
    $acttObj->editFun($table, $edit_id, 'docType', $docType);}
      if(isset($_POST['submit'])){$transType=implode(",",$_POST['transType']); $acttObj->editFun($table,$edit_id,'transType',$transType);}
      if(isset($_POST['submit'])){$trans_detail=implode(",",$_POST['trans_detail']); $acttObj->editFun($table,$edit_id,'trans_detail',$trans_detail);} ?>
      <div class="form-group col-md-6 col-sm-6">
    ...
    <div class="bg-info col-xs-12 form-group"><h4>Translation Delivery / Deadline Information</h4></div>
      <div class="form-group col-md-4 col-sm-6">
              <label class="input">Assignment Date*</label>
      <input name="asignDate" required type="date" class="form-control date_picker" value="<?php echo $asignDate; ?>"  />
    ...
    <div class="bg-info col-xs-12 form-group"><h4>Booking Details</h4></div>
      <div class="form-group col-md-6 col-sm-6">
    ...
    $acttObj->editFun($table, $edit_id, 'inchEmail', $inchEmail);}?>
      ...
      <div class="form-group col-md-6 col-sm-6 search-box">
                ...
                <input class="form-control" name="orgRef" id="orgRef" type="text" required='' autocomplete="off" placeholder="Type Reference ..." value="<?php echo $orgRef ?>"/>
...
?>
   ...
   <input name="orgName" id="orgName" type="text" value="<?php echo @$name_comp; ?>"  placeholder='' class="form-control" readonly />
       <?php if (isset($_POST['submit'])) {$acttObj->editFun($table, $edit_id, 'orgName', $orgName);}?>
    </div>
      <div class="form-group col-md-4 col-sm-6 hidden">
    ...
    $acttObj->editFun($table, $edit_id, 'line1', $line1);}?>
      ...
      <div class="form-group col-md-4 col-sm-6 hidden">
              <label class="input">Address Line 2</label>
                ...
                <?php if (isset($_POST['submit'])) {$line2 = $_POST['line2'];
    ...
    <div class="bg-info col-xs-12 form-group"><h4>Upload Documents (if any)</h4></div>
    ...
    <div class="bg-info col-xs-12 form-group"><h4>Booking Preferences</h4></div>
    <div class="form-group col-md-6 col-sm-6">
...
if($query_style=='0'){
  ...
  interpreter_reg.active='0' AND (interpreter_reg.actnow='Active' OR (interpreter_reg.actnow='Inactive' AND CURRENT_DATE() NOT BETWEEN interpreter_reg.actnow_time AND interpreter_reg.actnow_to)) AND interpreter_reg.trans='Yes' AND interpreter_reg.deleted_flag=0 AND interpreter_reg.subscribe=1 AND interpreter_reg.is_temp=0 AND interpreter_reg.on_hold='No'";
        ...
        $subject_int = "Bidding Invitation For Translation Project ".$edit_id;
        $sub_title = "New Translation job of ".$source_lang_req." language is available for you to bid.";
        $type_key="nj";
        //$app_int_ids=array();
