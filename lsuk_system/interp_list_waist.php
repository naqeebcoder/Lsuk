...
function myFunction() {
	 ...
	 window.location.href="interp_list.php" + '?interp=' + x + '&org=' + y + '&job=' + z + '&assignDate=' + w + '&our=' + p + '&ur=' + q + '&inov=' + o;
...
<!-- end of sidebar -->
	...
	<section id="main" class="column"><!-- end of stats article -->
		...
		<header><h3 class="tabs_involved" style="width:200px;"><a href="<?php echo basename(__FILE__);?>">F2F Dump list</a></h3>
...
$sql_opt="SELECT distinct interpreter_reg.name FROM interpreter
	   ...
	   where  interpreter.deleted_flag=0 and interpreter.order_cancel_flag=0 and  interpreter.assignDate like '$assignDate%' and interpreter.multInv_flag=0 and interpreter.commit=0 and source like '$job%' and (orgName like '%$_words%') and interpreter.nameRef like '%$our%' and interpreter.orgRef like '%$ur%' and interpreter.invoiceNo like '%$inov%'
 ORDER BY name ASC";
...
?>
  ...
  <input type="date" name="assignDate" id="assignDate" placeholder='' style="width:100px;border-radius: 5px;" onChange="myFunction()" value="<?php echo $assignDate; ?>"/>
	  ...
	  $query="SELECT $table.*,interpreter_reg.name FROM $table
	   ...
	   where  $table.deleted_flag=0 and $table.order_cancel_flag=0 and  $table.invoiceNo=''
	   
	   and  $table.assignDate like '$assignDate%' and source like '$job%' and interpreter_reg.name like '%$interp%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	   order by assignDate LIMIT {$startpoint} , {$limit}";
	    ...
	    $query="SELECT $table.*,interpreter_reg.name FROM $table
	   JOIN interpreter_reg ON $table.intrpName=interpreter_reg.id
	   where  $table.deleted_flag=0 and $table.order_cancel_flag=0 and  $table.hoursWorkd<>0 and $table.assignDate like '$assignDate%' and  $table.multInv_flag=0  and $table.commit=0  and source like '$job%' and interpreter_reg.name like '%$interp%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	   order by assignDate LIMIT {$startpoint} , {$limit}";	
		break; 
		case 'Operator':
	   ...
	   $query="SELECT $table.*,interpreter_reg.name FROM $table
	   JOIN interpreter_reg ON $table.intrpName=interpreter_reg.id
	   where  $table.deleted_flag=0 and $table.order_cancel_flag=0 and orderCancelatoin=0 and $table.hoursWorkd=0 and $table.assignDate like '$assignDate%' and  $table.multInv_flag=0  and $table.commit=0  and source like '$job%' and interpreter_reg.name like '%$interp%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	   order by assignDate LIMIT {$startpoint} , {$limit}";
		break;  
	  }
...
<?php if($_SESSION['prv']=='Management'){?><a href="#" onClick="MM_openBrWindow('del.php?del_id=<?php echo $row['id']; ?>&table=<?php echo $table; ?>','_blank','scrollbars=yes,resizable=yes,width=500,height=200')"><input type="image" src="images/icn_trash.png" title="Permanent Delete"></a> 
 <?php } ?>
                    </td> 
				</tr> 
                <?php } ?>
