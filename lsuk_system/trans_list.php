...
$asignDate=SafeVar::GetVar('asignDate',''); 
...
<html lang="en">
<head>
<title>TRANSLATION BOOKING LIST</title>
...
function myFunction() {
	 ...
	 window.location.href="trans_list.php" + '?interp=' + x + '&org=' + y + '&job=' + z + '&asignDate=' + w + '&our=' + p+ '&ur=' + q + '&inov=' + o;
...
<div class="col-md-12">
		<header>
		    <center><a href="<?php echo basename(__FILE__);?>"><h2 class="col-md-3 col-md-offset-4 text-center"><span class="label label-primary">TRANSLATION <span class="hidden-sm hidden-xs">INTERPRETING </span>BOOKING LIST</span></h2></a></center>
...
$sql_opt="SELECT distinct interpreter_reg.name FROM translation
	   ...
	   where  translation.deleted_flag=0 and translation.order_cancel_flag=0 and  translation.asignDate like '$asignDate%' 
	   and translation.multInv_flag=0 and translation.commit=0 and source like '$job%' $sql_words 
	   and translation.nameRef like '%$our%' and translation.orgRef like '%$ur%' and translation.invoiceNo like '%$inov%'
 ORDER BY name ASC";
...
?>
    ...
    <?php echo $options; ?>
	        ...
	        <div class="form-group col-md-2 col-sm-4">
  <input type="date" name="asignDate" id="asignDate" placeholder='' class="form-control" onChange="myFunction()" value="<?php echo $asignDate; ?>"/>
          </div>
		</header>
	  ...
	  switch($_SESSION['prv']){
	   ...
	   where  $table.deleted_flag=0 and $table.order_cancel_flag=0 and  $table.asignDate like '$asignDate%' and $table.multInv_flag=0  and  $table.multInv_flag=0  and $table.commit=0  and source like '%$job%' and interpreter_reg.name like '%$interp%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	  ...
	  $result = mysqli_query($con,$query);
			while($row = mysqli_fetch_array($result)){$counter++;?>      
				...
				<?php JobListInterCol::GetColTD($row,"numberUnit","Interp Units: "); ?>
...
<a href="#" onClick="MM_openBrWindow('trans_edit.php?edit_id=<?php echo $row['id']; ?>&duplicate=<?php echo 'yes'; ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')"><input type="image" src="images/commit.png" title="Create Duplicate"></a>

                      <?php if($_SESSION['prv']=='Management' || $_SESSION['prv']=='Operator'){?>
                      <?php if($_SESSION['prv']=='Management'){?>
<a href="#" onClick="MM_openBrWindow('trans_edit.php?edit_id=<?php echo $row['id']; ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')"><input type="image" src="images/icn_edit.png" title="Edit">
</a>
...
<?php }if(($row['numberUnit']!=0)  && $_SESSION['prv']=='Finance'){ ?>
...
<a href="#" onClick="MM_openBrWindow('comp_trans_credit_note.php?update_id=<?php echo $row['id']; ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')"><input type="image" src="images/icn_settings.png".png" title="Make Credit Note"></a>
<?php if($row['credit_note']){ ?>
<a href="#" onClick="MM_openBrWindow('credit_trans.php?invoice_id=<?php echo $row['id']; ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')"><input type="image" src="images/icn_categories.png" title="Credit Note"></a>
...
<a href="trans_list_edited.php?view_id=<?php echo $row['id']; ?>"><input type="image" src="images/feedback.png" title="Edited List"></a>
