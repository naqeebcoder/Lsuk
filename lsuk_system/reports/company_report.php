...
$table='interpreter';
 $query="SELECT *, total_charges_comp , interpreter_reg.name  FROM $table
	   ...
	   where paid_interp=1 and assignDate between '$frmDate' and '$toDate'";
...
<?php $i=1;
    ...
    <td><?php echo round($row["total_charges_interp"]*.02+$row["total_charges_interp"],2); ?></td>
    <td><?php echo date_format(date_create($row["paid_date"]), 'd-m-Y'); ?></td>
    <td><?php echo $vat=round($row["total_charges_interp"],2); ?></td>
    <td><?php echo round($row["total_charges_interp"]*.02); ?></td>
    <td><?php echo $row["otherCost"]+$vat; ?></td>
    <td><?php echo round($row["total_charges_interp"]+$row["otherCost"]+$vat,2); ?></td>    
    </tr>
  <?php  $i++;} ?>
 </tbody></table>
<?php } ?>
