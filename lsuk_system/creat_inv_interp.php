...
<?php include 'db.php';include_once ('function.php');$assignDate=@$_GET['assignDate']; $interp=@$_GET['interp']; $org=@$_GET['org']; $job=@$_GET['job'];
...
function myFunction() {
	 ...
	 window.location.href="creat_inv_interp.php" + '?interp=' + x + '&org=' + y + '&job=' + z + '&assignDate=' + w;
...
<!-- end of sidebar -->
	...
	<section id="main" class="column"><!-- end of stats article -->
				<article class="module width_full">
				  <header>
				    <h3 class="tabs_involved" style="width:220px;"> single inpterpreter invoice</h3>
				    <div align="center" style=" width:60%; margin-left:400px;margin-top:10px;">
				      <select id="interp" onChange="myFunction()" name="interp"style="width:125px;height:25px;">
				        <?php 			
...
?>
				      ...
				      |
				      <select id="org" name="org" onChange="myFunction()" style="width:125px; height:25px;">
				        <?php 			
...
?>
				      ...
				      |
				      <select name="job" id="job" onChange="myFunction()" style="width:125px;height:25px;">
				        <?php 			
...
?>
				      ...
				      |
				      <input type="date" name="assignDate" id="assignDate" placeholder='' style="width:125px;border-radius: 5px;" onChange="myFunction()" value="<?php echo $assignDate; ?>"/>
			        </div>
			      </header>
			...
			while($row = mysqli_fetch_array($result)){?>            
				<tr>
				  <td><?php echo $row['source']; ?></td>
                    ...
                    <?php if($_SESSION['prv']=='Management'){?> <a href="#" onClick="MM_openBrWindow('del.php?del_id=<?php echo $row['id']; ?>&table=<?php echo $table; ?>','_blank','scrollbars=yes,resizable=yes,width=400,height=200')"><input type="image" src="images/icn_trash.png" title="Trash"></a> <?php } ?>
                    
                    <?php if($row['total_charges_interp']==NULL || $row['total_charges_interp']==0 || $_SESSION['prv']=='Management'){ ?> <a href="#" onClick="MM_openBrWindow('update_expanses.php?update_id=<?php echo $row['id']; ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')"><input type="image" src="images/Update.png" title="Update Expanses"></a><?php } ?>
                     
                   <?php if($_SESSION['prv']=='Management' || $_SESSION['prv']=='Finance' ){?>
                   <?php if($row['multInv_flag']==0){ ?>  
  <a href="#" onClick="MM_openBrWindow('invoice.php?invoice_id=<?php echo $row['id']; ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')"><input type="image" src="images/invoice.png" title="Invoice"></a><?php } ?>
  ...
  <?php }} ?>
              ...
              <?php if($row['total_charges_comp']==NULL || $row['total_charges_comp']==0 || $_SESSION['prv']=='Management'){ ?>       <a href="#" onClick="MM_openBrWindow('co_update_expanses.php?update_id=<?php echo $row['id']; ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')"><input type="image" src="images/company-icon.jpg" title="Comp Update Expanses"></a><?php } ?>
            <?php if($_SESSION['prv']=='Management'){?> <a href="#" onClick="MM_openBrWindow('comp_earning.php?view_id=<?php echo $row['id']; ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')"><input type="image" src="images/earning.png" title="Earning"></a>
