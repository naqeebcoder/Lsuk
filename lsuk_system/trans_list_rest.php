...
<?php include 'db.php';include 'class.php';include_once ('function.php');$asignDate=@$_GET['assignDate']; $interp=@$_GET['interp']; $org=@$_GET['org']; $job=@$_GET['job'];$our=@$_GET['our'];$ur=@$_GET['ur'];$inov=@$_GET['inov'];
...
<!-- end of sidebar -->
	...
	<section id="main" class="column"><!-- end of stats article -->
		...
		<header><h3 class="tabs_involved" style="width:200px;"><a href="<?php echo basename(__FILE__);?>">Translation trash list</a></h3>
          ...
          <input type="text" name="our" id="our" style="width:80px;height:19px;" placeholder="Our hRef"onChange="myFunction()"value="<?php echo $our; ?>"/>
...
?>
  ...
  <input type="date" name="asignDate" id="asignDate" placeholder='' style="width:100px;border-radius: 5px;" onChange="myFunction()" value="<?php echo $asignDate; ?>"/>
	  ...
	  switch($_SESSION['prv']){
	   ...
	   where  $table.deleted_flag=1 and  $table.order_cancel_flag=0 and $table.asignDate like '$asignDate%' and $table.multInv_flag=0  and  $table.multInv_flag=0  and $table.commit=0  and source like '%$job%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	   ...
	   where  $table.deleted_flag=1 and  $table.order_cancel_flag=0 and $table.numberUnit<>0 and $table.asignDate like '$asignDate%' and  $table.multInv_flag=0  and $table.commit=0  and source like '%$job%' and interpreter_reg.name like '%$interp%' and $table.orgName like '%$org%' order by asignDate and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	   ...
	   $query="SELECT $table.*,interpreter_reg.name FROM $table
	   JOIN interpreter_reg ON $table.intrpName=interpreter_reg.id
	   where  $table.deleted_flag=1 and  $table.order_cancel_flag=0 and orderCancelatoin=0 and $table.numberUnit=0 and $table.asignDate like '$asignDate%' and  $table.multInv_flag=0  and $table.commit=0  and source like '%$job%' and interpreter_reg.name like '%$interp%' and $table.orgName like '%$org%' order by asignDate and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	    LIMIT {$startpoint} , {$limit}";
		break;  
	  }
