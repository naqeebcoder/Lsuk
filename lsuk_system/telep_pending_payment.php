...
function myFunction() {
	 ...
	 window.location.href="telep_pending_payment.php" + '?interp=' + x + '&org=' + y + '&job=' + z + '&assignDate=' + w + '&our=' + p+ '&ur=' + q + '&inov=' + o;
...
<!-- end of sidebar -->
	...
	<section id="main" class="column"><!-- end of stats article -->
				<article class="module width_full">
				  <header>
				    <h3 class="tabs_involved" style="width:220px;"><a href="<?php echo basename(__FILE__);?>">Pending Invoices - Telephone</a></h3>
...
where (orgName like '%$_words%') and telephone.multInv_flag=0 and telephone.commit=1 and (total_charges_comp > rAmount  or total_charges_comp = 0 )
 ORDER BY name ASC";
...
where telephone.multInv_flag=0 and telephone.commit=1 and (total_charges_comp > rAmount  or total_charges_comp = 0 )
...
?>
  ...
  <input type="date" name="assignDate" id="assignDate" placeholder='' style="width:100px;border-radius: 5px;" onChange="myFunction()" value="<?php echo $assignDate; ?>"/>
      ...
      <?php $table='telephone';$counter=0;
	    $query="SELECT $table.*,interpreter_reg.name,comp_reg.email  FROM $table
		JOIN interpreter_reg ON $table.intrpName=interpreter_reg.id	  
		left outer JOIN comp_reg ON $table.orgName=comp_reg.abrv
	   	where $table.deleted_flag = 0 and $table.order_cancel_flag=0 and commit=1 and (total_charges_comp > rAmount  or total_charges_comp = 0 ) and $table.assignDate like '$assignDate%' and source like '%$job%' and interpreter_reg.name like '%$interp%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%' order by assignDate LIMIT {$startpoint} , {$limit}";
...
{ 
	?>
	  <a href="#" onClick="MM_openBrWindow('credit_telep.php?invoice_id=<?php echo $row['id']; ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')">
