...
if (isset($_POST['yes']) && $_POST['job_id'] && $_POST['table']) {
    $job_id = $_POST['job_id'];
    $table = $_POST['table'];
    ...
    // $amend_options_data = $obj->read_specific('effect,effect_interp,amend_for', 'amend_options', 'id=' . $amend_id);
    ...
    $amend_note_text = $_POST['amend_details'];
    $client_email = '0';
    $int_email = '0';
    ...
    if (empty($chk_booked)) {
        ...
        $row = $obj->read_specific("$table.*,interpreter_reg.name,interpreter_reg.email,interpreter_reg.contactNo,interpreter_reg.rph,interpreter_reg.rpm,interpreter_reg.rpu,interpreter_reg.country as interpreter_country,comp_reg.name as orgzName", "$table,interpreter_reg,comp_reg", "$table.intrpName=interpreter_reg.id AND $table.orgName=comp_reg.abrv AND $table.id=" . $job_id);
    }
    $email = $row['email'];
    $email2 = $row['email2'];
    $source = $row['source'];
    ...
    $charged_amount = $_POST['given_amount'];
    ...
    if ($put_amend == '1' && $exist_amend == '0') {
        $obj->insert('amended_records', array('order_id' => $job_id, 'type' => $table, 'interpreter_id' => $row['intrpName'], 'amended_by' => $_SESSION['UserName'], "dated" => date("Y-m-d")));
        $amended_id = $obj->con->insert_id;
    }
    $obj->update($table, array("intrpName" => "", "pay_int" => 1, "amend_note" => $amend_note_text, "edited_by" => $_SESSION['UserName'], "edited_date" => date("Y-m-d H:i:s")), "id=" . $job_id);
    ...
    $obj->log_changes(json_encode($old_values), json_encode($new_values), $job_id, $table, "update", $_SESSION['userId'], $_SESSION['UserName'], "amend_order");
    ...
    $obj->insert('jobnotes', array('jobNote' => $obj->con->real_escape_string($job_note), 'tbl' => $table, 'time' => $misc->sys_datetime_db(), 'fid' => $job_id, 'submitted' => $_SESSION['UserName'], 'dated' => date('Y-m-d')));
    ...
    if (isset($_SESSION['userId'])) {
        ...
        if (empty($get_application['id'])) {
            ...
            $obj->insert("job_messages", array("order_id" => $job_id, "order_type" => $array_order_types[$table], "message_category" => 4, "interpreter_id" => $row['intrpName'], "created_by" => $_SESSION['userId'], "message_body" => $message_body, "sent_to" => $interpreter_phone));
    ...
    if (isset($_POST['status']) && $_POST['status'] == 4 && $_POST['given_amount']) {
        $charge_client = 1;
    ...
    if ($table == 'translation') {
        $append_table = "
            ...
            if (isset($charge_client) && $charge_client == 1) {
                $append_table .= "
                        <tr>
                            <td style='border: 1px solid red;padding:5px;'><strong>Chargeable Amount</strong></td>
                            <td style='border: 1px solid red;padding:5px;'><strong>" . $charged_amount . "</strong></td>
                        </tr>
                    ";
            }
            $append_table .= "
        ...
        if (!empty($chk_booked)) {
            ...
            ";
            if (isset($charge_interpreter) && $charge_interpreter == 1) {
                $append_table .= "
                        <tr>
        ...
        if (($_POST['email_cl']==1 && $client_email == '1' ) && ($_POST['email_int']==1 && $int_email == '1' && !empty($chk_booked)) ) {
            $_SESSION['returned_message'] = '<center><div class="alert alert-success alert-dismissible show col-md-8 col-md-offset-2" role="alert">
                <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                <strong>Success!</strong> Email successfully sent to Client and Interpreter. Thank you
            </div></center>';
        }elseif (($_POST['email_cl']==1 && $client_email == '1' ) && ($_POST['email_int']==0 && $int_email == '0') ) {
            $_SESSION['returned_message'] = '<center><div class="alert alert-success alert-dismissible show col-md-8 col-md-offset-2" role="alert">
                <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                <strong>Success!</strong> Email successfully sent to <strong>Client</strong> Only. Thank you
            </div></center>';
        }elseif (($_POST['email_cl']==0 && $client_email == '0' ) && ($_POST['email_int']==1 && $int_email == '1' && !empty($chk_booked)) ) {
            $_SESSION['returned_message'] = '<center><div class="alert alert-success alert-dismissible show col-md-8 col-md-offset-2" role="alert">
                <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                <strong>Success!</strong> Email successfully sent to <strong>Interpreter</strong> Only. Thank you
            </div></center>';
        } else {
            $_SESSION['returned_message'] = '<center><div class="alert alert-danger alert-dismissible show col-md-8 col-md-offset-2" role="alert">
                <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                <strong>Failed!</strong> Failed to send email to client and interpreter! Try again later
            </div></center>';
        }
    }
    //end if translation 
    ...
    if ($table == 'telephone') {
        ...
        $c_cat = $obj->read_specific("c_cat", "comunic_types", "c_id=" . $comunic)['c_cat'];
        $cat_typ = '';
        if($c_cat ='a'){
            $cat_typ = 'Audio';
        }elseif($c_cat ='v'){
        ...
        $append_table = "
            ...
            <td style='border: 1px solid #58abab;padding:5px;'>". "Remote/".$cat_typ . "/" .$communication_type . "</td>
            </tr>
            <tr>
            <td style='border: 1px solid #58abab;padding:5px;'>Source Language</td>
            <td style='border: 1px solid #58abab;padding:5px;'>" . $source . "</td>
        ...
        $to_add = $inchEmail;
        $subject = "Amending " ."Remote/".$cat_typ . "/" .$communication_type . " Project " . $job_id;
