  ...
  <section>
    ...
    function getMonths($fromDate, $toDate)
    {
      ...
      foreach ($period as $dt) {
        $months[$i]['month'] = $dt->format('F Y');
        $months[$i]['start'] = $dt->format('Y-m-d');
        $months[$i]['end'] = $dt->format('Y-m-t');
        $i++;
      }
      return $months;
    }
    ...
    foreach ($months as $key => $value) {
      ...
      $canc_telephone = $acttObj->read_specific("COUNT(telephone.id) as total_cancelled_jobs,round(IFNULL(sum(telephone.total_charges_comp),0)+ IFNULL(sum(telephone.total_charges_comp * telephone.cur_vat),0),2) as total_cost,SUM(order_cancelledby='LSUK') as lsuk_count,SUM(order_cancelledby='Client') as client_count,GROUP_CONCAT(CASE WHEN order_cancelledby='LSUK' THEN CONCAT (telephone.id,',',nameRef,',',paid_date,',','telephone',',',total_charges_comp,',',cur_vat,',',invoiceNo) END SEPARATOR '|') as lsuk_nameref,GROUP_CONCAT(CASE WHEN order_cancelledby='Client' THEN CONCAT(telephone.id,',',nameRef,',',paid_date,',','telephone',',',total_charges_comp,',',cur_vat,',',invoiceNo) END SEPARATOR '|') as client_nameref, COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=24 AND order_cancelledby='LSUK' THEN telephone.id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=24 AND order_cancelledby='LSUK' THEN telephone.id END)) as LSUK_cancelled_within_24hrs,COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=24 AND order_cancelledby='Client' THEN telephone.id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=24 AND order_cancelledby='Client' THEN telephone.id END)) as Client_cancelled_within_24hrs,COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>24 AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=48 AND order_cancelledby='LSUK' THEN telephone.id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>24 AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=48 AND order_cancelledby='LSUK' THEN telephone.id END)) as LSUK_cancelled_within_48hrs,COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>24 AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=48 AND order_cancelledby='Client' THEN telephone.id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>24 AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=48 AND order_cancelledby='Client' THEN telephone.id END)) as Client_cancelled_within_48hrs, COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>48 AND order_cancelledby='LSUK' THEN telephone.id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>48 AND order_cancelledby='LSUK' THEN telephone.id END)) as LSUK_cancelled_AFTER_48hrs, COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>48 AND order_cancelledby='Client' THEN telephone.id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>48 AND order_cancelledby='Client' THEN telephone.id END)) as Client_cancelled_AFTER_48hrs, COUNT((CASE WHEN cn_time IS NULL THEN telephone.id END)) AS null_cancellation_time", "telephone,comp_reg", " telephone.orgName = comp_reg.abrv AND telephone.assignDate BETWEEN '" . $value['start'] . "' and '" . $value['end'] . "' ".($p_org_ad!=''?"$p_org_ad":' and (telephone.orgName IN (' . $all_cz["all_cz"] . '))')." and telephone.deleted_flag = 0 and (telephone.order_cancel_flag=1 OR telephone.orderCancelatoin=1) AND telephone.cn_r_id NOT IN (7,16)");
      ...
      $deleted_jobs_interp = $acttObj->read_specific("COUNT(id) as total_deleted_jobs,round(IFNULL(sum(total_charges_comp),0)+ IFNULL(sum(total_charges_comp * cur_vat),0) +IFNULL(sum(C_otherexpns),0),2) as total_cost,COUNT(CASE WHEN deleted_reason LIKE '%LSUK%' AND deleted_reason LIKE '%provide the service%' THEN 1 END) AS lsuk_count,COUNT(CASE WHEN deleted_reason LIKE '%Customer no longer wish to proceed%' THEN 1 END) AS client_count ", "interpreter", " interpreter.assignDate between ('" . $value['start'] . "') and ('" . $value['end'] . "') and (interpreter.orgName IN (" . $all_cz["all_cz"] . ")) and interpreter.is_shifted = 0 and interpreter.deleted_flag = 1 AND (interpreter.deleted_reason LIKE '%Customer no longer wish to proceed%' OR interpreter.deleted_reason LIKE '%LSUK % provide the service%') AND interpreter.intrpName='' AND interpreter.id NOT IN (12027,14298,16772,14526)");

      $deleted_jobs_telep = $acttObj->read_specific("COUNT(id) as total_deleted_jobs,round(IFNULL(sum(telephone.total_charges_comp),0)+ IFNULL(sum(telephone.total_charges_comp * telephone.cur_vat),0),2) as total_cost,COUNT(CASE WHEN deleted_reason LIKE '%LSUK%' AND deleted_reason LIKE '%provide the service%' THEN 1 END) AS lsuk_count,COUNT(CASE WHEN deleted_reason LIKE '%Customer no longer wish to proceed%' THEN 1 END) AS client_count", "telephone", " telephone.assignDate BETWEEN '" . $value['start'] . "' and '" . $value['end'] . "' and (telephone.orgName IN (" . $all_cz['all_cz'] . ")) and telephone.is_shifted = 0 and telephone.deleted_flag = 1 AND (telephone.deleted_reason LIKE '%Customer no longer wish to proceed%' or telephone.deleted_reason LIKE '%LSUK % provide the service%') AND telephone.intrpName=''");
        
      $deleted_jobs_trans = $acttObj->read_specific("COUNT(id) as total_deleted_jobs,round(IFNULL(sum(total_charges_comp),0)+ IFNULL(sum(total_charges_comp * cur_vat),0),2) as total_cost,COUNT(CASE WHEN deleted_reason LIKE '%LSUK%' AND deleted_reason LIKE '%provide the service%' THEN 1 END) AS lsuk_count,COUNT(CASE WHEN deleted_reason LIKE '%Customer no longer wish to proceed%' THEN 1 END) AS client_count", "translation", " translation.asignDate BETWEEN '" . $value['start'] . "' and '" . $value['end'] . "' and (translation.orgName IN (" . $all_cz['all_cz'] . ")) and translation.deleted_flag = 1 AND (translation.deleted_reason LIKE '%Customer no longer wish to proceed%' or translation.deleted_reason LIKE '%LSUK % provide the service%') AND translation.intrpName=''");
      ...
      $del_arr = [
        'del_interp_lsuk' =>$deleted_jobs_interp['lsuk_count'],
        'del_telep_lsuk' =>$deleted_jobs_telep['lsuk_count'],
        'del_trans_lsuk' =>$deleted_jobs_trans['lsuk_count'],
        'del_total_lsuk' =>$deleted_jobs_interp['lsuk_count']+$deleted_jobs_telep['lsuk_count']+$deleted_jobs_trans['lsuk_count'],

        'del_interp_client' =>$deleted_jobs_interp['client_count'],
        'del_telep_client' =>$deleted_jobs_telep['client_count'],
        'del_trans_client' =>$deleted_jobs_trans['client_count'],
        'del_total_client' =>$deleted_jobs_interp['client_count']+$deleted_jobs_telep['client_count']+$deleted_jobs_trans['client_count'],
    ...
    $gpcn_data = "SELECT interpreter.id,interpreter.order_cancel_remarks,interpreter.nameRef,interpreter.paid_date,interpreter.invoiceNo,interpreter.total_charges_comp,interpreter.cur_vat,interpreter.c_otherexpns as other_exp,'int_lsuk_nameref' as type,'f2f' as job_type,interpreter.cn_r_id,interpreter.cn_t_id FROM interpreter WHERE interpreter.assignDate BETWEEN '" . $search_2 . "' AND '" . $search_3 . "' AND (interpreter.orgName IN (" . $all_cz['all_cz'] . ")) AND interpreter.deleted_flag=0 AND  (interpreter.orderCancelatoin=1 OR interpreter.order_cancel_flag=1) AND interpreter.cn_r_id NOT IN (7,16) AND order_cancelledby='LSUK' UNION ALL SELECT interpreter.id,interpreter.order_cancel_remarks,interpreter.nameRef,interpreter.paid_date,interpreter.invoiceNo,interpreter.total_charges_comp,interpreter.cur_vat,interpreter.c_otherexpns as other_exp,'int_client_nameref' as type,'f2f' as job_type,interpreter.cn_r_id,interpreter.cn_t_id FROM interpreter WHERE interpreter.assignDate BETWEEN '" . $search_2 . "' AND '" . $search_3 . "' AND (interpreter.orgName IN (" . $all_cz['all_cz'] . ")) AND interpreter.deleted_flag=0 AND  (interpreter.orderCancelatoin=1 OR interpreter.order_cancel_flag=1) AND interpreter.cn_r_id NOT IN (7,16) AND order_cancelledby='Client' UNION ALL SELECT telephone.id,telephone.order_cancel_remarks,telephone.nameRef,telephone.paid_date,telephone.invoiceNo,telephone.total_charges_comp,telephone.cur_vat,telephone.c_otherCharges as other_exp,'telep_lsuk_nameref' as type,'telephone' as job_type,telephone.cn_r_id,telephone.cn_t_id FROM telephone WHERE telephone.assignDate BETWEEN '" . $search_2 . "' AND '" . $search_3 . "' AND (telephone.orgName IN (" . $all_cz['all_cz'] . ")) AND telephone.deleted_flag=0 AND  (telephone.orderCancelatoin=1 OR telephone.order_cancel_flag=1) AND telephone.cn_r_id NOT IN (7,16) AND order_cancelledby='LSUK' UNION ALL SELECT telephone.id,telephone.order_cancel_remarks,telephone.nameRef,telephone.paid_date,telephone.invoiceNo,telephone.total_charges_comp,telephone.cur_vat,telephone.c_otherCharges as other_exp,'telep_client_nameref' as type,'telep' as job_type,telephone.cn_r_id,telephone.cn_t_id FROM telephone WHERE telephone.assignDate BETWEEN '" . $search_2 . "' AND '" . $search_3 . "' and (telephone.orgName IN (" . $all_cz['all_cz'] . ")) AND telephone.deleted_flag=0 AND  (telephone.orderCancelatoin=1 OR telephone.order_cancel_flag=1) AND telephone.cn_r_id NOT IN (7,16) AND order_cancelledby='Client' UNION ALL SELECT translation.id,translation.order_cancel_remarks,translation.nameRef,translation.paid_date,translation.invoiceNo,translation.total_charges_comp,translation.cur_vat,translation.c_otherCharg as other_exp,'trans_lsuk_nameref' as type,'translation' as job_type,translation.cn_r_id,translation.cn_t_id FROM translation WHERE translation.asignDate BETWEEN '" . $search_2 . "' AND '" . $search_3 . "' AND (translation.orgName IN (" . $all_cz['all_cz'] . ")) AND translation.deleted_flag=0 AND  (translation.orderCancelatoin=1 OR translation.order_cancel_flag=1) AND translation.cn_r_id NOT IN (7,16) AND order_cancelledby='LSUK' UNION ALL SELECT translation.id,translation.order_cancel_remarks,translation.nameRef,translation.paid_date,translation.invoiceNo,translation.total_charges_comp,translation.cur_vat,translation.c_otherCharg as other_exp,'trans_client_nameref' as type,'translation' as job_type,translation.cn_r_id,translation.cn_t_id FROM translation WHERE translation.asignDate BETWEEN '" . $search_2 . "' AND '" . $search_3 . "' AND (translation.orgName IN (" . $all_cz['all_cz'] . ")) AND translation.deleted_flag=0 AND  (translation.orderCancelatoin=1 OR translation.order_cancel_flag=1) AND translation.cn_r_id NOT IN (7,16) AND order_cancelledby='Client'";
                ...
                <tr>
                  ...
                  while ($row = mysqli_fetch_assoc($exe_gpcn)) {
                    if ($row['type'] == "int_lsuk_nameref" || $row['type'] == "telep_lsuk_nameref" || $row['type'] == "trans_lsuk_nameref") {
                      ...
                      $cancelReasonRow = !empty($row["cn_r_id"])
                          ? $acttObj->read_specific("cr_title", "cancel_reasons", "cr_id=" . $row["cn_r_id"])
                          : null;

                      $cancellation_reason = $cancelReasonRow && !empty($cancelReasonRow['cr_title'])
                          ? $cancelReasonRow['cr_title']
                          : (!empty($row['order_cancel_remarks']) ? $row['order_cancel_remarks'] : 'No Remark');
                ...
                <tr>
                  ...
                  while ($row2 = mysqli_fetch_assoc($exe_gpcn2)) {
                    if ($row2['type'] == "int_client_nameref" || $row2['type'] == "telep_client_nameref" || $row2['type'] == "trans_client_nameref") {
                      ...
                      $cancelReasonRow = !empty($row2["cn_r_id"])
                          ? $acttObj->read_specific("cr_title", "cancel_reasons", "cr_id=" . $row2["cn_r_id"])
                          : null;

                      $cancellation_reason2 = $cancelReasonRow && !empty($cancelReasonRow['cr_title'])
                          ? $cancelReasonRow['cr_title']
                          : (!empty($row2['order_cancel_remarks']) ? $row2['order_cancel_remarks'] : 'No Remark');
  ...
  <div class="container" style="margin-top:5rem;">
    ...
    <div class="table1" style="margin-top:5rem;">
      ...
      for ($i = 0; $i < count($months); $i++) :
        ...
        $del_interp_lsuk += $months[$i]['deleted']['del_interp_lsuk'];
        $del_telep_lsuk += $months[$i]['deleted']['del_telep_lsuk'];
        $del_trans_lsuk += $months[$i]['deleted']['del_trans_lsuk'];
        $del_total_lsuk += $months[$i]['deleted']['del_total_lsuk'];

        $del_interp_client += $months[$i]['deleted']['del_interp_client'];
        $del_telep_client += $months[$i]['deleted']['del_telep_client'];
        $del_trans_client += $months[$i]['deleted']['del_trans_client'];
        $del_total_client += $months[$i]['deleted']['del_total_client'];
      ...
      <table class="table table-striped table-hover mt-5">
        ...
        <tbody>
          <tr>
            ...
            <td><?php echo "(F2F: $del_interp_lsuk | TP: $del_telep_lsuk | TR: $del_trans_lsuk) = ".$del_total_lsuk; ?></td>
            <td><?php echo "(F2F: $del_interp_client | TP: $del_telep_client | TR: $del_trans_client) = ".$del_total_client; ?></td>
            <td><?php echo "<b>(F2F: $del_interp | TP : $del_telep | TR : $del_trans) = $del_total</b>"; ?></td>
          </tr>
        </tbody>
      </table>
