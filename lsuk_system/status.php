...
if ($_SESSION['is_root'] == 0) {
    ...
    if (empty($get_page_access)) {
        die("<center><h2 class='text-center text-danger'>You do not have access to <u>Confirm Inquiry</u> action for jobs!<br>Kindly contact admin for further process.</h2></center>");
    }
} ?>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="css/bootstrap.css">
<div align="center">
    <h3>Record ID: <span class="label label-info"><?php echo @$_GET['status_id']; ?></span></h3><br />
    <form action="" method="post">
        <h4>Are you sure to <span class="text-success h3"><b>confirm</b></span> this job ?</h4>
...
if (isset($_POST['yes'])) {
    ...
    $edit_id = $_GET['status_id'];
    $row = $obj->read_specific('*', $table, "id=" . $edit_id);
    if (empty($row['invoiceNo'])) {
        //Make invoiceNo if not created already
        $max_number = $obj->read_specific("MAX(id) as max_id", "invoice", "1")['max_id'] + 1;
        if ($max_number == NULL) {
            $max_number = 0;
        }
        $new_number = str_pad($max_number, 5, "0", STR_PAD_LEFT);
        $invoice = date("my") . $new_number;
        $obj->insert("invoice", array("invoiceNo" => $invoice, "dated" => date('Y-m-d')));
        $obj->update($table, array('jobStatus' => 1, "invoiceNo" => $invoice), "id=" . $status_id);
    } else {
        $obj->update($table, array('jobStatus' => 1), "id=" . $status_id);
    }
    ...
    $obj->insert("daily_logs", array("action_id" => 34, "user_id" => $_SESSION['userId'], "details" => $label . " Job ID: " . $edit_id));
    ...
    $orgRef = $row['orgRef'];
    if ($jobDisp == '1' && $jobStatus == '1' && $is_temp == '0') {
        ...
        if ($source_lang == $target_lang) {
            ...
            $put_lang = "interp_lang.lang='$target_lang' and interp_lang.level<3";
            $query_style = '2';
        } else if ($source_lang != 'English' && $target_lang == 'English') {
            $put_lang = "interp_lang.lang='$source_lang' and interp_lang.level<3";
            $query_style = '2';
        } else {
            $put_lang = "";
            $query_style = '3';
        ...
        if ($query_style == '0') {
            ...
            $query_emails = $obj->read_all("DISTINCT interpreter_reg.name, interpreter_reg.email, interpreter_reg.id", "interpreter_reg,interp_lang", "interpreter_reg.code=interp_lang.code AND (SELECT COUNT(DISTINCT interp_lang.lang) FROM interp_lang WHERE interp_lang.type='$chek_col' AND interp_lang.lang IN ('" . $source_lang . "','" . $target_lang . "') and interp_lang.level<3 and interp_lang.code=interpreter_reg.code)=2 and 
            interpreter_reg.active='0' AND (interpreter_reg.actnow='Active' OR (interpreter_reg.actnow='Inactive' AND CURRENT_DATE() NOT BETWEEN interpreter_reg.actnow_time AND interpreter_reg.actnow_to)) AND interpreter_reg.$chek_col='Yes' $dbs_required $put_gender AND interpreter_reg.deleted_flag=0 AND interpreter_reg.subscribe=1 AND interpreter_reg.is_temp=0 AND interpreter_reg.isAdhoc=0" . $append_specific_interpreters);
        } else if ($query_style == '2') {
            $query_emails = $obj->read_all("DISTINCT interpreter_reg.name, interpreter_reg.email, interpreter_reg.id", "interpreter_reg,interp_lang", "interpreter_reg.code=interp_lang.code AND interp_lang.type='$chek_col' AND $put_lang and 
            interpreter_reg.active='0' AND (interpreter_reg.actnow='Active' OR (interpreter_reg.actnow='Inactive' AND CURRENT_DATE() NOT BETWEEN interpreter_reg.actnow_time AND interpreter_reg.actnow_to)) AND interpreter_reg.$chek_col='Yes' $dbs_required $put_gender AND interpreter_reg.deleted_flag=0 AND interpreter_reg.subscribe=1 AND interpreter_reg.is_temp=0 AND interpreter_reg.isAdhoc=0" . $append_specific_interpreters);
        } else {
            $query_emails = $obj->read_all("DISTINCT interpreter_reg.name, interpreter_reg.email, interpreter_reg.id", "interpreter_reg", "interpreter_reg.active='0' AND (interpreter_reg.actnow='Active' OR (interpreter_reg.actnow='Inactive' AND CURRENT_DATE() NOT BETWEEN interpreter_reg.actnow_time AND interpreter_reg.actnow_to)) AND
                interpreter_reg.$chek_col='Yes' $dbs_required $put_gender AND interpreter_reg.deleted_flag=0 AND interpreter_reg.subscribe=1 AND interpreter_reg.is_temp=0 AND interpreter_reg.isAdhoc=0" . $append_specific_interpreters);
        }
        $type_key = "nj";
        while ($row_emails = $query_emails->fetch_assoc()) {
            if ($obj->read_specific("COUNT(*) as blacklisted", "interp_blacklist", "interpName='id-" . $row_emails['id'] . "' AND orgName='" . $orgName . "' AND deleted_flag=0 AND blocked_for=" . $blocked_for[$table])["blacklisted"] == 0) {
                ...
                $array_tokens = explode(',', $obj->read_specific("GROUP_CONCAT( DISTINCT token) as tokens", "int_tokens", "int_id=" . $row_emails['id'])['tokens']);
    ...
    <script>
        alert('Job Status Successfuly Updated !');
