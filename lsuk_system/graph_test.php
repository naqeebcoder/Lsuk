...
<html lang="en">
	...
	<body>
		...
		<script type="text/javascript">
          ...
          zd="<?php echo $search_3; ?>";
			...
			function myFunction_date() {
				...
				if (Date.parse(dt)) {
					window.location.assign('<?php echo basename(__FILE__); ?>?search_1=' + x + '&type=' + type + '&search_2=' + dt+ '&search_3=' + dt2);
				} else {
		...
		<section class="container-fluid">
			<div class="col-md-12">
				<header>
					<center><h2 class="col-md-4 col-md-offset-4 text-center"><span class="label label-primary">Company Wise Job List Bar Graph</span></h2></center>
  ...
  $months = getMonths($search_2,$search_3);
   ...
   ?>
     ...
     foreach ($months as $key=> $value)
     {
      ...
      AND translation.asignDate BETWEEN '".$value['start']."' and '".$value['end']."' and (translation.orgName IN (".$all_cz['all_cz'].")) and translation.deleted_flag = 0 and translation.order_cancel_flag=0) as grp")
      ...
      $canc_interp = $acttObj->read_specific("COUNT(id) as total_cancelled_jobs,round(IFNULL(sum(total_charges_comp),0)+ IFNULL(sum(total_charges_comp * cur_vat),0) +IFNULL(sum(C_otherexpns),0),2) as total_cost,SUM(order_cancelledby='LSUK') as lsuk_count,SUM(order_cancelledby='Client') as client_count, GROUP_CONCAT(CASE WHEN order_cancelledby='LSUK' THEN CONCAT (id,',',nameRef,',',paid_date,',','Face 2 Face',',',total_charges_comp,',',cur_vat,',',invoiceNo) END SEPARATOR '|') as lsuk_nameref,GROUP_CONCAT(CASE WHEN order_cancelledby='Client' THEN CONCAT(id,',',nameRef,',',paid_date,',','Face 2 Face',',',total_charges_comp,',',cur_vat,',',invoiceNo) END SEPARATOR '|') as client_nameref, COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=24 AND order_cancelledby='LSUK' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=24 AND order_cancelledby='LSUK' THEN id END)) as LSUK_cancelled_within_24hrs,COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=24 AND order_cancelledby='Client' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=24 AND order_cancelledby='Client' THEN id END)) as Client_cancelled_within_24hrs,COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>24 AND  HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=48 AND order_cancelledby='LSUK' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>24 AND  HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=48 AND order_cancelledby='LSUK' THEN id END)) as LSUK_cancelled_within_48hrs,COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>24 AND  HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=48 AND order_cancelledby='Client' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>24 AND  HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=48 AND order_cancelledby='Client' THEN id END)) as Client_cancelled_within_48hrs, COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>48 AND order_cancelledby='LSUK' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>48 AND order_cancelledby='LSUK' THEN id END)) as LSUK_cancelled_AFTER_48hrs, COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>48 AND order_cancelledby='Client' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>48 AND order_cancelledby='Client' THEN id END)) as Client_cancelled_AFTER_48hrs, COUNT((CASE WHEN cn_time IS NULL THEN id END)) AS null_cancellation_time","interpreter"," interpreter.assignDate between ('".$value['start']."') and ('".$value['end']."') and (interpreter.orgName IN (" .$all_cz["all_cz"] .")) and interpreter.deleted_flag = 0 AND (interpreter.order_cancel_flag=1 OR interpreter.orderCancelatoin=1) AND interpreter.id NOT IN (12027,14298,16772,14526)");
      $canc_telephone =$acttObj->read_specific("COUNT(id) as total_cancelled_jobs,round(IFNULL(sum(telephone.total_charges_comp),0)+ IFNULL(sum(telephone.total_charges_comp * telephone.cur_vat),0),2) as total_cost,SUM(order_cancelledby='LSUK') as lsuk_count,SUM(order_cancelledby='Client') as client_count,GROUP_CONCAT(CASE WHEN order_cancelledby='LSUK' THEN CONCAT (id,',',nameRef,',',paid_date,',','telephone',',',total_charges_comp,',',cur_vat,',',invoiceNo) END SEPARATOR '|') as lsuk_nameref,GROUP_CONCAT(CASE WHEN order_cancelledby='Client' THEN CONCAT(id,',',nameRef,',',paid_date,',','telephone',',',total_charges_comp,',',cur_vat,',',invoiceNo) END SEPARATOR '|') as client_nameref,  COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=24 AND order_cancelledby='LSUK' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=24 AND order_cancelledby='LSUK' THEN id END)) as LSUK_cancelled_within_24hrs,COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=24 AND order_cancelledby='Client' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=24 AND order_cancelledby='Client' THEN id END)) as Client_cancelled_within_24hrs,COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>24 AND  HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=48 AND order_cancelledby='LSUK' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>24 AND  HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=48 AND order_cancelledby='LSUK' THEN id END)) as LSUK_cancelled_within_48hrs,COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>24 AND  HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))<=48 AND order_cancelledby='Client' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>24 AND  HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))<=48 AND order_cancelledby='Client' THEN id END)) as Client_cancelled_within_48hrs, COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>48 AND order_cancelledby='LSUK' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>48 AND order_cancelledby='LSUK' THEN id END)) as LSUK_cancelled_AFTER_48hrs, COUNT((CASE WHEN cn_time IS NOT NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', cn_time) ))>48 AND order_cancelledby='Client' THEN id WHEN cn_time IS NULL AND HOUR(timediff(CONCAT(assignDate, ' ', assignTime) ,CONCAT(cn_date, ' ', '09:00:00') ))>48 AND order_cancelledby='Client' THEN id END)) as Client_cancelled_AFTER_48hrs, COUNT((CASE WHEN cn_time IS NULL THEN id END)) AS null_cancellation_time","telephone"," telephone.assignDate BETWEEN '".$value['start']."' and '".$value['end']."' and (telephone.orgName IN (".$all_cz['all_cz'].")) and telephone.deleted_flag = 0 and (telephone.order_cancel_flag=1 OR telephone.orderCancelatoin=1)");
      $canc_trans = $acttObj->read_specific("COUNT(id) as total_cancelled_jobs,round(IFNULL(sum(total_charges_comp),0)+ IFNULL(sum(total_charges_comp * cur_vat),0),2) as total_cost,SUM(order_cancelledby='LSUK') as lsuk_count,SUM(order_cancelledby='Client') as client_count, GROUP_CONCAT(CASE WHEN order_cancelledby='LSUK' THEN CONCAT (id,',',nameRef,',',paid_date,',','translation',',',total_charges_comp,',',cur_vat,',',invoiceNo) END SEPARATOR '|') as lsuk_nameref,GROUP_CONCAT(CASE WHEN order_cancelledby='Client' THEN CONCAT(id,',',nameRef,',',paid_date,',','translation',',',total_charges_comp,',',cur_vat,',',invoiceNo) END SEPARATOR '|') as client_nameref, COUNT((CASE WHEN datediff(asignDate ,cn_date )<=1 AND order_cancelledby='LSUK' THEN id END)) as LSUK_cancelled_within_24hrs,COUNT((CASE WHEN datediff(asignDate ,cn_date )<=1 AND order_cancelledby='Client' THEN id END)) as Client_cancelled_within_24hrs, COUNT((CASE WHEN datediff(asignDate ,cn_date )>1 AND  datediff(asignDate ,cn_date )<=2 AND order_cancelledby='LSUK' THEN id END)) as LSUK_cancelled_within_48hrs,COUNT((CASE WHEN datediff(asignDate ,cn_date )>1 AND  datediff(asignDate ,cn_date )<=2 AND order_cancelledby='Client' THEN id END)) as Client_cancelled_within_48hrs, COUNT((CASE WHEN datediff(asignDate ,cn_date )>2 AND order_cancelledby='LSUK' THEN id END)) as LSUK_cancelled_AFTER_48hrs,COUNT((CASE WHEN datediff(asignDate ,cn_date )>2 AND order_cancelledby='Client' THEN id END)) as Client_cancelled_AFTER_48hrs, COUNT((CASE WHEN cn_date IS NULL THEN id END)) AS null_cancellation_time","translation"," translation.asignDate BETWEEN '".$value['start']."' and '".$value['end']."' and (translation.orgName IN (".$all_cz['all_cz'].")) and translation.deleted_flag = 0 and (translation.order_cancel_flag=1 OR translation.orderCancelatoin=1)"); 
                    ...
                    <tr>
                      ...
                      while($row = mysqli_fetch_assoc($exe_gpcn)){
                          if($row['type']=="int_lsuk_nameref" || $row['type']=="telep_lsuk_nameref" || $row['type']=="trans_lsuk_nameref"){
                            ...
                            $pd_status="";
                            if($row['job_type']=="f2f"){
                              $tcl = $row['total_charges_comp']+($row['total_charges_comp']*$row['cur_vat']);+$row['other_exp'];
                            }else{
                              $tcl = $row['total_charges_comp']+($row['total_charges_comp']*$row['cur_vat']);
                            }
                            
                            if(trim($row['paid_date'])=="1001-01-01"){ $pd_status="Unpaid"; }else{ $pd_status= $row['paid_date']; }
                    ...
                    <tr>
                      ...
                      while($row2 = mysqli_fetch_assoc($exe_gpcn2)){
                        if($row2['type']=="int_client_nameref" || $row2['type']=="telep_client_nameref" || $row2['type']=="trans_client_nameref"){
                            ...
                            if(trim($row2['paid_date'])=="1001-01-01"){ $pd_status2="Unpaid"; }else{ $pd_status2= $row2['paid_date']; }
