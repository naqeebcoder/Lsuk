...
<!-- end of sidebar -->
	...
	<section id="main" class="column"><!-- end of stats article -->
		...
		<header><h3 class="tabs_involved" style="width:200px;"><a href="<?php echo basename(__FILE__);?>">F2F Cancelled Orders list</a></h3>
...
?>
  ...
  <input type="date" name="assignDate" id="assignDate" placeholder='' style="width:100px;border-radius: 5px;" onChange="myFunction()" value="<?php echo $assignDate; ?>"/>
	  ...
	  switch($_SESSION['prv']){
	   ...
	   where  $table.deleted_flag=0 and $table.order_cancel_flag=1 and  $table.assignDate like '$assignDate%' and $table.multInv_flag=0 and $table.commit=0 and source like '$job%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	   ...
	   $query="SELECT $table.* FROM $table
	   where  $table.deleted_flag=1 and  $table.order_cancel_flag=1 and  $table.hoursWorkd<>0 and $table.assignDate like '$assignDate%' and  $table.multInv_flag=0  and $table.commit=0  and source like '$job%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	   order by assignDate LIMIT {$startpoint} , {$limit}";	
		break; 
		case 'Operator':
		
	   $query="SELECT $table.* FROM $table
	   where  $table.deleted_flag=0 and  $table.order_cancel_flag=1 and  orderCancelatoin=0 and $table.hoursWorkd=0 and $table.assignDate like '$assignDate%' and  $table.multInv_flag=0  and $table.commit=0  and source like '$job%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	   order by assignDate LIMIT {$startpoint} , {$limit}";
		break;  
	  }
				...
				<tr title="<?php echo "Tracking ID:". $row['id']; ?>">
				  <td><?php echo $row['source']; ?></td>
    				...
    				<td><?php echo $row['bookinType']; ?></td>  
				  	<td><?php echo $row['order_cancel_remarks']; ?></td>
    				<td align="center">
