...
if(!empty($credit_note)){
  $row_credit_note=$acttObj->read_specific("credit_notes.data","credit_notes","order_id=".$invoice_id." AND order_type='tr' AND status=1");
  $row_credit=json_decode($row_credit_note['data'], true);
  /*foreach($row as $key=>$value){
    $$key=$value;
  }*/
  $C_numberUnit=$row_credit['C_numberUnit'];
  $C_rpU=$row_credit['C_rpU'];
  $C_otherCharg=$row_credit['C_otherCharg'];
  $total_charges_comp=$row_credit['total_charges_comp'];
  $certificationCost=$row_credit['certificationCost'];
  $proofCost=$row_credit['proofCost'];
  $postageCost=$row_credit['postageCost'];
  $C_numberWord=$row_credit['C_numberWord'];
  $C_rpW=$row_credit['C_rpW'];
  $C_admnchargs=$row_credit['C_admnchargs'];
  $cur_vat=@$row_credit['cur_vat'];
  //Row data update
  $row['C_numberUnit']=$row_credit['C_numberUnit'];
  $row['C_rpU']=$row_credit['C_rpU'];
  $row['C_otherCharg']=$row_credit['C_otherCharg'];
  $row['total_charges_comp']=$row_credit['total_charges_comp'];
  $row['certificationCost']=$row_credit['certificationCost'];
  $row['proofCost']=$row_credit['proofCost'];
  $row['postageCost']=$row_credit['postageCost'];
  $row['C_numberWord']=$row_credit['C_numberWord'];
  $row['C_rpW']=$row_credit['C_rpW'];
