...
$action_view_job = $_SESSION['is_root'] == 1 || in_array(28, $get_actions);
$action_edit_job = $_SESSION['is_root'] == 1 || in_array(29, $get_actions);
$action_delete_job = $_SESSION['is_root'] == 1 || in_array(30, $get_actions);
$action_restore_job = $_SESSION['is_root'] == 1 || in_array(31, $get_actions);
$action_duplicate = $_SESSION['is_root'] == 1 || in_array(32, $get_actions);
$action_resume_job = $_SESSION['is_root'] == 1 || in_array(33, $get_actions);
$action_assign_interpreter = $_SESSION['is_root'] == 1 || in_array(34, $get_actions);
$action_view_applicants = $_SESSION['is_root'] == 1 || in_array(35, $get_actions);
$action_confirm_enquiry = $_SESSION['is_root'] == 1 || in_array(36, $get_actions);
$action_confirm_temporary = $_SESSION['is_root'] == 1 || in_array(37, $get_actions);
$action_reallocate_operator = $_SESSION['is_root'] == 1 || in_array(38, $get_actions);
$action_check_job = $_SESSION['is_root'] == 1 || in_array(39, $get_actions);
$action_job_note = $_SESSION['is_root'] == 1 || in_array(40, $get_actions);
$action_force_assign_interpreter = $_SESSION['is_root'] == 1 || in_array(147, $get_actions);
$action_can_view_all_jobs = $_SESSION['is_root'] == 1 || in_array(148, $get_actions);
$action_dropdown_trashed_jobs_filter = $_SESSION['is_root'] == 1 || in_array(153, $get_actions);
$action_dropdown_cancelled_jobs_filter = $_SESSION['is_root'] == 1 || in_array(154, $get_actions);
$action_dropdown_enquiry_jobs_filter = $_SESSION['is_root'] == 1 || in_array(228, $get_actions);
$action_edited_history = $_SESSION['is_root'] == 1 || in_array(157, $get_actions);
//Access actions
$tp = @$_GET['tp'];
$table = 'translation';
                ...
                <table class="tablesorter table table-bordered tbl_data" cellspacing="0" cellpadding="0">
                    ...
                    <tbody>
                        ...
                        if (isset($_GET['tp']) && $_GET['tp'] == 'en'){
                            $result = $acttObj->read_all("$table.*", "$table", "(asignDate >= CURDATE() or  bookeddate >= CURDATE() ) and(jobStatus = 0)" . $deleted_flag . $order_cancel_flag . " ORDER BY $table.assignDate ASC LIMIT 100");
                        }
                        else if (isset($_GET['tp']) && $_GET['tp'] != 'a') {
                            $result = $acttObj->read_all("*", "$table", "(intrpName='' OR intrpName IS NULL) " . $deleted_flag . $order_cancel_flag . " ORDER BY $table.dated DESC LIMIT 100");
                        } else {
                            $result = $acttObj->read_all("*", "$table", "(intrpName='' OR intrpName IS NULL) " . $deleted_flag . $order_cancel_flag . " ORDER BY $table.asignDate ASC LIMIT 100");
                        }
                        ...
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            $g_row = &$row;
                            $urgent = (date('Y-m-d') == $row['asignDate'] || $req_date1 == $row['asignDate'] || $req_date2 == $row['asignDate'] || $req_date3 == $row['asignDate'] || $req_date4 == $row['asignDate']) ? 'class="bg-danger"' : '';
                            ...
                            <td <?php if ($row['is_temp'] == 1) { ?>title="This job is booked by Temporary Role. Kindly confirm to process." style="background-color:#cbda78;" <?php } ?>>
                                <div class="col-lg-4 col-md-4 col-sm-4">
                                    <ul class="w3-ul">
                                        ...
                                        <li>Booking Person <span class="w3-right"><?php echo $row['orgContact']; ?></span></li>
                                        <li>Booking Reference <span class="w3-right"><?php echo $row['nameRef']; ?></span></li>
                                ...
                                <div class="col-lg-4 col-md-4 col-sm-4">
                                    <ul class="w3-ul">
                                        <li><?php $put_class = strlen($row['orgName']) > 19 ? 'w3-small' : 'w3-medium';
                                            echo '<span class="label ' . $put_class . ' w3-blue">' . $row['orgName'] . '</span>'; ?><span class="w3-medium w3-right" style="font-weight:bold"><?php echo date('d-m-Y', strtotime($row['asignDate'])); ?></span></li>
                                ...
                                <div class="col-sm-3 col-xs-12">
                                    <ul class="w3-ul">
                                        <!-- <li>Gender <span class=" w3-right">No Preference</span></li> -->
                                        <li>Gender <span class=" w3-right"><?php echo (empty($row['gender'])?"No Preference":$row['gender']); ?></span></li>
                                        ...
                                        <?php $trans_drop = $acttObj->read_specific("GROUP_CONCAT(CONCAT(td_title)  SEPARATOR '<br>') as td_title", "trans_dropdown", "td_id IN (" . $row['transType'] . ")")['td_title']; ?>
                                        <li>Translation Type <span class="w3-right <?php echo strlen($trans_drop) > 21 ? 'w3-small' : ''; ?>"><?php echo $trans_drop ?: '- - -'; ?></span></li>
                                        <li><b>DB Reference <span class="w3-right"><?php echo $row['reference_no']; ?></span></b></li>
                                    </ul>
                                </div>
                                ...
                                <div class="col-sm-12 text-center action_buttons">
                                    <?php if ($row['approve_portal_mngt']>0){ if ($action_view_job) { ?>
                                        <a data-toggle="tooltip" data-placement="top" href="javascript:void(0)" class="w3-button w3-small w3-circle w3-white w3-border w3-border-blue" title="View Job" onClick="popupwindow('order_view.php?view_id=<?php echo $row['id']; ?>&table=<?php echo $table; ?>', 'View translation order', 1000, 1000);"><i class="fa fa-eye"></i></a>
                                    <?php }
                                    if ($row['deleted_flag'] == 0 && $row['order_cancel_flag'] == 0) {
                                        if ($action_confirm_temporary) {
                                            if ($row['is_temp'] == 1) { ?>
                                                <a data-toggle="tooltip" data-placement="top" href="javascript:void(0)" class="w3-button w3-small w3-circle w3-yellow w3-border w3-border-blue" title="This job must be confirmed by Manager or Supervisor" onClick="popupwindow('confirm_record.php?id=<?php echo $row['id']; ?>&table=<?php echo $table; ?>', 'Confirm translation order', 520,350);"><i class="fa fa-check-circle"></i></a>
                                            <?php }
                                        }
                                        if ($action_duplicate) {
                                            if ($row['is_temp'] == 0) { ?>
                                                <a data-toggle="tooltip" data-placement="top" href="javascript:void(0)" class="w3-button w3-small w3-circle w3-white w3-border w3-border-blue" title="Create Duplicate" onClick="popupwindow('trans_edit.php?edit_id=<?php echo $row['id']; ?>&duplicate=<?php echo 'yes'; ?>','Duplicate translation order',1250, 730)"><i class="fa fa-clone"></i></a>
                                            <?php }
                                        }
                                    }
                                    if ($action_job_note) {
                                    ...
                                    <?php }
                                    $dox_counter = $acttObj->read_specific('count(id) as counter', 'job_files', 'status=1 and tbl="' . $table . '" and file_type="c_portal" and order_id=' . $row['id']);
                                    if ($dox_counter['counter'] > 0) {
                                        if ($row['is_temp'] == 0) { ?>
                                            <a data-toggle="tooltip" data-placement="top" href="javascript:void(0)" onClick="popupwindow('trans_dox_view.php?order_id=<?php echo $row['id']; ?>&table=<?php echo $table; ?>','View Translation Document(s)',620,450)" title="View Translation Document(s)"><i class="fa fa-file"></i></a>
                                        <?php }
                                    }
