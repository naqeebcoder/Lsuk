...
function myFunction() {
	 ...
	 window.location.href="trans_pending_payment.php" + '?interp=' + x + '&org=' + y + '&job=' + z + '&asignDate=' + w + '&our=' + p+ '&ur=' + q + '&inov=' + o;
...
<!-- end of sidebar -->
	...
	<section id="main" class="column"><!-- end of stats article -->
				<article class="module width_full">
				  <header>
				    <h3 class="tabs_involved" style="width:230px;"><a href="<?php echo basename(__FILE__);?>">Pending Trans Invoices</a></h3>
...
where (orgName like '%$_words%') and translation.multInv_flag=0 and translation.commit=1 and  (total_charges_comp > rAmount  or total_charges_comp =0) 
 ORDER BY name ASC";
...
where translation.multInv_flag=0 and translation.commit=1 and  (total_charges_comp > rAmount  or total_charges_comp =0)
  ...
  <input type="date" name="asignDate" id="asignDate" placeholder='' style="width:100px;border-radius: 5px;" onChange="myFunction()" value="<?php echo $asignDate; ?>"/>
      ...
      <?php $table='translation';$counter=0;
	   ...
	   left outer JOIN comp_reg ON $table.orgName=comp_reg.abrv
	    where $table.deleted_flag = 0 and $table.order_cancel_flag=0 and $table.commit=1 and  (total_charges_comp > rAmount  or total_charges_comp =0) and $table.asignDate like '$asignDate%'  and source like '%$job%' and interpreter_reg.name like '%$interp%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	   order by asignDate LIMIT {$startpoint} , {$limit}";
			$result = mysqli_query($con,$query);
			while($row = mysqli_fetch_array($result)){$counter++;?>    
				<tr>
				  <td><?php if($row['numberUnit']==0){ ?>
