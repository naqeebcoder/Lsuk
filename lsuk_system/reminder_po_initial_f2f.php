...
$query_reminder = $obj->read_all("*", "(SELECT interpreter.porder_email,interpreter.porder,interpreter.id,interpreter.orgName,interpreter.inchEmail,interpreter.inchPerson,interpreter.source,interpreter.target,interpreter.invoic_date,interpreter.assignDate,SUBSTRING(interpreter.assignTime,1,5) as assignTime,interpreter.total_charges_comp,interpreter.cur_vat,interpreter.C_otherexpns, interpreter.credit_note,interpreter.C_admnchargs,interpreter.nameRef,interpreter.orgRef,interpreter.invoiceNo FROM interpreter,comp_reg WHERE interpreter.orgName=comp_reg.abrv and interpreter.multInv_flag=0 AND interpreter.deleted_flag = 0 and interpreter.order_cancel_flag=0 and interpreter.commit=1 AND interpreter.total_charges_comp >0 and comp_reg.po_req=1 and (interpreter.porder='' OR interpreter.porder='Nil') and (round(interpreter.rAmount,2) < round((interpreter.total_charges_comp+(interpreter.total_charges_comp*interpreter.cur_vat)),2) or interpreter.total_charges_comp =0) and YEAR(interpreter.assignDate)>'2018') as grp", "id NOT IN 
(SELECT DISTINCT po_requested.order_id from po_requested WHERE po_requested.order_type='f2f') LIMIT 30");
