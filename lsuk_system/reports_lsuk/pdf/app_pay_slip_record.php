...
$data_slip = $obj->read_specific('*', 'interp_salary', 'invoice="' . $invoice . '"');
$array_job_type = array(1 => "F2F", 2 => "Telephone", 3 => "Translation");
$row1 = $obj->read_specific("*,CONCAT('****',SUBSTRING(IF(acNo IS NULL or acNo = '', '00000000', acNo),5,8)) as acNo,CONCAT(LEFT(acntCode,2),'**',RIGHT(acntCode,2)) as acntCode,CONCAT(buildingName,' ',line1,' ',line2,' ',line3) as address", "interpreter_reg", "id=" . $data_slip['interp']);
...
$query_interp = "SELECT interpreter.id,interpreter.ni_dedu,interpreter.tax_dedu,interpreter.source,interpreter.assignDate, interpreter.orgName, interpreter.hoursWorkd, interpreter.chargInterp, interpreter.chargeTravel, interpreter.chargeTravelTime, interpreter.travelCost, interpreter.otherCost, interpreter.admnchargs, interpreter.deduction, interpreter.total_charges_interp, (interpreter.total_charges_interp*interpreter.int_vat) as vat_f2f FROM interpreter,invoice where interpreter.invoiceNo=invoice.invoiceNo AND interpreter.deleted_flag = 0 and interpreter.order_cancel_flag=0 and interpreter.intrp_salary_comit=1 and interpreter.intrpName='" . $int_id . "' and interpreter.$check_salary_id order by interpreter.assignDate";
...
$query_telep = "SELECT telephone.id,telephone.ni_dedu,telephone.tax_dedu,telephone.source,telephone.assignDate,telephone.orgName, telephone.hoursWorkd, telephone.rateHour, telephone.chargInterp, telephone.otherCharges, telephone.admnchargs, telephone.deduction, telephone.total_charges_interp, (telephone.total_charges_interp*telephone.int_vat) as vat_tp FROM telephone ,invoice where telephone.invoiceNo=invoice.invoiceNo AND telephone.deleted_flag = 0 and telephone.order_cancel_flag=0 and telephone.intrp_salary_comit=1 and telephone.intrpName='" . $int_id . "' and telephone.$check_salary_id order by telephone.assignDate";
...
<table width="30%" border="1" cellpadding="2">
      <tr>
        <td width="50%" align="left" bgcolor="#F4F4F4" scope="col">Grand Total </td>
...
if ($data_slip['given_amount'] > 0) {
  $tbl .= '<tr>
      <td bgcolor="#F4F4F4" style="color:darkgreen">Addition Payment</td>
...
$tbl .= '<tr>
    ...
    <td><span class="date"><b>' . $misc->numberFormat_fun(($data_slip['salry'] - $total_ni_deduction - $total_tax_deduction - $salary_payable_deduction) + $salary_non_payable_deduction) . '</b></span></td>
