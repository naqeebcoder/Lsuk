...
$tbl = <<<EOD
<style>
table {border-collapse: collapse; width:717px;word-wrap: break-word;}
...
$tbl.=<<<EOD
<div>
<h2 align="center"><u>Interpreter Pending Payment Report</u></h2>
...
{
  $vat=$row["vat"];
  $vat_total_charges_comp=$row["total_charges_comp"]+$vat;
  $total_vat_total_charges_comp=
  $vat_total_charges_comp + $total_vat_total_charges_comp;
...
$tbl.=<<<EOD
    <tr>
		...
		<td>{$row["source"]}</td>
		<td>{$misc->numberFormat_fun($vat_total_charges_comp)}</td>
    </tr>
EOD;
 $i++;
...
$tbl.=<<<EOD
	 <tr>
		...
		<td></td>
		<td>Total</td>
		<td>{$misc->numberFormat_fun($total_vat_total_charges_comp)}</td> 
		</tr>
</table>
