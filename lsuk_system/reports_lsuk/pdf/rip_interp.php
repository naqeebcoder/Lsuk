...
else
{
  $query="SELECT $table.*, interpreter_reg.name  FROM $table
	   inner join interpreter_reg on $table.intrpName = interpreter_reg.id 
	   where $table.deleted_flag = 0 and $table.order_cancel_flag=0 
...
function UpdateCompTotal(&$map,&$row,$i)
{
	...
	$map["i"]=$i;
	$map["name"]=$row["name"];
	$map["assignDate"]=$row["assignDate"];
	$map["assignTime"]=$row["assignTime"];
	$map["dueDate"]=$row["dueDate"];
	$map["source"]=$row["source"];
	$map["invoiceNo"]=$row["invoiceNo"];
	$map["orgName"]=$row["orgName"];
}
...
/*$tbl.=<<<EOD
	<tr>
  <td style="width:45px;">{$map['i']}</td>
  <td>{$map['name']}</td>
  <td>{$misc->dated($map['assignDate'])}</td>
  <td>{$map['assignTime']}</td>
  
  <td>{$misc->numberFormat_fun($map["total_charges_interp"])}</td>
  <td>{$misc->dated($map['dueDate'])}</td>	  
  <td>{$map['source']}</td>
  <td>{$map['invoiceNo']}</td>
  <td>{$map['orgName']}</td>
  </tr>				
