...
$makCompanyEmail = $_GET['company_email'];

//$table='interpreter';
$table = $_GET['table'];

$invno = $acttObj->unique_data($table, 'invoiceNo', 'id', $invoice_id);
...
// $to_add = $coEmail;
$to_add = $makCompanyEmail;
$from_add = "info@lsuk.org";
$strSubject = "LSUK Invoice # " . $invno . $append_invoiceNo;
$query_format = "SELECT em_format FROM email_format where id='1'";
$result_format = mysqli_query($con, $query_format);
$row_format = mysqli_fetch_assoc($result_format);
//Get format from database
$strMsg = $row_format['em_format'];
$strMsg =  str_replace("<Number_of_days>", @$payment_terms, $strMsg); // please also updata the database replace "21 days" with "<Number_of_days> days"
if (isset($makCompanyEmail) && !empty($makCompanyEmail)) {
    try {
        $mail->SMTPDebug = 0;
        //$mail->isSMTP(); 
        ...
        $mail->addStringAttachment($pdfhere, "invoice_" . $invno . $append_invoiceNo . ".pdf");
        $mail->isHTML(true);
