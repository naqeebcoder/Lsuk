...
<table class="aa" border="1" cellspacing="0" cellpadding="0" style="width:250px">
  ...
  <tr>
    <th>Source_Lang</th>
   <?php $query="SELECT distinct(month(asignDate)) as asignDate FROM $table					
	   			where year(asignDate) = '$search_2' and (orgName = '$_words') order by $table.asignDate";
    ...
    <th><?php echo date('F', mktime(0, 0, 0, $asignDate, 10)); ?></th>
    <?php $counter++;} ?>
  </tr>
  <?php  $arr_Month=array_unique($arr_Month);$query="SELECT distinct ($table.source) FROM $table					
	   			where year(asignDate) = '$search_2' and (orgName = '$_words') order by $table.source";
 ...
 <?php foreach($arr_Month as $month){?>
   ...
   $query_inner="SELECT count(source) as source_num FROM $table
  				INNER JOIN invoice ON $table.invoiceNo=invoice.invoiceNo
	   			where year(asignDate) = '$search_2' and month(asignDate)= '$month' and (orgName = '$_words') and source='$lang'";
...
<td>Total</td>
 <?php  foreach($arr_Month as $month){ ?>
  ...
  $query_total_inner="SELECT count(source) as source_num FROM $table
						INNER JOIN invoice ON $table.invoiceNo=invoice.invoiceNo  
	   					where year(asignDate) = '$search_2' and month(asignDate)= '$month' and (orgName = '$_words')";
