...
else
{
	$query="SELECT $table.*, interpreter_reg.name, comp_reg.name as orgNam  FROM $table
	   ...
	   asignDate between '$search_2' and '$search_3' 
	   order by orgName";
}
$result = mysqli_query($con, $query);
...
$tbl.=<<<EOD
<div>
<h2 align="center"><u>Client General Report – Translation Interpreting</u></h2>
...
while($row = mysqli_fetch_assoc($result))
{
	...
	$C_otherCharg=$row["C_otherCharg"] + $C_otherCharg;
...
<td>{$misc->numberFormat_fun($C_otherCharg)}</td>
