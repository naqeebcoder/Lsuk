...
<table class="aa" border="1" cellspacing="0" cellpadding="0" style="width:250px">
  ...
  <?php $query="SELECT distinct (telephone.source) FROM telephone						
	   			where (telephone.orgName = '$_words') and telephone.assignDate between '$search_2' and '$search_3'
				order by source
				";
 ...
 <?php foreach($arr as $orgName){$orgName=$orgName;?>
  ...
  $query_inner="SELECT count(telephone.source) as source_num FROM telephone
  				INNER JOIN invoice ON telephone.invoiceNo=invoice.invoiceNo
	   			where telephone.orgName = '$orgName' and telephone.source='$lang' and telephone.assignDate between '$search_2' and '$search_3'
				";
...
<td>Total</td>
 <?php  foreach($arr as $orgName){$orgName=$orgName;?>
   ...
   <?php	 
  $query_total_inner="SELECT count(telephone.source) as source_num FROM telephone
						INNER JOIN invoice ON telephone.invoiceNo=invoice.invoiceNo  
	   					where telephone.orgName = '$orgName' and telephone.assignDate between '$search_2' and '$search_3'
						";
...
<td>Total Cost</td>
 <?php foreach($arr as $orgName){$orgName=$orgName;?>
   ...
   <?php	 
  $query_total_inner="SELECT sum(telephone.total_charges_comp) as total_charges_comp, sum(telephone.total_charges_comp * telephone.cur_vat) as total_charges_comp_vat FROM telephone
						INNER JOIN invoice ON telephone.invoiceNo=invoice.invoiceNo  
	   					where telephone.orgName = '$orgName' and telephone.assignDate between '$search_2' and '$search_3'						
						";
	   $result_total_inner = mysqli_query($con, $query_total_inner);while($row_total_inner = mysqli_fetch_assoc($result_total_inner)){?>
