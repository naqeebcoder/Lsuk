...
$i=1;$table='telephone';$C_otherCharges=0;$C_chargInterp=0;$total_charges_comp=0;$C_admnchargs=0;
...
if(!empty($search_1))
{
	$query="SELECT $table.*, interpreter_reg.name, comp_reg.name as orgNam  FROM $table
	   ...
	   assignDate between '$search_2' and '$search_3' and (orgName like '%$_words%')  
	   order by orgName";
}
else
{
	$query="SELECT $table.*, interpreter_reg.name, comp_reg.name as orgNam  FROM $table
	   ...
	   inner join comp_reg on $table.orgName = comp_reg.abrv 
	   where $table.deleted_flag = 0 and $table.order_cancel_flag=0 and 
	   assignDate between '$search_2' and '$search_3'  
	   order by orgName";
}
...
$tbl.=<<<EOD
<div>
<h2 align="center"><u>Client General Report – Telephone Interpreting</u></h2>
...
while($row = mysqli_fetch_assoc($result))
{
	$C_chargInterp=$row["C_chargInterp"] + $C_chargInterp;$C_otherCharges=$row["C_otherCharges"] + 
		$C_otherCharges;$total_charges_comp=$row["total_charges_comp"] + 
			$total_charges_comp;$C_admnchargs=$row["C_admnchargs"] + $C_admnchargs; 
	if($row["rAmount"]>0)
	{
		$invstst='Paid';
	}
...
$tbl.=<<<EOD
    <tr>
		...
		<td>{$row["invoiceNo"]}</td>
		<td>{$row["dated"]}</td>
		<td>{$misc->dated($row["assignDate"])}<br/>{$row['assignTime']}</td>
		<td>{$row["source"]}</td>
		<td>{$row["name"]}</td>
		<td>{$row["C_hoursWorkd"]}</td>
		<td>{$row["C_rateHour"]}</td>
		<td>{$row["C_chargInterp"]}</td>
		<td>{$row["C_otherCharges"]}</td>		
...
<td></td>
<td>Total</td>
<td>{$misc->numberFormat_fun($C_chargInterp)}</td>
<td>{$misc->numberFormat_fun($C_otherCharges)}</td>
<td>{$misc->numberFormat_fun($C_admnchargs)}</td>		
