...
if($search_4=='Management'){
	$query_outer="SELECT comp_credit.porder, comp_credit.mode, comp_credit.credit from comp_credit
	where comp_credit.orgName='$search_1' and comp_credit.porder='$search_4' and comp_credit.porder<>'' 
	";
}else{$query_outer="SELECT comp_credit.porder, comp_credit.mode, comp_credit.credit from comp_credit
	where comp_credit.orgName='$search_1' and comp_credit.porder<>'' 
	";}
...
$htmlTable .='<h2 style="text-decoration:underline; text-align:center">Purchase Order Report for "'.$comp_name.'"</h2>
...
<tr>
	...
	<th style="background-color:#039;color:#FFF;">P Order # </th>
...
while($row_outer = mysqli_fetch_assoc($result_outer)){$porder=$row_outer["porder"];$mode=$row_outer["mode"];$credit=$row_outer["credit"];
   
		$htmlTable .=' <tr><td></td>
		<td></td>';
		$htmlTable .='<td>'.$porder.'</td>';
		$htmlTable .='<td>'.$credit.'</td>';
		$htmlTable .='<td></td>';
		$htmlTable .='<td></td>
    </tr>';
...
$query="SELECT interpreter.invoiceNo,(interpreter.total_charges_comp + interpreter.C_otherexpns + interpreter.total_charges_comp * interpreter.cur_vat) as total_charges_comp, 'Interpreter' as tbl from interpreter	
	where interpreter.orgName='$search_1' and interpreter.porder='$porder' and interpreter.porder <>'' and interpreter.assignDate BETWEEN '$search_2' AND '$search_3'
	union
	...
	where telephone.orgName='$search_1' and telephone.porder='$porder' and telephone.porder <>'' and telephone.assignDate BETWEEN '$search_2' AND '$search_3'
	union
	...
	where translation.orgName='$search_1' and translation.porder='$porder' and translation.porder <>'' and translation.asignDate BETWEEN '$search_2' AND '$search_3'
