...
$query="SELECT $table.*, interpreter_reg.name FROM $table
	   ...
	   where $table.deleted_flag = 0 and $table.order_cancel_flag=0 and $table.asignDate between '$search_2' and '$search_3' 
...
if ($loop!=0)
  ShowCompTotal($mapCoTotals,$htmlTable);
	...
	$htmlTable .='<tr>';      
	$htmlTable .='<td style="font-weight:bold;"  colspan="3" align="right">Total</td>';
...
function ShowCompTotal(&$map,&$tbl)
{
  ...
  <td style="font-weight:bold;" colspan="3" align="right"> Interp. Tot:</td>
  <td style="font-weight:bold;" >{$misc->numberFormat_fun($map["total_charges_interp"])}</td>
  <td></td>
  <td></td>
  <td></td>
  <td></td>
