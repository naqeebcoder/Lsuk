...
foreach ($res as $row) {
	...
	if ($bank_info['sort_code']) {
		$sort_code = '<br>Sort Code: ' . $bank_info['sort_code'];
	}
	if ($bank_info['iban_no']) {
		$iban_no = '<br>Iban No: ' . $bank_info['iban_no'];
	}
	...
	$htmlTable .= '<td>' . $payment_type .
		'<p style="font-size: 11px; margin-bottom:0;">
				Bank: ' . $bank_info['bank_name'] . ' <br>
				Account#: ' . $bank_info['account_no'] . $sort_code . $iban_no . '</p> </td>';
