<?php include '../../db.php';include_once ('../../class.php'); $excel=@$_GET['excel'];$search_1=$_GET['search_1'];$search_2=@$_GET['search_2'];$search_3=@$_GET['search_3'];$counter=0;$x=0; $source_num=0;$table='translation';$org='';
...
$htmlTable .='<h2 style="text-decoration:underline; text-align:center">Company Wise Consolidate Report(Tranlation)</h2>
...
$htmlTable.='<th>'.$orgName.'</th>';
    $counter++;}
$htmlTable.='</tr>';
$query="SELECT distinct (translation.source) FROM translation						
	   			where (translation.orgName = '$_words') and translation.asignDate between '$search_2' and '$search_3'
				order by source";
...
foreach($arr as $orgName){$orgName=$orgName;$x=$counter;$u=0; while($x>$u){ 
  $query_inner="SELECT count(translation.source) as source_num FROM translation
  				INNER JOIN invoice ON translation.invoiceNo=invoice.invoiceNo
	   			where translation.orgName = '$orgName' and translation.source='$lang' and translation.asignDate between '$search_2' and '$search_3'
				";
...
$htmlTable.='<td>Total</td>';
 foreach($arr as $orgName){$orgName=$orgName;	 
  $query_total_inner="SELECT count(translation.source) as source_num FROM translation
						INNER JOIN invoice ON translation.invoiceNo=invoice.invoiceNo  
	   					where translation.orgName = '$orgName' and translation.asignDate between '$search_2' and '$search_3'";
...
$htmlTable.='<td>Total Cost</td>';
 foreach($arr as $orgName){$orgName=$orgName;
  $query_total_inner="SELECT sum(translation.total_charges_comp) as total_charges_comp, sum(translation.total_charges_comp * translation.cur_vat) as total_charges_comp_vat FROM translation
						INNER JOIN invoice ON translation.invoiceNo=invoice.invoiceNo  
	   					where translation.orgName = '$orgName' and translation.asignDate between '$search_2' and '$search_3'";
	   $result_total_inner = mysqli_query($con, $query_total_inner);while($row_total_inner = mysqli_fetch_assoc($result_total_inner)){
