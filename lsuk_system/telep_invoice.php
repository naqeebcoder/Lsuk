...
include "loadinvoicedbtelep.php";
  ...
  if (isset($_POST['submit'])) {
    ...
    if ($commit == 0 || @$invoic_date == '0000-00-00') {
      ...
      $acttObj->insert("daily_logs", array("action_id" => 16, "user_id" => $_SESSION['userId'], "details" => "TP Job ID: " . $invoice_id));
      ...
      */
      ...
      $makSub_total = floatval(preg_replace('/[^\d.]/', '', @$sub_total));
      $makVat = floatval(preg_replace('/[^\d.]/', '', $misc->numberFormat_fun(floatval(preg_replace('/[^\d.]/', '', @$sub_total)) * @$cur_vat)));
      $credit_amount = $misc->numberFormat_fun($makSub_total + $makVat);

      $description = '[Remote] ' . @$source . " to " . @$target . " on " . $misc->dated(@$assignDate) . " " . $misc->timeFormat(@$assignTime);
  ...
  <form action="" method="post" class="form-inline">
    <div class="form-group"><input type="submit" class='prnt btn btn-primary' name="submit" value="Press to Print" />
  ...
  <div class="container-fluid">
    ...
    <table class="table table-bordered">
      ...
      <tbody>
        <tr>
          ...
          <td>Remote (<?= $communication_type; ?>)</td>
    ...
    <table class="table table-bordered" style="text-transform: uppercase;">
      <thead class="bg-info">
        <tr>
            <th>PER <?= $remote_unit_in_hours == 1 ? 'HOUR' : 'MINUTE' ?> PRICE (£)</th>
            <th>DURATION IN <?= $remote_unit_in_hours == 1 ? 'HOURS' : 'MINUTES' ?></th>
            <th style="text-align:center">INTERPRETING CHARGES (£)</th>
        </tr>
      </thead>
      <tbody>
        <tr>
            <td><a class="cut">-</a>
                <span class="total">
                    ...
                    if (!empty($row['remote_unit_in_hours']) && $row['remote_unit_in_hours'] == 1) {
                        echo $misc->numberFormat_fun($rateHour*60); 
                    } else {
                        echo $misc->numberFormat_fun($rateHour); 
                    }
                    ?>
                </span>
            </td>
            ...
            <td>
                <span class="total">
                    <?php 
                    if (!empty($row['remote_unit_in_hours']) && $row['remote_unit_in_hours'] == 1) {
                        echo formatDuration($hoursWorkd); 
                    } else {
                        echo $misc->numberFormat_fun($hoursWorkd); 
                    }
                    ?>
                </span>
            </td>
          ...
          ?>
          <td align="center"><span class="total"><?php echo $misc->numberFormat_fun($makRateHour * $makHoursWord); ?></span></td>
        </tr>
      </tbody>
    </table>
    <table class="table table-bordered" style="text-transform: uppercase;">
    ...
    <?php
    $part_amount = array();
    $part_pay = $acttObj->read_all('id,amount,dated', 'partial_amounts', ' status="1" and tbl="tp" and order_id="' . $invoice_id . '"');
      ...
      <table class="table table-bordered">
        ...
        <tbody>
          <tr>
            ...
            ?>
            <td><span style="color:#000"><?php echo $misc->numberFormat_fun(($makSub_total + $makVat - $rAmount)); ?></span></td>
          </tr>
        </tbody>
      </table>
    <?php } ?>
    <aside><br>
      <?php echo $write_cancellation; ?><br>
      ...
      <div style="width:95%" align="center">
        <p style=" font-size:14px;" align="center">Please make all cheques payable to Language Services UK Limited for BACS payment Sort Code 20-13-34 Account Number 33161234.
          ...
          Thank You Doing For Business With Us<br /><br />
