...
<html lang="en">
  <head>
    ...
    <style>      @media print {
      ...
      @page { margin: 0; }</style>
      <?php  if(isset($_POST['submit'])){$acttObj->editFun($table,$slip_id,'intrp_salary_comit',1);?><script>window.print()</script><style>.prnt{  display:none; }</style><?php } ?>
  </head>
  <body>
   <div><form action="" method="post"><input type="submit" class='prnt' name="submit" value="Press to Print" style="background-color:#06F; color:#FFF; border:1px solid #09F"onclick="printpage();"/> | <input type="submit" class='prnt' name="email" value="Press to Email" style="background-color:#06F; color:#FFF; border:1px solid #09F"/></form></div>
...
where intrpName=$slip_id and commit=1 and intrp_salary_comit = 0 and intrp_salary_comit = 0 and
 assignDate  
...
while($row = mysqli_fetch_array($result)){?>
          <tr>
           ...
           <?php  if(isset($_POST['submit'])){ $acttObj->editFun('interpreter',$row['id'],'intrp_salary_comit',1);}?> 
          <?php } ?>
        ...
        <?php $i=1;
$query="SELECT * FROM telephone where intrpName=$slip_id and commit=1 and intrp_salary_comit = 0 and
 assignDate  
BETWEEN('$fdate')AND('$tdate')";			
$result = mysqli_query($con,$query);
while($row = mysqli_fetch_array($result)){?>
          <tr>
           ...
           <?php  if(isset($_POST['submit'])){ $acttObj->editFun('telephone',$row['id'],'intrp_salary_comit',1);}?> 
          <?php } ?>
        ...
        <?php $i=1;
$query="SELECT * FROM translation where intrpName=$slip_id and commit=1 and intrp_salary_comit = 0 and
 asignDate  
BETWEEN('$fdate')AND('$tdate')";				
$result = mysqli_query($con,$query);
while($row = mysqli_fetch_array($result)){?>
          ...
          <?php  if(isset($_POST['submit'])){ $acttObj->editFun('translation',$row['id'],'intrp_salary_comit',1);}?> 
          <?php } ?>
        </tbody>
        <tfoot>
          <tr>
