...
$allowed_type_idz = "236";
//Check if user has current action allowed
if ($_SESSION['is_root'] == 0) {
	$get_page_access = $acttObj->read_specific("id", "action_permissions", "user_id=" . $_SESSION['userId'] . " AND action_id IN (" . $allowed_type_idz . ")")['id'];
	if (empty($get_page_access)) {
		die("<center><h2 class='text-center text-danger'>You do not have access to this page!<br>Kindly contact admin for further process.</h2></center>");
	}
}
...
<head>
	<title>Journal Ledger Bank Statement</title>
...
<body>
	...
	<section class="container-fluid" style="overflow-x:auto">
			...
			<center>
				<h2 class="col-md-4 col-md-offset-4 text-center m-b-50">
					<div class="label label-primary">
						<a href="<?php echo basename(__FILE__); ?>">Journal Ledger Bank</a>
					</div>
				</h2>
			</center>
		</div>
		...
		<div class="col-md-12 m-b-30">
			<div class="row">
				<div class="form-group col-md-2 col-sm-3">
					...
					<input type="text" name="invoice_no" id="invoice_no" placeholder='Invoice/Track#' class="form-control" onChange="myFunction()" value="<?php echo $invoice_no; ?>" />
				</div>
				<div class="form-group col-md-2 col-sm-3">
				...
				<div class="form-group col-md-2 col-sm-3">
					...
					<select name="category" id="category" class="form-control" onChange="myFunction()">
						...
						<option value="interpreter_salary" <?php echo ($category == 'interpreter_salary') ? "selected" : ""; ?>>Interpreter Salary</option>
						<?php 
							// $categories = $acttObj->full_fetch_array('SELECT DISTINCT tbl FROM account_journal_ledger WHERE tbl IS NOT NULL');
							// foreach($categories as $cate){
							// 	echo '<option value="'.$cate['tbl'].'" '.(($category == '.$cate[`tbl`].') ? "selected" : "").'>'.ucwords(str_replace('_',' ', $cate['tbl'])).'</option>';
							// }
						?>
					</select>
				</div>
				<div class="form-group col-md-2 col-sm-3">
					<label>Banks</label>
					<?php
					$bank_infos = $acttObj->full_fetch_array("SELECT id, name as bank_name, account_no, sort_code, iban_no FROM account_payment_modes WHERE is_bank = 1 AND status = 1 ORDER BY name");
					?>
					<select name="bank_id" id="bank_id" class="form-control" onChange="myFunction()">
				...
				<div class="form-group col-md-2 col-sm-3">
					<label>Status</label>
					<?php
					$journal_status = $acttObj->full_fetch_array("SELECT DISTINCT status FROM account_journal_ledger WHERE is_bank = 1 ORDER BY status");
					?>
					<select name="status" id="status" class="form-control" onChange="myFunction()">
						...
						<?php foreach ($journal_status as $j_status) { ?>
							<?php 
								$selected = '';
								if(!isset($_GET['status']) && $j_status['status'] == 'paid') {
									$selected = 'selected';  
								} else if (isset($_GET['status']) && $j_status['status'] == $_GET['status']){
									$selected = 'selected';  
								}
								?>
							<option value="<?php echo $j_status['status']; ?>" <?php echo $selected; ?>>
		...
		<div class="col-md-12">
			...
			if (isset($category)) {
				if ($category === 'all') {
					$conditions .= " AND is_receivable IN (0,1,2,3,4)";
				...
				} elseif ($category === 'interpreter_salary') {
					$conditions .= " AND is_receivable = 3";
				} elseif ($category === 'taxes') {
					$conditions .= " AND is_receivable = 4";
				}
			}
			...
			<table class="table table-striped table-bordered table-condensed table-hover" id="dt_receivable">
				...
				<tbody>
					...
					if (count($res) > 0) {
						foreach ($res as $row) {
					?>
							<tr data-toggle="tooltips"
								style="<?php echo ($row['status'] != 'paid') ? 'background-color: #dfa4211a' : ''; ?>"
								title="<?php echo ($row['status'] != 'paid') ? ucwords(str_replace('_', ' ', $row['status'])) : ''; ?>">
								<td>
									<?= $row["invoice_no"]; ?><br>
									<label class="label label-<?= $row['is_receivable'] == 1 ? 'success' : 'danger'; ?>">
								...
								<td>
									...
									if ($row["payment_type"] == 'bacs') {
										...
										if ($row['payment_type'] == 'card') {
											...
											$payment_type = ucwords($row["payment_type"]);
										}
									}
									echo $payment_type;

									$bank_info = $acttObj->read_specific("name as bank_name, account_no, sort_code, iban_no", "account_payment_modes", " is_bank = 1 AND id = " . $row['account_id']);

									echo '<p class="fs-10 m-b-0">
									Bank: ' . $bank_info['bank_name'] . ' <br>
									Account#: ' . $bank_info['account_no'];

									if ($bank_info['sort_code']) {
										echo '<br>Sort Code: ' . $bank_info['sort_code'];
									}
									if ($bank_info['iban_no']) {
										echo '<br>Iban No: ' . $bank_info['iban_no'];
									}
