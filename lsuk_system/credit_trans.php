...
<body>
  ...
  if (isset($_POST['submit'])) {
    $data = $acttObj->read_specific("*", "translation", "id=" . $invoice_id);
    unset(
      $data['remrks'],
      $data['I_Comments'],
      $data['aloct_by'],
    ...
    $check_existing = $acttObj->read_specific("CONCAT(id) as ids", "credit_notes", "order_id=" . $invoice_id . " AND order_type='tr' AND status=1")['ids'];
    if (!empty($check_existing)) {
      $acttObj->update_custom("credit_notes", array("status" => 0), "id IN (" . $check_existing . ")");
    }
    $is_inserted = $acttObj->insert("credit_notes", array("order_id" => $row['id'], "order_type" => "tr", "data" => $data, "inserted_by" => $_SESSION['userId']));
    if ($is_inserted) {
      $insert_id = $acttObj->read_specific("id,CONCAT(DATE_FORMAT(dated,'%y%m'),'_',LPAD(id, 3, '0')) as credit_note_no", "credit_notes", "order_id=" . $invoice_id . " AND order_type='tr' AND status=1");
      ...
      */
        ...
        $description = '[Credit Note][Translation] ' . $row['source'] . " to " . $row['target'] . " on " . $misc->dated($asignDate);

        $makunitCost = $misc->numberFormat_fun($row['C_numberUnit'] * $row['C_rpU']);
        $makWordCost = $misc->numberFormat_fun($row['C_numberWord'] * $row['C_rpW']);
        ...
        $total = $misc->numberFormat_fun($makunitCost + $makWordCost + $certificationCost + $postageCost + $proofCost + $C_otherCharg + $C_admnchargs);

        $cur_vat = $row['cur_vat'];
        $makVat = floatval(preg_replace('/[^\d.]/', '', $misc->numberFormat_fun($total * $cur_vat)));

        $credit_amount = $misc->numberFormat_fun($makVat + $total);
      ...
      $acttObj->update("translation", array("credit_note" => $insert_id['id'], "commit" => 0, "rAmount" => 0), array("id" => $invoice_id));
      $acttObj->update('partial_amounts', array("status" => 0), array("order_id" => $invoice_id, "status" => 1, "tbl" => "tr"));
      $acttObj->insert('daily_logs', ['action_id' => 17, 'user_id' => $_SESSION['userId'], 'details' => "Invoice No: " . @$invoiceNo]);
    }
  }
  $old_credit_note = $acttObj->read_specific("id,CONCAT(DATE_FORMAT(dated,'%y%m'),'_',LPAD(id, 3, '0')) as credit_note_no", "credit_notes", "order_id=" . $invoice_id . " AND order_type='tr' AND status=1");
  ...
  if (isset($_POST['company_email'])) {
    ...
    <script>
      window.location.href = "./reports_lsuk/pdf/sendcreditnotemail.php?loaddb=load_credit_tr.php&htm=creditnote_tr.htm&invoice_id=<?php echo $invoice_id . '&company_email=' . $makCompanyEmail . '&table=translation'; ?>";
    </script>
  <?php }
  if (!empty($credit_note)) {
    $append_credit_invoiceNo = $invoiceNo . "-0" . $acttObj->read_specific("count(*)-1 as counter", "credit_notes", "order_id=" . $invoice_id . " and order_type='tr'")['counter'];
  } ?>
  <br>
