...
<head>
	<title>Payable Statement - Accounts</title>
...
<body>
	...
	<section class="container-fluid" style="overflow-x:auto">
			...
			<center>
				<h2 class="col-md-4 col-md-offset-4 text-center m-b-50">
					<div class="label label-primary">
						<a href="<?php echo basename(__FILE__); ?>">Payable Statement</a>
					</div>
				</h2>
			</center>
		</div>
		...
		<div class="row">
			<div class="col-md-12 m-b-30">
				<div class="form-group col-md-2 col-sm-3">
					<label>Track#/Invoice No</label>
					<input type="text" name="invoice_no" id="invoice_no" placeholder='Invoice No' class="form-control" onChange="myFunction()" value="<?php echo $invoice_no; ?>" />
				</div>
				<div class="form-group col-md-2 col-sm-3">
		...
		<div class="col-md-12">
			...
			$opening_balance_conditions = $conditions;
			...
			$query = "
				WITH opening_balance_cte AS (
					SELECT 
						ROUND(IFNULL(SUM(credit), 0) - IFNULL(SUM(debit), 0), 2) AS opening_balance
					FROM 
						account_payables  -- or account_expenses, depending on your schema
					WHERE 
						id < (
							SELECT MIN(id) 
				...
				running_balance_cte AS (
					SELECT 
						t.*,
						SUM(credit - debit) OVER (ORDER BY id ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS transaction_running_balance
					FROM 
