...
$query="SELECT *, interpreter_reg.name  FROM $table
...
$pdf->WriteHTML('<para><h1>            Language Services UK Limited<br>         Translation and Interpreting Service</h1><br><u>Suite 2 Davis House Lodge Causeway Trading Estate<br>Lodge Causeway - Fishponds Bristol BS163JB</u></para><br><br><br><br><u>Account Statement Report for "'.$comp_name.'"<br>Paid Invoices Face to Face Interpreting<br><br>Date Range: '.$misc->dated($search_2).' to '.$misc->dated($search_3).'<br><br>Report Date: '.$misc->sys_date().'</u>');
...
while($row = mysqli_fetch_assoc($result)){$g_total=$row["total_charges_comp"] + round($row["total_charges_comp"]*.2,2) + $C_otherCost=$row["C_otherCost"] + $g_total;$g_vat=$row["total_charges_comp"]*0.2 +$g_vat;$C_otherCost=$row["C_otherCost"] + $C_otherCost;$non_vat=$row["total_charges_comp"] + $non_vat;
$htmlTable .='<tr>';
$htmlTable .='<td>'.$i.'</td>';
