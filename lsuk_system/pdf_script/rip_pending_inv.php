...
$i=1;$table='interpreter';$total_charges_comp=0;$C_otherCharges=0;$g_total=0;$g_vat=0;$C_otherCharges=0;$non_vat=0;
...
$pdf->WriteHTML('<para><h1>            Language Services UK Limited<br>         Translation and Interpreting Service</h1><br><u>Suite 2 Davis House Lodge Causeway Trading Estate<br>Lodge Causeway - Fishponds Bristol BS163JB</u></para><br><br><br><br><u>Account Statement Report for "'.$comp_name.'"<br>Pending Invoices Face to Face Interpreting<br><br>Date Range: '.$misc->dated($search_2).' to '.$misc->dated($search_3).'<br><br>Report Date: '.$misc->sys_date().'</u>');
...
$htmlTable .='<tr>
    ...
    <td>Non-VAT Costs</td>
    <td>Invoice Total</td>';
while($row = mysqli_fetch_assoc($result)){$total=$row["total_charges_comp"] + round($row["total_charges_comp"]*.2,2) + $C_otherCost=$row["C_otherCost"] ;$g_total= $total + $g_total;$non_vat=$row["total_charges_comp"] + $non_vat;$g_vat=round($row["vat"],2) +$g_vat;
$htmlTable .='<tr>';
$htmlTable .='<td>'.$i.'</td>';
