...
$i=1;$table='translation';$total_charges_comp=0;$C_otherCharg=0;$g_total=0;$g_vat=0;$C_otherCharg=0;$non_vat=0;$comp_name=$acttObj->unique_data('comp_reg','name','abrv',$search_1);
$query="SELECT *, total_charges_comp * 0.2 as vat, interpreter_reg.name  FROM $table
	   ...
	   where  $table.orgName like '$search_1%' and asignDate between '$search_2' and '$search_3' and  rAmount > 0";
...
$pdf->WriteHTML('<para><h1>            Language Services UK Limited<br>         Translation and Interpreting Service</h1><br><u>Suite 2 Davis House Lodge Causeway Trading Estate<br>Lodge Causeway - Fishponds Bristol BS163JB</u></para><br><br><br><br><u>Account Statement Report for "'.$comp_name.'"<br>Paid Invoices Translation<br><br>Date Range: '.$misc->dated($search_2).' to '.$misc->dated($search_3).'<br><br>Report Date: '.$misc->sys_date().'</u>');
...
while($row = mysqli_fetch_assoc($result)){$g_total=$row["total_charges_comp"] + round($row["total_charges_comp"]*.2,2) + $C_otherCost=$row["C_otherCharg"] + $g_total;$g_vat=round($row["vat"],2) +$g_vat;$C_otherCharg=$row["C_otherCharg"] + $C_otherCharg;$non_vat=$row["total_charges_comp"] + $non_vat;
$htmlTable .='<tr>';
$htmlTable .='<td>'.$i.'</td>';
...
$htmlTable .='<td>'.round( $row["total_charges_comp"] + $row["total_charges_comp"] *0.2 + $row["C_otherCharg"],2).'</td>';
