...
$result = mysqli_query($con, $query);while($row = mysqli_fetch_assoc($result)){ $source=$row['source'];$assignDate=$row['assignDate'];$assignTime=$row['assignTime'];$buildingName=$row['buildingName'];$street=$row['street'];$assignCity=$row['assignCity'];$postCode=$row['postCode'];$orgRef=$row['orgRef'];$orgName=$row['orgName'];$name=$row['name'];$orgContact=$row['orgContact']; }
...
<br><br><u>Place of Assignment: '.$buildingName.' '.$buildingName.'  '.$street.'  '.$assignCity.'  '.$postCode.'</u>
