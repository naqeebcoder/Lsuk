...
$query="SELECT $table.*, interpreter_reg.name  FROM $table
	   ...
	   where $table.deleted_flag = 0 and $table.order_cancel_flag=0 and asignDate between '$search_2' and '$search_3' and comp_reg.abrv='$search_1' order by asignDate";}
	   else{$query="SELECT $table.*, interpreter_reg.name  FROM $table
	   inner join interpreter_reg on $table.intrpName = interpreter_reg.id 
	   where $table.deleted_flag = 0 and $table.order_cancel_flag=0 and asignDate between '$search_2' and '$search_3' order by asignDate";}
$result = mysqli_query($con, $query);
...
while($row = mysqli_fetch_assoc($result)){ $rpuXnumb=$row["rpU"]*$row["numberUnit"] + $rpuXnumb;$otherCharg=$row["otherCharg"] + $otherCharg;$total_charges_comp=$row["total_charges_comp"] + $total_charges_comp;
...
$htmlTable .='<td>'.$row["otherCharg"].'</td>';
