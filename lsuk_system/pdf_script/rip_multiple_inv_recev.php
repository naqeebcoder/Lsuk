<?php include '../db.php';include_once ('../class.php'); $proceed=$_GET['proceed'];$multInvoicNo=@$_GET['multInvoicNo']; $search_1=@$_GET['search_1'];$search_2=@$_GET['search_2'];$search_3=@$_GET['search_3'];$i=1;$table='interpreter';$g_total_interp=0;$g_total_telep=0;$g_total_trans=0;$non_vat=0;$non_vat_tlep=0;$non_vat_trans=0;$non_vat_interp=0;$withou_VAT_interp=0;$withou_VAT_telp=0;$withou_VAT_trans=0;$C_travelCost=0;$C_rateMile_cost=0;$C_admnchargs_interp=0;$C_admnchargs_telep=0;$C_admnchargs_trans=0;$C_otherexpns=0;$total=0;$C_chargeTravelTime=0;

$query="SELECT  due_date,dated FROM mult_inv 	   
	   where m_inv='$multInvoicNo'";$result = mysqli_query($con, $query);while($row = mysqli_fetch_assoc($result)){$rep_date=$row["dated"];$due_date=$row["due_date"];}
...
$query="SELECT interpreter.*, interpreter_reg.name  FROM interpreter
	   ...
	   where interpreter.deleted_flag = 0 and interpreter.order_cancel_flag=0 and  interpreter.multInvoicNo='$multInvoicNo' and interpreter.commit=0 and interpreter.multInv_flag=1 and assignDate between '$search_2' and '$search_3' and interpreter.orgName='$search_1' order by assignDate Asc ";
...
$pdf->WriteHTML('<para><h1>            Language Services UK Limited<br>         Translation and Interpreting Service</h1><br>Suite 2 Davis House Lodge Causeway Trading Estate<br>    <u>Lodge Causeway - Fishponds Bristol BS163JB</u></para><br><br>Invoice ('.@$multInvoicNo.')'.'<br><br>Invoice Date: '.$misc->dated($rep_date).'<br><br>'.@$name .'<br>'. @$buildingName .'<br>'. @$line1 .'<br>'. @$line2 .'<br>'. @$streetRoad .'<br>'. @$city .' '. @$postCode.'<u><br>Date Range: '.$misc->dated($search_2).' to '.$misc->dated($search_3).'<br>Invoice Due Date: '.$misc->dated($due_date).'</u>');
...
$htmlTable .='<tr>
    ...
    <td>Job<br>Cost</td>
    <td>Travel Cost</td>      
    <td>Travel<br>Expenses</td>                  
    <td width="300">Admn<br>Charges</td>  
    <td>Total<br>Vatable Cost</td>    
    <td>Other Expenses<br>Non-Vatable</td>       
    <td>Job<br>Notes</td>';
...
$query_telep="SELECT telephone.*, interpreter_reg.name  FROM telephone
	   ...
	   where telephone.deleted_flag = 0 and telephone.order_cancel_flag=0 and  telephone.multInvoicNo='$multInvoicNo' and telephone.commit=0 and telephone.multInv_flag=1 and assignDate between '$search_2' and '$search_3' and telephone.orgName='$search_1' order by assignDate Asc ";
...
$query_trans="SELECT translation.*, interpreter_reg.name  FROM translation
	   ...
	   where translation.deleted_flag = 0 and  translation.order_cancel_flag=0 and  translation.multInvoicNo='$multInvoicNo' and translation.commit=0 and translation.multInv_flag=1 and asignDate between '$search_2' and '$search_3' and translation.orgName='$search_1' order by asignDate  Asc ";
...
$htmlTable .='<td>'. $grand_total=$C_otherexpns + ($g_total_interp  + $g_total_telep + $g_total_trans) + number_format(($g_total_interp  + $g_total_telep + $g_total_trans)*0.2,2).'</td>';
$htmlTable .='<td></td>';
$htmlTable .='<td></td>
</tr>';
