...
$query="SELECT $table.*, interpreter_reg.name,interpreter_reg.city FROM $table
	   ...
	   where $table.deleted_flag = 0 and $table.order_cancel_flag=0 and asignDate between '$search_2' and '$search_3' and source='$search_1'";}
	   else{$query="SELECT $table.*, interpreter_reg.name,interpreter_reg.city FROM $table
	   inner join interpreter_reg on $table.intrpName = interpreter_reg.id
...
$pdf->WriteHTML('<para><h1>            Language Services UK Limited<br>         Translation and Interpreting Service</h1><br><u>Suite 2 Davis House Lodge Causeway Trading Estate<br>Lodge Causeway - Fishponds Bristol BS163JB</u></para><br><br><br><br><u>Language Report from "Translation" Interpreter<br><br>Date: '.$misc->sys_date().'</u>');
