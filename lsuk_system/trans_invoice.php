...
<body>
  ...
  if (isset($_POST['submit'])) {
    if ($commit == 0 || @$invoic_date == '0000-00-00') {
      ...
      $acttObj->insert("daily_logs", array("action_id" => 16, "user_id" => $_SESSION['userId'], "details" => "TR Job ID: " . $invoice_id));
      ...
      $vat = $misc->numberFormat_fun($makTotal * @$cur_vat);
      $makVat = floatval(preg_replace('/[^\d.]/', '', $vat));

      $credit_amount = $misc->numberFormat_fun($makVat + $makTotal);

      $description = '[Translation] ' . @$source . " to " . @$target . " on " . (($dueDate == '' || $dueDate == '0000-00-00') ? $misc->dated(date("Y-m-d", strtotime($pay_terms))) : $misc->dated($dueDate));      // Checking if record already exists
  ...
  if (isset($_POST['email'])) {
    ...
    if ($commit == 0 || @$invoic_date == '0000-00-00') {
      ...
      */
      ...
      $credit_amount = $misc->numberFormat_fun($makVat + $makTotal);
      $description = '[Translation] ' . @$source . " to " . @$target . " on " . (($dueDate == '' || $dueDate == '0000-00-00') ? $misc->dated(date("Y-m-d", strtotime($pay_terms))) : $misc->dated($dueDate));
  ...
  <div class="container-fluid">
    ...
    <table class="table table-bordered" style="text-transform: uppercase;">
      ...
      <tbody>
        <tr>
          <td><a class="cut">-</a><span class="total"><?php echo $C_rpU; ?></span></td>
    ...
    <?php
      $part_amount=array();
      $part_pay=$acttObj->read_all('id,amount,dated','partial_amounts',' status="1" and tbl="tr" and order_id="'.$invoice_id.'"');
      if($part_pay->num_rows > 0){
        ...
        <tbody>
          <tr>
            ...
            ?>
            <td><span style="color:#000"><?php echo $misc->numberFormat_fun(($makVat + $makTotal-$rAmount)); ?></span></td>
          </tr>
        </tbody>
      </table>
      <?php } ?>
