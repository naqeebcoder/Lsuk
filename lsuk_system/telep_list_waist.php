...
$sql_opt="SELECT distinct interpreter_reg.name FROM telephone
	   ...
	   where  telephone.deleted_flag=0 and telephone.order_cancel_flag=0 and  telephone.assignDate like '$assignDate%' and telephone.multInv_flag=0 and telephone.commit=0 and source like '$job%' and (orgName like '%$_words%') and telephone.nameRef like '%$our%' and telephone.orgRef like '%$ur%' and telephone.invoiceNo like '%$inov%'
 ORDER BY name ASC";
...
where telephone.multInv_flag=0 and telephone.commit=0 and (status <> 'Company Seized trading in this name or Company closed' or status <> 'Company Blacklisted')
  ...
  <input type="date" name="assignDate" id="assignDate" placeholder='' style="width:100px;border-radius: 5px;" onChange="myFunction()" value="<?php echo $assignDate; ?>"/>
	...
	<tbody style="font-size:11px;"> 
	  ...
	  switch($_SESSION['prv']){
		...
		where  $table.deleted_flag=0 and $table.order_cancel_flag=0 and  $table.invoiceNo='' and  $table.assignDate like '$assignDate%' and source like '%$job%' and interpreter_reg.name like '%$interp%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
		...
		$query="SELECT $table.*,interpreter_reg.name FROM $table
		JOIN interpreter_reg ON $table.intrpName=interpreter_reg.id	   
		where  $table.deleted_flag=0 and $table.order_cancel_flag=0 and  orderCancelatoin=0 and $table.hoursWorkd=0 and $table.assignDate like '$assignDate%' and  $table.multInv_flag=0  and $table.commit=0  and $table.multInv_flag=0 and source like '%$job%' and interpreter_reg.name like '%$interp%' and $table.orgName like '%$org%' order by assignDate and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
		LIMIT {$startpoint} , {$limit}";	
		break;  
	  }
