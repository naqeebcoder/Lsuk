...
$action_view_job = $_SESSION['is_root'] == 1 || in_array(70, $get_actions);
$action_edit_job = $_SESSION['is_root'] == 1 || in_array(71, $get_actions);
$action_update_expenses = $_SESSION['is_root'] == 1 || in_array(72, $get_actions);
$action_job_note = $_SESSION['is_root'] == 1 || in_array(73, $get_actions);
...
$action_view_invoice = $_SESSION['is_root'] == 1 || in_array(76, $get_actions);
$action_edited_history = $_SESSION['is_root'] == 1 || in_array(127, $get_actions);
$action_make_credit_note = $_SESSION['is_root'] == 1 || in_array(77, $get_actions);
$action_uncommit_payment = $_SESSION['is_root'] == 1 || in_array(78, $get_actions);
$action_purchase_order = $_SESSION['is_root'] == 1 || in_array(79, $get_actions);
$action_view_earnings = $_SESSION['is_root'] == 1 || in_array(80, $get_actions);
$action_export_to_excel = $_SESSION['is_root'] == 1 || in_array(81, $get_actions);
...
if (isset($_POST['disposed_submit'])) {
    $id = $_POST['id'];
    $table = $_POST['type'];
    $submitted_by = $_SESSION['UserName'];
    $disposed_reason = $_POST['disposed_reason'];
    $cur_time = date('Y-m-d H:i:s');
    if (!empty($table) && $table == 'Interpreter') {
        $sql = "UPDATE interpreter SET disposed_of = 1 WHERE id = $id";
        mysqli_query($con, $sql);
        $sql = "INSERT INTO jobnotes (jobNote,tbl,`time`,fid,submitted) VALUES ('$disposed_reason' , 'interpreter','$cur_time' , $id , '$submitted_by')";
        mysqli_query($con, $sql);
    }
    if (!empty($table) && $table == 'Telephone') {
        $sql = "UPDATE telephone SET disposed_of = 1 WHERE id = $id";
        mysqli_query($con, $sql);
        $sql = "INSERT INTO jobnotes (jobNote,tbl,`time`,fid,submitted) VALUES ('$disposed_reason' , 'telephone','$cur_time' , $id , '$submitted_by')";
        mysqli_query($con, $sql);
    }
    if (!empty($table) && $table == 'Translation') {
        $sql = "UPDATE translation SET disposed_of = 1 WHERE id = $id";
        mysqli_query($con, $sql);
        $sql = "INSERT INTO jobnotes (jobNote,tbl,`time`,fid,submitted) VALUES ('$disposed_reason' , 'translation','$cur_time' , $id , '$submitted_by')";
        mysqli_query($con, $sql);
    }
}
?>
...
<body>
    <script>
        function myFunction(e) {
            ...
            if ($("#over_dues").is(':checked')) {
                append_url += '&over_dues=on';
            }
            var over_due_days = $("#over_due_days").val();
            if (over_due_days) {
                append_url += '&over_due_days=' + over_due_days;
            }
            ...
            <header>
                <center>
                    <div style="margin-top: 15px;" class="form-group col-md-2 col-sm-4">
                        ...
                        <h2 class="col-md-3 text-center"><span class="label label-primary">CREDIT NOTES LIST</span></h2>
                    </a>
                    <div class="col-md-2 pull-right">
                        <label class="pull-left hidden"><input <?php if (isset($multi) && $multi == "on") {
                                                                    echo 'checked';
                                                                } ?> type="checkbox" id="multi"> Multi invoices</label>
                        <label style="margin-top:-4px;" class="pull-left"><input <?php if (isset($over_dues) && $over_dues == "on") {
                                                                                        echo 'checked';
                                                                                    } ?> type="checkbox" name="over_dues" id="over_dues"> Over Due <input <?php echo $over_due_days ? "value='" . $over_due_days . "'" : ""; ?> type="number" id="over_due_days" class="form-control input-sm" placeholder="Enter over due days ..."></label>
                    </div>
                </center>
                ...
                <div class="col-md-12"><br>
                    ...
                    <div class="form-group col-md-2 col-sm-4">
                        ...
                        if (isset($org) && !empty($org)) {
                            $append_orgName_int = " and interpreter.orgName IN (" . $all_cz['all_cz'] . ")";
                            $append_orgName_tp = " and telephone.orgName IN (" . $all_cz['all_cz'] . ")";
                            $append_orgName_tr = " and translation.orgName IN (" . $all_cz['all_cz'] . ")";
                            $append_orgName_all = " and orgName IN (" . $all_cz['all_cz'] . ")";
                        } else {
                            $append_orgName_int = "";
                            $append_orgName_tp = "";
                            $append_orgName_tr = "";
...
AND interpreter.intrpName=interpreter_reg.id AND interpreter.orgName=comp_reg.abrv AND interpreter.deleted_flag = 0 AND interpreter.disposed_of = 0 and interpreter.order_cancel_flag=0 $append_multi_int $append_over_dues_int and interpreter.commit=1 $po_string_int AND interpreter.credit_note!='' and interpreter.assignDate like '$assignDate%' and interpreter.invoic_date like '$invoic_date%' and (interpreter.orgName like '%$_words%')) as grp  ORDER BY name ASC";
...
AND telephone.intrpName=interpreter_reg.id AND telephone.orgName=comp_reg.abrv AND telephone.deleted_flag = 0 AND telephone.disposed_of = 0 and telephone.order_cancel_flag=0 $append_multi_tp $append_over_dues_tp and telephone.commit=1 $po_string_tp AND telephone.credit_note!='' and telephone.assignDate like '$assignDate%' and telephone.invoic_date like '$invoic_date%' and (telephone.orgName like '%$_words%')) as grp  ORDER BY name ASC";
...
AND translation.intrpName=interpreter_reg.id AND translation.orgName=comp_reg.abrv AND translation.deleted_flag = 0 AND translation.disposed_of = 0 and translation.order_cancel_flag=0 $append_multi_tr $append_over_dues_tr and translation.commit=1 $po_string_tr AND translation.credit_note!='' and translation.asignDate like '$assignDate%' and translation.invoic_date like '$invoic_date%' and (translation.orgName like '%$_words%')) as grp  ORDER BY name ASC";
                        } else {
                            $sql_opt = "SELECT DISTINCT name,id,gender,city from (SELECT distinct interpreter_reg.name,interpreter_reg.id,interpreter_reg.gender, interpreter_reg.city,interpreter.multInv_flag,interpreter.commit,interpreter.total_charges_comp,interpreter.rAmount,interpreter.orgName,interpreter.deleted_flag,interpreter.order_cancel_flag,
interpreter.porder,comp_reg.po_req FROM interpreter_reg,interp_lang,interpreter,comp_reg WHERE interpreter_reg.code=interp_lang.code
AND interpreter.intrpName=interpreter_reg.id AND interpreter.orgName=comp_reg.abrv AND interpreter.deleted_flag = 0 AND interpreter.disposed_of = 0 and interpreter.order_cancel_flag=0 $append_multi_int $append_over_dues_int and interpreter.commit=1 $po_string_int AND interpreter.credit_note!='' and interpreter.assignDate like '$assignDate%' and interpreter.invoic_date like '$invoic_date%' and (interpreter.orgName like '%$_words%')
...
telephone.porder,comp_reg.po_req FROM interpreter_reg,interp_lang,telephone,comp_reg WHERE interpreter_reg.code=interp_lang.code
AND telephone.intrpName=interpreter_reg.id AND telephone.orgName=comp_reg.abrv AND telephone.deleted_flag = 0 AND telephone.disposed_of = 0 and telephone.order_cancel_flag=0 $append_multi_tp $append_over_dues_tp and telephone.commit=1 $po_string_tp AND telephone.credit_note!='' and telephone.assignDate like '$assignDate%' and telephone.invoic_date like '$invoic_date%' and (telephone.orgName like '%$_words%')
...
AND translation.intrpName=interpreter_reg.id AND translation.orgName=comp_reg.abrv AND translation.deleted_flag = 0 AND translation.disposed_of = 0 and translation.order_cancel_flag=0 $append_multi_tr $append_over_dues_tr and translation.commit=1 $po_string_tr AND translation.credit_note!='' and translation.asignDate like '$assignDate%' and translation.invoic_date like '$invoic_date%' and (translation.orgName like '%$_words%')) as grp  ORDER BY name ASC";
                    ...
                    <div class="form-group col-md-2 col-sm-4">
                        <?php
                        if (!empty($type) && $type == 'Interpreter') {
                            $sql_opt = "SELECT DISTINCT id,name,abrv from (SELECT DISTINCT comp_reg.id,comp_reg.name,comp_reg.abrv,comp_reg.po_req,interpreter.porder,interpreter.multInv_flag,interpreter.commit,interpreter.total_charges_comp,interpreter.rAmount,interpreter.orgName,interpreter.deleted_flag,interpreter.order_cancel_flag FROM comp_reg,interpreter WHERE interpreter.orgName=comp_reg.abrv AND interpreter.deleted_flag=0 AND interpreter.order_cancel_flag=0 $append_multi_int $append_over_dues_int and interpreter.commit=1 and interpreter.assignDate like '$assignDate%' and interpreter.invoic_date like '$invoic_date%' $po_string_int  AND interpreter.credit_note!='') as grp 
                                ORDER BY name ASC";
                        } else if (!empty($type) && $type == 'Telephone') {
                            $sql_opt = "SELECT DISTINCT id,name,abrv from (SELECT DISTINCT comp_reg.id,comp_reg.name,comp_reg.abrv,comp_reg.po_req,telephone.porder,telephone.multInv_flag,telephone.commit,telephone.total_charges_comp,telephone.rAmount,telephone.orgName,telephone.deleted_flag,telephone.order_cancel_flag FROM comp_reg,telephone WHERE telephone.orgName=comp_reg.abrv AND telephone.deleted_flag=0 AND telephone.order_cancel_flag=0 $append_multi_tp $append_over_dues_tp and telephone.commit=1 and telephone.assignDate like '$assignDate%' and telephone.invoic_date like '$invoic_date%' $po_string_tp  AND telephone.credit_note!='') as grp 
                                ORDER BY name ASC";
                        } else if (!empty($type) && $type == 'Translation') {
                            $sql_opt = "SELECT DISTINCT id,name,abrv from (SELECT DISTINCT comp_reg.id,comp_reg.name,comp_reg.abrv,comp_reg.po_req,translation.porder,translation.multInv_flag,translation.commit,translation.total_charges_comp,translation.rAmount,translation.orgName,translation.deleted_flag,translation.order_cancel_flag FROM comp_reg,translation WHERE translation.orgName=comp_reg.abrv AND translation.deleted_flag=0 AND translation.order_cancel_flag=0 $append_multi_tr $append_over_dues_tr and translation.commit=1 and translation.asignDate like '$assignDate%' and translation.invoic_date like '$invoic_date%' $po_string_tr  AND translation.credit_note!='') as grp 
                                ORDER BY name ASC";
                        } else {
                            $sql_opt = "SELECT DISTINCT id,name,abrv from (SELECT DISTINCT comp_reg.id,comp_reg.name,comp_reg.abrv,comp_reg.po_req,interpreter.porder,interpreter.multInv_flag,interpreter.commit,interpreter.total_charges_comp,interpreter.rAmount,interpreter.orgName,interpreter.deleted_flag,interpreter.order_cancel_flag FROM comp_reg,interpreter WHERE interpreter.orgName=comp_reg.abrv AND interpreter.deleted_flag=0 AND interpreter.order_cancel_flag=0 $append_multi_int $append_over_dues_int and interpreter.commit=1 and interpreter.assignDate like '$assignDate%' and interpreter.invoic_date like '$invoic_date%' $po_string_int  AND interpreter.credit_note!=''
                                UNION SELECT DISTINCT comp_reg.id,comp_reg.name,comp_reg.abrv,comp_reg.po_req,telephone.porder,telephone.multInv_flag,telephone.commit,telephone.total_charges_comp,telephone.rAmount,telephone.orgName,telephone.deleted_flag,telephone.order_cancel_flag FROM comp_reg,telephone WHERE telephone.orgName=comp_reg.abrv AND telephone.deleted_flag=0 AND telephone.order_cancel_flag=0 $append_multi_tp $append_over_dues_tp and telephone.commit=1 and telephone.assignDate like '$assignDate%' and telephone.invoic_date like '$invoic_date%' $po_string_tp  AND telephone.credit_note!=''
                    ...
                    <div class="form-group col-md-2 col-sm-4">
                        <?php if (!empty($type) && $type == 'Interpreter') {
                            $sql_opt = "SELECT distinct lang.lang,interpreter.total_charges_comp,interpreter.rAmount,interpreter.cur_vat FROM lang,interpreter WHERE interpreter.source=lang.lang and interpreter.assignDate LIKE '$assignDate%' and interpreter.invoic_date LIKE '$invoic_date%' $append_multi_all $append_over_dues_all and commit=1 and deleted_flag=0 and disposed_of=0 and order_cancel_flag=0 and (round(rAmount,2) < round((total_charges_comp+(total_charges_comp*cur_vat)),2) or total_charges_comp =0) GROUP BY lang ORDER BY lang ASC";
                        } else if (!empty($type) && $type == 'Telephone') {
                            $sql_opt = "SELECT distinct lang.lang,telephone.total_charges_comp,telephone.rAmount,telephone.cur_vat FROM lang,telephone WHERE telephone.source=lang.lang and telephone.assignDate LIKE '$assignDate%' and telephone.invoic_date LIKE '$invoic_date%' $append_multi_all $append_over_dues_all and commit=1 and deleted_flag=0 and disposed_of=0 and order_cancel_flag=0 and (round(rAmount,2) < round((total_charges_comp+(total_charges_comp*cur_vat)),2) or total_charges_comp =0) GROUP BY lang ORDER BY lang ASC";
                        } else if (!empty($type) && $type == 'Translation') {
                            $sql_opt = "SELECT distinct lang.lang,translation.total_charges_comp,translation.rAmount,translation.cur_vat FROM lang,translation WHERE translation.source=lang.lang and translation.asignDate LIKE '$assignDate%' and translation.invoic_date LIKE '$invoic_date%' $append_multi_all $append_over_dues_all and commit=1 and deleted_flag=0 and disposed_of=0 and order_cancel_flag=0 and (round(rAmount,2) < round((total_charges_comp+(total_charges_comp*cur_vat)),2) or total_charges_comp =0) GROUP BY lang ORDER BY lang ASC";
                        } else {
                            $sql_opt = "SELECT DISTINCT lang from (SELECT distinct lang,interpreter.multInv_flag,interpreter.commit,interpreter.disposed_of,interpreter.deleted_flag,interpreter.order_cancel_flag,interpreter.assignDate,interpreter.invoic_date,interpreter.total_charges_comp,interpreter.rAmount,interpreter.cur_vat FROM lang,interpreter WHERE interpreter.source=lang.lang 
                            UNION ALL SELECT distinct lang,telephone.multInv_flag,telephone.commit,telephone.disposed_of,telephone.deleted_flag,telephone.order_cancel_flag,telephone.assignDate,telephone.invoic_date,telephone.total_charges_comp,telephone.rAmount,telephone.cur_vat FROM lang,telephone WHERE telephone.source=lang.lang 
                            UNION ALL SELECT distinct lang,translation.multInv_flag,translation.commit,translation.disposed_of,translation.deleted_flag,translation.order_cancel_flag,translation.asignDate as 'assignDate',translation.invoic_date,translation.total_charges_comp,translation.rAmount,translation.cur_vat FROM lang,translation WHERE translation.source=lang.lang) as grp WHERE commit=1 $append_multi_all $append_over_dues_all and deleted_flag=0 and order_cancel_flag=0 and disposed_of=0 and (round(rAmount,2) < round((total_charges_comp+(total_charges_comp*cur_vat)),2) or total_charges_comp =0) and assignDate LIKE '$assignDate%' and invoic_date LIKE '$invoic_date%' ORDER BY lang ASC";
                        } ?>
            ...
            if (!empty($type) && $type == 'Interpreter') {
                $query =
                    "SELECT * from (SELECT interpreter.porder,comp_reg.po_req,'Interpreter' as type,interpreter.id,interpreter.intrpName,interpreter.orgName,interpreter.inchEmail,interpreter_reg.name,interpreter.source,interpreter.target,interpreter.invoic_date,interpreter.assignDate,interpreter.assignTime,interpreter.orgContact,interpreter.submited, interpreter.aloct_by,interpreter.aloct_date,interpreter.dated,interpreter.hrsubmited,interpreter.comp_hrsubmited,interpreter.interp_hr_date, interpreter.comp_hr_date,interpreter.hoursWorkd,interpreter.C_hoursWorkd,interpreter.total_charges_comp,interpreter.cur_vat,interpreter.C_otherexpns, interpreter.credit_note,interpreter.C_admnchargs,interpreter.rAmount,interpreter.rDate,interpreter.sentemail,interpreter.printed,interpreter.printedby,interpreter.deleted_flag,interpreter.order_cancel_flag,interpreter.nameRef,interpreter.orgRef,interpreter.invoiceNo,interpreter.commit,interpreter.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM interpreter,interpreter_reg,comp_reg WHERE interpreter.intrpName=interpreter_reg.id AND interpreter.orgName=comp_reg.abrv $append_multi_int $append_over_dues_int AND interpreter.deleted_flag = 0 AND interpreter.disposed_of = 0 and interpreter.order_cancel_flag=0 and interpreter.commit=1 $po_string_int AND interpreter.credit_note!='' and interpreter.invoic_date like '$invoic_date%' and interpreter.assignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and ( (interpreter.source="' . $job . '" OR interpreter.target="' . $job . '") OR (interpreter.source="' . $job . '" AND interpreter.target="English") OR (interpreter.source="English" AND interpreter.target="' . $job . '")) ' : '') . " and interpreter_reg.name like '%$interp%' $p_org_ad $append_orgName_int and interpreter.invoiceNo like '%$inov%') as grp WHERE type like '%$type%' ORDER BY CONCAT(assignDate,' ',assignTime) LIMIT {$startpoint} , {$limit}";
            } else if (!empty($type) && $type == 'Telephone') {
                $query =
                    "SELECT * from (SELECT telephone.porder,comp_reg.po_req,'Telephone' as type,telephone.id,telephone.intrpName,telephone.orgName,telephone.inchEmail,interpreter_reg.name,telephone.source,telephone.target,telephone.invoic_date,telephone.assignDate,telephone.assignTime,telephone.orgContact,telephone.submited, telephone.aloct_by,telephone.aloct_date,telephone.dated,telephone.hrsubmited,telephone.comp_hrsubmited,telephone.interp_hr_date,telephone.comp_hr_date, telephone.hoursWorkd,telephone.C_hoursWorkd,telephone.total_charges_comp,telephone.cur_vat,telephone.calCharges as C_otherexpns,telephone.credit_note,telephone.C_admnchargs, telephone.rAmount,telephone.rDate,telephone.sentemail,telephone.printed,telephone.printedby,telephone.deleted_flag,telephone.order_cancel_flag,telephone.nameRef,telephone.orgRef,telephone.invoiceNo,telephone.commit,telephone.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM telephone,interpreter_reg,comp_reg WHERE telephone.intrpName=interpreter_reg.id AND telephone.orgName=comp_reg.abrv $append_multi_tp $append_over_dues_tp AND telephone.deleted_flag = 0 AND telephone.disposed_of = 0 and telephone.order_cancel_flag=0 and telephone.commit=1 $po_string_tp AND telephone.credit_note!='' and telephone.invoic_date like '$invoic_date%' and telephone.assignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and ( (telephone.source="' . $job . '" OR telephone.target="' . $job . '") OR (telephone.source="' . $job . '" AND telephone.target="English") OR (telephone.source="English" AND telephone.target="' . $job . '")) ' : '') . " and interpreter_reg.name like '%$interp%' $p_org_ad $append_orgName_tp and telephone.invoiceNo like '%$inov%') as grp WHERE type like '%$type%' ORDER BY CONCAT(assignDate,' ',assignTime) LIMIT {$startpoint} , {$limit}";
            } else if (!empty($type) && $type == 'Translation') {
                $query =
                    "SELECT * from (SELECT translation.porder,comp_reg.po_req,'Translation' as type,translation.id,translation.intrpName,translation.orgName,translation.inchEmail,interpreter_reg.name,translation.source,translation.target,translation.invoic_date,translation.asignDate as assignDate,'00:00:00' as assignTime,translation.orgContact,translation.submited, translation.aloct_by,translation.aloct_date,translation.dated,translation.hrsubmited,translation.comp_hrsubmited,translation.interp_hr_date,translation.comp_hr_date, translation.numberUnit as hoursWorkd,translation.C_numberUnit as C_hoursWorkd,translation.total_charges_comp,translation.cur_vat,0 as C_otherexpns,translation.credit_note,translation.C_admnchargs, translation.rAmount,translation.rDate,translation.sentemail,translation.printed,translation.printedby,translation.deleted_flag,translation.order_cancel_flag,translation.nameRef,translation.orgRef,translation.invoiceNo,translation.commit,translation.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM translation,interpreter_reg,comp_reg WHERE translation.intrpName=interpreter_reg.id AND translation.orgName=comp_reg.abrv $append_multi_tr $append_over_dues_tr AND translation.deleted_flag = 0 AND translation.disposed_of = 0 and translation.order_cancel_flag=0 and translation.commit=1 $po_string_tr AND translation.credit_note!='' and translation.invoic_date like '$invoic_date%' and translation.asignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and ( (translation.source="' . $job . '" OR translation.target="' . $job . '") OR (translation.source="' . $job . '" AND translation.target="English") OR (translation.source="English" AND translation.target="' . $job . '")) ' : '') . " and interpreter_reg.name like '%$interp%' $p_org_ad $append_orgName_tr and translation.invoiceNo like '%$inov%') as grp WHERE type like '%$type%' ORDER BY CONCAT(assignDate,' ',assignTime) LIMIT {$startpoint} , {$limit}";
            } else {
                if (isset($string) && !empty($string)) {
                    $query = "SELECT * from (SELECT interpreter.porder,comp_reg.po_req,'Interpreter' as type,interpreter.id,interpreter.intrpName,interpreter.orgName,interpreter.inchEmail,interpreter_reg.name,interpreter.source,interpreter.target,interpreter.invoic_date,interpreter.assignDate,interpreter.assignTime,interpreter.orgContact,interpreter.submited, interpreter.aloct_by,interpreter.aloct_date,interpreter.dated,interpreter.hrsubmited,interpreter.comp_hrsubmited,interpreter.interp_hr_date, interpreter.comp_hr_date,interpreter.hoursWorkd,interpreter.C_hoursWorkd,interpreter.total_charges_comp,interpreter.cur_vat,interpreter.C_otherexpns, interpreter.credit_note,interpreter.C_admnchargs,interpreter.rAmount,interpreter.rDate,interpreter.sentemail,interpreter.printed,interpreter.printedby,interpreter.deleted_flag,interpreter.order_cancel_flag,interpreter.nameRef,interpreter.orgRef,interpreter.invoiceNo,interpreter.commit,interpreter.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM interpreter,interpreter_reg,comp_reg WHERE interpreter.intrpName=interpreter_reg.id AND interpreter.orgName=comp_reg.abrv $p_org_ad $append_multi_int $append_over_dues_int AND interpreter.deleted_flag = 0 AND interpreter.disposed_of = 0 and interpreter.order_cancel_flag=0 and interpreter.commit=1 AND interpreter.credit_note!='' and (interpreter.orgRef like '%$string%' OR interpreter.porder like '%$string%' OR interpreter.nameRef like '%$string%' OR interpreter.invoiceNo like '%$string%' OR interpreter.id like '$string%' OR interpreter.reference_no like '$string%') 
                    UNION ALL SELECT telephone.porder,comp_reg.po_req,'Telephone' as type,telephone.id,telephone.intrpName,telephone.orgName,telephone.inchEmail,interpreter_reg.name,telephone.source,telephone.target,telephone.invoic_date,telephone.assignDate,telephone.assignTime,telephone.orgContact,telephone.submited, telephone.aloct_by,telephone.aloct_date,telephone.dated,telephone.hrsubmited,telephone.comp_hrsubmited,telephone.interp_hr_date,telephone.comp_hr_date, telephone.hoursWorkd,telephone.C_hoursWorkd,telephone.total_charges_comp,telephone.cur_vat,telephone.calCharges as C_otherexpns,telephone.credit_note,telephone.C_admnchargs, telephone.rAmount,telephone.rDate,telephone.sentemail,telephone.printed,telephone.printedby,telephone.deleted_flag,telephone.order_cancel_flag,telephone.nameRef,telephone.orgRef,telephone.invoiceNo,telephone.commit,telephone.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM telephone,interpreter_reg,comp_reg WHERE telephone.intrpName=interpreter_reg.id AND telephone.orgName=comp_reg.abrv $p_org_ad $append_multi_tp $append_over_dues_tp AND telephone.deleted_flag = 0 AND telephone.disposed_of = 0 and telephone.order_cancel_flag=0 and telephone.commit=1 AND telephone.credit_note!='' and (telephone.orgRef like '%$string%' OR telephone.porder like '%$string%' OR telephone.nameRef like '%$string%' OR telephone.invoiceNo like '%$string%' OR telephone.id like '$string%' OR telephone.reference_no like '$string%') 
                    UNION ALL SELECT translation.porder,comp_reg.po_req,'Translation' as type,translation.id,translation.intrpName,translation.orgName,translation.inchEmail,interpreter_reg.name,translation.source,translation.target,translation.invoic_date,translation.asignDate as assignDate,'00:00:00' as assignTime,translation.orgContact,translation.submited, translation.aloct_by,translation.aloct_date,translation.dated,translation.hrsubmited,translation.comp_hrsubmited,translation.interp_hr_date,translation.comp_hr_date, translation.numberUnit as hoursWorkd,translation.C_numberUnit as C_hoursWorkd,translation.total_charges_comp,translation.cur_vat,0 as C_otherexpns,translation.credit_note,translation.C_admnchargs, translation.rAmount,translation.rDate,translation.sentemail,translation.printed,translation.printedby,translation.deleted_flag,translation.order_cancel_flag,translation.nameRef,translation.orgRef,translation.invoiceNo,translation.commit,translation.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM translation,interpreter_reg,comp_reg WHERE translation.intrpName=interpreter_reg.id AND translation.orgName=comp_reg.abrv $p_org_ad $append_multi_tr $append_over_dues_tr AND translation.deleted_flag = 0 AND translation.disposed_of = 0 and translation.order_cancel_flag=0 and translation.commit=1 AND translation.credit_note!='' and (translation.orgRef like '%$string%' OR translation.porder like '%$string%' OR translation.nameRef like '%$string%' OR translation.invoiceNo like '%$string%')) as grp ORDER BY CONCAT(assignDate,' ',assignTime) LIMIT {$startpoint} , {$limit}";
                } else {
                    $query = "SELECT * from (SELECT interpreter.porder,comp_reg.po_req,'Interpreter' as type,interpreter.id,interpreter.intrpName,interpreter.orgName,interpreter.inchEmail,interpreter_reg.name,interpreter.source,interpreter.target,interpreter.invoic_date,interpreter.assignDate,interpreter.assignTime,interpreter.orgContact,interpreter.submited, interpreter.aloct_by,interpreter.aloct_date,interpreter.dated,interpreter.hrsubmited,interpreter.comp_hrsubmited,interpreter.interp_hr_date, interpreter.comp_hr_date,interpreter.hoursWorkd,interpreter.C_hoursWorkd,interpreter.total_charges_comp,interpreter.cur_vat,interpreter.C_otherexpns, interpreter.credit_note,interpreter.C_admnchargs,interpreter.rAmount,interpreter.rDate,interpreter.sentemail,interpreter.printed,interpreter.printedby,interpreter.deleted_flag,interpreter.order_cancel_flag,interpreter.nameRef,interpreter.orgRef,interpreter.invoiceNo,interpreter.commit,interpreter.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM interpreter,interpreter_reg,comp_reg WHERE interpreter.intrpName=interpreter_reg.id AND interpreter.orgName=comp_reg.abrv $p_org_ad $append_multi_int $append_over_dues_int AND interpreter.deleted_flag = 0 AND interpreter.disposed_of = 0 and interpreter.order_cancel_flag=0 and interpreter.commit=1 $po_string_int AND interpreter.credit_note!='' and interpreter.invoic_date like '$invoic_date%' and interpreter.assignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and ( (interpreter.source="' . $job . '" OR interpreter.target="' . $job . '") OR (interpreter.source="' . $job . '" AND interpreter.target="English") OR (interpreter.source="English" AND interpreter.target="' . $job . '")) ' : '') . " and interpreter_reg.name like '%$interp%' " . (!empty($org) && $org != "" ? '  and interpreter.orgName IN (' . $all_cz["all_cz"] . ') ' : '') . " and interpreter.invoiceNo   like '%$inov%' 
                        UNION ALL SELECT telephone.porder,comp_reg.po_req,'Telephone' as type,telephone.id,telephone.intrpName,telephone.orgName,telephone.inchEmail,interpreter_reg.name,telephone.source,telephone.target,telephone.invoic_date,telephone.assignDate,telephone.assignTime,telephone.orgContact,telephone.submited, telephone.aloct_by,telephone.aloct_date,telephone.dated,telephone.hrsubmited,telephone.comp_hrsubmited,telephone.interp_hr_date,telephone.comp_hr_date, telephone.hoursWorkd,telephone.C_hoursWorkd,telephone.total_charges_comp,telephone.cur_vat,telephone.calCharges as C_otherexpns,telephone.credit_note,telephone.C_admnchargs, telephone.rAmount,telephone.rDate,telephone.sentemail,telephone.printed,telephone.printedby,telephone.deleted_flag,telephone.order_cancel_flag,telephone.nameRef,telephone.orgRef,telephone.invoiceNo,telephone.commit,telephone.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM telephone,interpreter_reg,comp_reg WHERE telephone.intrpName=interpreter_reg.id AND telephone.orgName=comp_reg.abrv $p_org_ad $append_multi_tp $append_over_dues_tp AND telephone.deleted_flag = 0 AND telephone.disposed_of = 0 and telephone.order_cancel_flag=0 and telephone.commit=1 $po_string_tp AND telephone.credit_note!='' and telephone.invoic_date like '$invoic_date%' and telephone.assignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and (  (telephone.source="' . $job . '" OR telephone.target="' . $job . '") OR (telephone.source="' . $job . '" AND telephone.target="English") OR (telephone.source="English" AND telephone.target="' . $job . '") ) ' : '') . " and interpreter_reg.name like '%$interp%' " . (!empty($org) && $org != "" ? '  and telephone.orgName IN (' . $all_cz["all_cz"] . ') ' : '') . " and telephone.invoiceNo like '%$inov%' 
                        UNION ALL SELECT translation.porder,comp_reg.po_req,'Translation' as type,translation.id,translation.intrpName,translation.orgName,translation.inchEmail,interpreter_reg.name,translation.source,translation.target,translation.invoic_date,translation.asignDate as assignDate,'00:00:00' as assignTime,translation.orgContact,translation.submited, translation.aloct_by,translation.aloct_date,translation.dated,translation.hrsubmited,translation.comp_hrsubmited,translation.interp_hr_date,translation.comp_hr_date, translation.numberUnit as hoursWorkd,translation.C_numberUnit as C_hoursWorkd,translation.total_charges_comp,translation.cur_vat,0 as C_otherexpns,translation.credit_note,translation.C_admnchargs, translation.rAmount,translation.rDate,translation.sentemail,translation.printed,translation.printedby,translation.deleted_flag,translation.order_cancel_flag,translation.nameRef,translation.orgRef,translation.invoiceNo,translation.commit,translation.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM translation,interpreter_reg,comp_reg WHERE translation.intrpName=interpreter_reg.id AND translation.orgName=comp_reg.abrv $p_org_ad $append_multi_tr $append_over_dues_tr AND translation.deleted_flag = 0 AND translation.disposed_of = 0 and translation.order_cancel_flag=0 and translation.commit=1 $po_string_tr AND translation.credit_note!='' and translation.invoic_date like '$invoic_date%' and translation.asignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and ( (translation.source="' . $job . '" OR translation.target="' . $job . '") OR (translation.source="' . $job . '" AND translation.target="English") OR (translation.source="English" AND translation.target="' . $job . '") ) ' : '') . " and interpreter_reg.name like '%$interp%' " . (!empty($org) && $org != "" ? '  and translation.orgName IN (' . $all_cz["all_cz"] . ') ' : '') . " and translation.invoiceNo like '%$inov%') as grp 
                ...
                <table class="tablesorter table table-bordered tbl_data" cellspacing="0" cellpadding="0">
                        ...
                        <tr>
                            <td align="center">
                                <?php if ($action_export_to_excel) {
                                    $export_link = isset($string) && !empty($string) ? 'reports_lsuk/excel/' . basename(__FILE__) . '?str=' . $string : 'reports_lsuk/excel/' . basename(__FILE__) . '?interp=' . $interp . '&p_org=' . $p_org . '&org=' . $org . '&job=' . $job . '&assignDate=' . $assignDate . '&inov=' . $inov . '&type=' . $type . '&po=' . $po . '&invoic_date=' . $invoic_date . '&multi=' . $multi . '&over_dues=' . $over_dues . '&over_due_days=' . $over_due_days; ?>
                    ...
                    <tbody>
                        ...
                        if (mysqli_num_rows($result) == 0) {
                            echo '<tr><td><h4 class="text-danger text-center">Sorry ! There are no records.</h4></td></tr>';
                        } else {
                                ...
                                <tr>
                                    <td><?php echo '<span class="w3-badge w3-blue badge-counter">' . $page_count . '</span>'; ?>
                                        ...
                                        <div class="col-sm-4 col-xs-12">
                                            <ul class="w3-ul">
                                                <?php 
                                                $order_type = array('Interpreter' => 'int','Telephone' => 'tp','Translation' => 'tr');
                                                $row_part=$acttObj->read_specific('sum(amount) as amount','partial_amounts',' status="1" and tbl="'.$order_type[$row['type']].'" and order_id="'.$row['id'].'"')['amount']?:0; ?>
                                                ...
                                                <li>Remaining Amount<span class="w3-large w3-right"><?php echo ($row_part>0?(round($totinvnow,2)-round($row['rAmount'],2)):$misc->numberFormat_fun($totinvnow)); ?></span></li>
                                        ...
                                        <div class="col-sm-12 text-center action_buttons">
                                            <?php if ($_SESSION['userId'] == 27) { ?>
                                                <button type="button" class="btn btn-primary" onclick="show_disposed(<?php echo $row['id']; ?> , '<?php echo $row['type'] ?>')">
                                                    Disposed Of
                                                </button>
                                            <?php } ?>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="save_disposed" onClick="save_disposed_of()" class="btn btn-primary">Save changes</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function show_disposed(id, type) {
            $("#disposed_id").val(id);
            $("#disposed_type").val(type);
            $('#disposed_modal').modal('show');
        }

        function save_disposed_of() {
            let id = $("#disposed_id").val();
            let type = $("#disposed_type").val();
            let disposed_reason = $('input[name="disposed_reason"]:checked').val();
            $("#save_disposed").prop('disabled', true);
            $.ajax({
                type: "POST",
                url: "all_pending_payment.php",
                data: {
                    disposed_submit: 1,
                    id,
                    type,
