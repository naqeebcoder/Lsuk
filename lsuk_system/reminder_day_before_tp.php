...
$query_reminder="SELECT concat('Telephone Assignment Reminder on ',telephone.assignDate,' at ',telephone.assignTime) as title,telephone.id as job_id,telephone.intrpName as interpreter_id,telephone.assignDate,telephone.assignTime,telephone.assignDur,'',interpreter_reg.name as interpreter_name,interpreter_reg.email as interpreter_email,'day_before' as type,'".$dated."' as inserted_date,'telephone' as table_type FROM telephone,interpreter_reg WHERE telephone.intrpName=interpreter_reg.id and telephone.deleted_flag=0 and telephone.orderCancelatoin=0 and telephone.order_cancel_flag=0 and telephone.hoursWorkd=0 and telephone.intrp_salary_comit=0 and telephone.assignDate = '".$tomorrow."'";
$result_reminder=mysqli_query($con,$query_reminder);
$em_format=$obj_new->read_specific("em_format","email_format","id=37")['em_format'];
...
$data_title=" at ".$row_reminder['assignTime']." on ".$row_reminder['assignDate']." which has a duration of ".$get_dur;
