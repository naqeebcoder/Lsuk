...
$query_reminder = $obj->read_all("telephone.porder_email,telephone.porder,telephone.id,telephone.orgName,telephone.inchEmail,telephone.inchPerson,telephone.source,telephone.target,telephone.invoic_date,telephone.assignDate,SUBSTRING(telephone.assignTime,1,5) as assignTime,telephone.total_charges_comp,telephone.cur_vat, telephone.credit_note,telephone.C_admnchargs,telephone.nameRef,telephone.orgRef,telephone.invoiceNo", "(SELECT po_requested.order_id,MAX(dated) as latest_date FROM po_requested WHERE order_type='tp' GROUP BY order_id,order_type) as grp,telephone,comp_reg", "telephone.orgName=comp_reg.abrv and telephone.multInv_flag=0 AND telephone.deleted_flag = 0 and telephone.order_cancel_flag=0 and telephone.commit=1 AND telephone.total_charges_comp >0 and comp_reg.po_req=1 and (telephone.porder='' OR telephone.porder='Nil') and (round(telephone.rAmount,2) < round((telephone.total_charges_comp+(telephone.total_charges_comp*telephone.cur_vat)),2) or telephone.total_charges_comp =0) and YEAR(telephone.assignDate)>'2018' and grp.order_id=telephone.id AND DATEDIFF(now(),grp.latest_date)>=7 LIMIT 30");
