...
function myFunction() {
	 ...
	 window.location.href="telep_work_list.php" + '?name=' + x + '&from=' + y + '&to=' + z;
}
</script>
...
<!-- end of sidebar -->
	...
	<section id="main" class="column"><!-- end of stats article -->
		...
		<header><h3 class="tabs_involved" style="width:230px;">Salary List(Tel.)</h3>
	 ...
	 JOIN telephone ON $table.id=telephone.intrpName
	   where (telephone.assignDate between '$from' and '$to') and telephone.commit=1 and telephone.intrp_salary_comit=0 or ($table.name='$name')
	   LIMIT {$startpoint} , {$limit}";	}
	   else{$query="SELECT $table.* FROM $table 
		JOIN telephone ON $table.id=telephone.intrpName
	   	where telephone.commit=1 and telephone.intrp_salary_comit=0  LIMIT {$startpoint} , {$limit}";	}
