...
<head>
    <title>PENDING INVOICES LIST</title>
            ...
            <header>
                <center>
                    <div style="margin-top: 15px;" class="form-group col-md-2 col-sm-4">
                        ...
                        <h2 class="col-md-3 text-center"><span class="label label-primary">PENDING INVOICES LIST</span></h2>
                    </a>
...
AND interpreter.intrpName=interpreter_reg.id AND interpreter.orgName=comp_reg.abrv AND interpreter.deleted_flag = 0 AND interpreter.disposed_of = 0 and interpreter.order_cancel_flag=0 $append_multi_int $append_over_dues_int and interpreter.commit=1 $po_string_int and (round(interpreter.rAmount,2) < round((interpreter.total_charges_comp+(interpreter.total_charges_comp*interpreter.cur_vat)),2) or interpreter.total_charges_comp =0) and interpreter.assignDate like '$assignDate%' and interpreter.invoic_date like '$invoic_date%' and (interpreter.orgName like '%$_words%')) as grp  ORDER BY name ASC";
...
AND telephone.intrpName=interpreter_reg.id AND telephone.orgName=comp_reg.abrv AND telephone.deleted_flag = 0 AND telephone.disposed_of = 0 and telephone.order_cancel_flag=0 $append_multi_tp $append_over_dues_tp and telephone.commit=1 $po_string_tp and (round(telephone.rAmount,2) < round((telephone.total_charges_comp+(telephone.total_charges_comp*telephone.cur_vat)),2) or telephone.total_charges_comp =0) and telephone.assignDate like '$assignDate%' and telephone.invoic_date like '$invoic_date%' and (telephone.orgName like '%$_words%')) as grp  ORDER BY name ASC";
...
AND translation.intrpName=interpreter_reg.id AND translation.orgName=comp_reg.abrv AND translation.deleted_flag = 0 AND translation.disposed_of = 0 and translation.order_cancel_flag=0 $append_multi_tr $append_over_dues_tr and translation.commit=1 $po_string_tr and (round(translation.rAmount,2) < round((translation.total_charges_comp+(translation.total_charges_comp*translation.cur_vat)),2) or translation.total_charges_comp =0) and translation.asignDate like '$assignDate%' and translation.invoic_date like '$invoic_date%' and (translation.orgName like '%$_words%')) as grp  ORDER BY name ASC";
                        } else {
                            $sql_opt = "SELECT DISTINCT name,id,gender,city from (SELECT distinct interpreter_reg.name,interpreter_reg.id,interpreter_reg.gender, interpreter_reg.city,interpreter.multInv_flag,interpreter.commit,interpreter.total_charges_comp,interpreter.rAmount,interpreter.orgName,interpreter.deleted_flag,interpreter.order_cancel_flag,
interpreter.porder,comp_reg.po_req FROM interpreter_reg,interp_lang,interpreter,comp_reg WHERE interpreter_reg.code=interp_lang.code
AND interpreter.intrpName=interpreter_reg.id AND interpreter.orgName=comp_reg.abrv AND interpreter.deleted_flag = 0 AND interpreter.disposed_of = 0 and interpreter.order_cancel_flag=0 $append_multi_int $append_over_dues_int and interpreter.commit=1 $po_string_int and (round(interpreter.rAmount,2) < round((interpreter.total_charges_comp+(interpreter.total_charges_comp*interpreter.cur_vat))+interpreter.C_otherexpns,2) or interpreter.total_charges_comp =0) and interpreter.assignDate like '$assignDate%' and interpreter.invoic_date like '$invoic_date%' and (interpreter.orgName like '%$_words%')
...
AND translation.intrpName=interpreter_reg.id AND translation.orgName=comp_reg.abrv AND translation.deleted_flag = 0 AND translation.disposed_of = 0 and translation.order_cancel_flag=0 $append_multi_tr $append_over_dues_tr and translation.commit=1 $po_string_tr and (round(translation.rAmount,2) < round((translation.total_charges_comp+(translation.total_charges_comp*translation.cur_vat)),2) or translation.total_charges_comp =0) and translation.asignDate like '$assignDate%' and translation.invoic_date like '$invoic_date%' and (translation.orgName like '%$_words%')) as grp  ORDER BY name ASC";
                    ...
                    <div class="form-group col-md-2 col-sm-4">
                        <?php
                        if (!empty($type) && $type == 'Interpreter') {
                            $sql_opt = "SELECT DISTINCT id,name,abrv from (SELECT DISTINCT comp_reg.id,comp_reg.name,comp_reg.abrv,comp_reg.po_req,interpreter.porder,interpreter.multInv_flag,interpreter.commit,interpreter.total_charges_comp,interpreter.rAmount,interpreter.orgName,interpreter.deleted_flag,interpreter.order_cancel_flag FROM comp_reg,interpreter WHERE interpreter.orgName=comp_reg.abrv AND interpreter.deleted_flag=0 AND interpreter.order_cancel_flag=0 $append_multi_int $append_over_dues_int and interpreter.commit=1 and interpreter.assignDate like '$assignDate%' and interpreter.invoic_date like '$invoic_date%' $po_string_int  and (round(interpreter.rAmount,2) < round((interpreter.total_charges_comp+(interpreter.total_charges_comp*interpreter.cur_vat)),2) or interpreter.total_charges_comp =0)) as grp 
                                ORDER BY name ASC";
                        } else if (!empty($type) && $type == 'Telephone') {
                            $sql_opt = "SELECT DISTINCT id,name,abrv from (SELECT DISTINCT comp_reg.id,comp_reg.name,comp_reg.abrv,comp_reg.po_req,telephone.porder,telephone.multInv_flag,telephone.commit,telephone.total_charges_comp,telephone.rAmount,telephone.orgName,telephone.deleted_flag,telephone.order_cancel_flag FROM comp_reg,telephone WHERE telephone.orgName=comp_reg.abrv AND telephone.deleted_flag=0 AND telephone.order_cancel_flag=0 $append_multi_tp $append_over_dues_tp and telephone.commit=1 and telephone.assignDate like '$assignDate%' and telephone.invoic_date like '$invoic_date%' $po_string_tp  and (round(telephone.rAmount,2) < round((telephone.total_charges_comp+(telephone.total_charges_comp*telephone.cur_vat)),2) or telephone.total_charges_comp =0)) as grp 
                                ORDER BY name ASC";
                        } else if (!empty($type) && $type == 'Translation') {
                            $sql_opt = "SELECT DISTINCT id,name,abrv from (SELECT DISTINCT comp_reg.id,comp_reg.name,comp_reg.abrv,comp_reg.po_req,translation.porder,translation.multInv_flag,translation.commit,translation.total_charges_comp,translation.rAmount,translation.orgName,translation.deleted_flag,translation.order_cancel_flag FROM comp_reg,translation WHERE translation.orgName=comp_reg.abrv AND translation.deleted_flag=0 AND translation.order_cancel_flag=0 $append_multi_tr $append_over_dues_tr and translation.commit=1 and translation.asignDate like '$assignDate%' and translation.invoic_date like '$invoic_date%' $po_string_tr  and (round(translation.rAmount,2) < round((translation.total_charges_comp+(translation.total_charges_comp*translation.cur_vat)),2) or translation.total_charges_comp =0)) as grp 
                                ORDER BY name ASC";
                        } else {
            ...
            if (!empty($type) && $type == 'Interpreter') {
                $query =
                    "SELECT * from (SELECT interpreter.porder,comp_reg.po_req,'Interpreter' as type,interpreter.id,interpreter.intrpName,interpreter.orgName,interpreter.inchEmail,interpreter_reg.name,interpreter.source,interpreter.target,interpreter.invoic_date,interpreter.assignDate,interpreter.assignTime,interpreter.orgContact,interpreter.submited, interpreter.aloct_by,interpreter.aloct_date,interpreter.dated,interpreter.hrsubmited,interpreter.comp_hrsubmited,interpreter.interp_hr_date, interpreter.comp_hr_date,interpreter.hoursWorkd,interpreter.C_hoursWorkd,interpreter.total_charges_comp,interpreter.cur_vat,interpreter.C_otherexpns, interpreter.credit_note,interpreter.C_admnchargs,interpreter.rAmount,interpreter.rDate,interpreter.sentemail,interpreter.printed,interpreter.printedby,interpreter.deleted_flag,interpreter.order_cancel_flag,interpreter.nameRef,interpreter.orgRef,interpreter.invoiceNo,interpreter.commit,interpreter.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM interpreter,interpreter_reg,comp_reg WHERE interpreter.intrpName=interpreter_reg.id AND interpreter.orgName=comp_reg.abrv $append_multi_int $append_over_dues_int AND interpreter.deleted_flag = 0 AND interpreter.disposed_of = 0 and interpreter.order_cancel_flag=0 and interpreter.commit=1 $po_string_int and (round(interpreter.rAmount,2) < round((interpreter.total_charges_comp+(interpreter.total_charges_comp*interpreter.cur_vat))+interpreter.c_otherexpns,2) or interpreter.total_charges_comp =0) and interpreter.invoic_date like '$invoic_date%' and interpreter.assignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and ( (interpreter.source="' . $job . '" OR interpreter.target="' . $job . '") OR (interpreter.source="' . $job . '" AND interpreter.target="English") OR (interpreter.source="English" AND interpreter.target="' . $job . '")) ' : '') . " and interpreter_reg.name like '%$interp%' $p_org_ad $append_orgName_int and interpreter.invoiceNo like '%$inov%') as grp WHERE type like '%$type%' ORDER BY CONCAT(assignDate,' ',assignTime) LIMIT {$startpoint} , {$limit}";
            } else if (!empty($type) && $type == 'Telephone') {
                $query =
                    "SELECT * from (SELECT telephone.porder,comp_reg.po_req,'Telephone' as type,telephone.id,telephone.intrpName,telephone.orgName,telephone.inchEmail,interpreter_reg.name,telephone.source,telephone.target,telephone.invoic_date,telephone.assignDate,telephone.assignTime,telephone.orgContact,telephone.submited, telephone.aloct_by,telephone.aloct_date,telephone.dated,telephone.hrsubmited,telephone.comp_hrsubmited,telephone.interp_hr_date,telephone.comp_hr_date, telephone.hoursWorkd,telephone.C_hoursWorkd,telephone.total_charges_comp,telephone.cur_vat,telephone.calCharges as C_otherexpns,telephone.credit_note,telephone.C_admnchargs, telephone.rAmount,telephone.rDate,telephone.sentemail,telephone.printed,telephone.printedby,telephone.deleted_flag,telephone.order_cancel_flag,telephone.nameRef,telephone.orgRef,telephone.invoiceNo,telephone.commit,telephone.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM telephone,interpreter_reg,comp_reg WHERE telephone.intrpName=interpreter_reg.id AND telephone.orgName=comp_reg.abrv $append_multi_tp $append_over_dues_tp AND telephone.deleted_flag = 0 AND telephone.disposed_of = 0 and telephone.order_cancel_flag=0 and telephone.commit=1 $po_string_tp and (round(telephone.rAmount,2) < round((telephone.total_charges_comp+(telephone.total_charges_comp*telephone.cur_vat)),2) or telephone.total_charges_comp =0) and telephone.invoic_date like '$invoic_date%' and telephone.assignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and ( (telephone.source="' . $job . '" OR telephone.target="' . $job . '") OR (telephone.source="' . $job . '" AND telephone.target="English") OR (telephone.source="English" AND telephone.target="' . $job . '")) ' : '') . " and interpreter_reg.name like '%$interp%' $p_org_ad $append_orgName_tp and telephone.invoiceNo like '%$inov%') as grp WHERE type like '%$type%' ORDER BY CONCAT(assignDate,' ',assignTime) LIMIT {$startpoint} , {$limit}";
            } else if (!empty($type) && $type == 'Translation') {
                $query =
                    "SELECT * from (SELECT translation.porder,comp_reg.po_req,'Translation' as type,translation.id,translation.intrpName,translation.orgName,translation.inchEmail,interpreter_reg.name,translation.source,translation.target,translation.invoic_date,translation.asignDate as assignDate,'00:00:00' as assignTime,translation.orgContact,translation.submited, translation.aloct_by,translation.aloct_date,translation.dated,translation.hrsubmited,translation.comp_hrsubmited,translation.interp_hr_date,translation.comp_hr_date, translation.numberUnit as hoursWorkd,translation.C_numberUnit as C_hoursWorkd,translation.total_charges_comp,translation.cur_vat,0 as C_otherexpns,translation.credit_note,translation.C_admnchargs, translation.rAmount,translation.rDate,translation.sentemail,translation.printed,translation.printedby,translation.deleted_flag,translation.order_cancel_flag,translation.nameRef,translation.orgRef,translation.invoiceNo,translation.commit,translation.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM translation,interpreter_reg,comp_reg WHERE translation.intrpName=interpreter_reg.id AND translation.orgName=comp_reg.abrv $append_multi_tr $append_over_dues_tr AND translation.deleted_flag = 0 AND translation.disposed_of = 0 and translation.order_cancel_flag=0 and translation.commit=1 $po_string_tr and (round(translation.rAmount,2) < round((translation.total_charges_comp+(translation.total_charges_comp*translation.cur_vat)),2) or translation.total_charges_comp =0) and translation.invoic_date like '$invoic_date%' and translation.asignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and ( (translation.source="' . $job . '" OR translation.target="' . $job . '") OR (translation.source="' . $job . '" AND translation.target="English") OR (translation.source="English" AND translation.target="' . $job . '")) ' : '') . " and interpreter_reg.name like '%$interp%' $p_org_ad $append_orgName_tr and translation.invoiceNo like '%$inov%') as grp WHERE type like '%$type%' ORDER BY CONCAT(assignDate,' ',assignTime) LIMIT {$startpoint} , {$limit}";
            } else {
                if (isset($string) && !empty($string)) {
                    $query = "SELECT * from (SELECT interpreter.porder,comp_reg.po_req,'Interpreter' as type,interpreter.id,interpreter.intrpName,interpreter.orgName,interpreter.inchEmail,interpreter_reg.name,interpreter.source,interpreter.target,interpreter.invoic_date,interpreter.assignDate,interpreter.assignTime,interpreter.orgContact,interpreter.submited, interpreter.aloct_by,interpreter.aloct_date,interpreter.dated,interpreter.hrsubmited,interpreter.comp_hrsubmited,interpreter.interp_hr_date, interpreter.comp_hr_date,interpreter.hoursWorkd,interpreter.C_hoursWorkd,interpreter.total_charges_comp,interpreter.cur_vat,interpreter.C_otherexpns, interpreter.credit_note,interpreter.C_admnchargs,interpreter.rAmount,interpreter.rDate,interpreter.sentemail,interpreter.printed,interpreter.printedby,interpreter.deleted_flag,interpreter.order_cancel_flag,interpreter.nameRef,interpreter.orgRef,interpreter.invoiceNo,interpreter.commit,interpreter.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM interpreter,interpreter_reg,comp_reg WHERE interpreter.intrpName=interpreter_reg.id AND interpreter.orgName=comp_reg.abrv $p_org_ad $append_multi_int $append_over_dues_int AND interpreter.deleted_flag = 0 AND interpreter.disposed_of = 0 and interpreter.order_cancel_flag=0 and interpreter.commit=1 and (round(interpreter.rAmount,2) < round((interpreter.total_charges_comp+(interpreter.total_charges_comp*interpreter.cur_vat))+interpreter.c_otherexpns,2) or interpreter.total_charges_comp =0) and (interpreter.orgRef like '%$string%' OR interpreter.porder like '%$string%' OR interpreter.nameRef like '%$string%' OR interpreter.invoiceNo like '%$string%' OR interpreter.id like '$string%' OR interpreter.reference_no like '$string%') 
                    UNION ALL SELECT telephone.porder,comp_reg.po_req,'Telephone' as type,telephone.id,telephone.intrpName,telephone.orgName,telephone.inchEmail,interpreter_reg.name,telephone.source,telephone.target,telephone.invoic_date,telephone.assignDate,telephone.assignTime,telephone.orgContact,telephone.submited, telephone.aloct_by,telephone.aloct_date,telephone.dated,telephone.hrsubmited,telephone.comp_hrsubmited,telephone.interp_hr_date,telephone.comp_hr_date, telephone.hoursWorkd,telephone.C_hoursWorkd,telephone.total_charges_comp,telephone.cur_vat,telephone.calCharges as C_otherexpns,telephone.credit_note,telephone.C_admnchargs, telephone.rAmount,telephone.rDate,telephone.sentemail,telephone.printed,telephone.printedby,telephone.deleted_flag,telephone.order_cancel_flag,telephone.nameRef,telephone.orgRef,telephone.invoiceNo,telephone.commit,telephone.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM telephone,interpreter_reg,comp_reg WHERE telephone.intrpName=interpreter_reg.id AND telephone.orgName=comp_reg.abrv $p_org_ad $append_multi_tp $append_over_dues_tp AND telephone.deleted_flag = 0 AND telephone.disposed_of = 0 and telephone.order_cancel_flag=0 and telephone.commit=1 and (round(telephone.rAmount,2) < round((telephone.total_charges_comp+(telephone.total_charges_comp*telephone.cur_vat)),2) or telephone.total_charges_comp =0) and (telephone.orgRef like '%$string%' OR telephone.porder like '%$string%' OR telephone.nameRef like '%$string%' OR telephone.invoiceNo like '%$string%' OR telephone.id like '$string%' OR telephone.reference_no like '$string%') 
                    UNION ALL SELECT translation.porder,comp_reg.po_req,'Translation' as type,translation.id,translation.intrpName,translation.orgName,translation.inchEmail,interpreter_reg.name,translation.source,translation.target,translation.invoic_date,translation.asignDate as assignDate,'00:00:00' as assignTime,translation.orgContact,translation.submited, translation.aloct_by,translation.aloct_date,translation.dated,translation.hrsubmited,translation.comp_hrsubmited,translation.interp_hr_date,translation.comp_hr_date, translation.numberUnit as hoursWorkd,translation.C_numberUnit as C_hoursWorkd,translation.total_charges_comp,translation.cur_vat,0 as C_otherexpns,translation.credit_note,translation.C_admnchargs, translation.rAmount,translation.rDate,translation.sentemail,translation.printed,translation.printedby,translation.deleted_flag,translation.order_cancel_flag,translation.nameRef,translation.orgRef,translation.invoiceNo,translation.commit,translation.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM translation,interpreter_reg,comp_reg WHERE translation.intrpName=interpreter_reg.id AND translation.orgName=comp_reg.abrv $p_org_ad $append_multi_tr $append_over_dues_tr AND translation.deleted_flag = 0 AND translation.disposed_of = 0 and translation.order_cancel_flag=0 and translation.commit=1 and (round(translation.rAmount,2) < round((translation.total_charges_comp+(translation.total_charges_comp*translation.cur_vat)),2) or translation.total_charges_comp =0) and (translation.orgRef like '%$string%' OR translation.porder like '%$string%' OR translation.nameRef like '%$string%' OR translation.invoiceNo like '%$string%')) as grp ORDER BY CONCAT(assignDate,' ',assignTime) LIMIT {$startpoint} , {$limit}";
                } else {
                    $query = "SELECT * from (SELECT interpreter.porder,comp_reg.po_req,'Interpreter' as type,interpreter.id,interpreter.intrpName,interpreter.orgName,interpreter.inchEmail,interpreter_reg.name,interpreter.source,interpreter.target,interpreter.invoic_date,interpreter.assignDate,interpreter.assignTime,interpreter.orgContact,interpreter.submited, interpreter.aloct_by,interpreter.aloct_date,interpreter.dated,interpreter.hrsubmited,interpreter.comp_hrsubmited,interpreter.interp_hr_date, interpreter.comp_hr_date,interpreter.hoursWorkd,interpreter.C_hoursWorkd,interpreter.total_charges_comp,interpreter.cur_vat,interpreter.C_otherexpns, interpreter.credit_note,interpreter.C_admnchargs,interpreter.rAmount,interpreter.rDate,interpreter.sentemail,interpreter.printed,interpreter.printedby,interpreter.deleted_flag,interpreter.order_cancel_flag,interpreter.nameRef,interpreter.orgRef,interpreter.invoiceNo,interpreter.commit,interpreter.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM interpreter,interpreter_reg,comp_reg WHERE interpreter.intrpName=interpreter_reg.id AND interpreter.orgName=comp_reg.abrv $p_org_ad $append_multi_int $append_over_dues_int AND interpreter.deleted_flag = 0 AND interpreter.disposed_of = 0 and interpreter.order_cancel_flag=0 and interpreter.commit=1 $po_string_int and (round(interpreter.rAmount,2) < round((interpreter.total_charges_comp+(interpreter.total_charges_comp*interpreter.cur_vat))+interpreter.C_otherexpns,2) or interpreter.total_charges_comp =0) and interpreter.invoic_date like '$invoic_date%' and interpreter.assignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and ( (interpreter.source="' . $job . '" OR interpreter.target="' . $job . '") OR (interpreter.source="' . $job . '" AND interpreter.target="English") OR (interpreter.source="English" AND interpreter.target="' . $job . '")) ' : '') . " and interpreter_reg.name like '%$interp%' " . (!empty($org) && $org != "" ? '  and interpreter.orgName IN (' . $all_cz["all_cz"] . ') ' : '') . " and interpreter.invoiceNo   like '%$inov%' 
                        UNION ALL SELECT telephone.porder,comp_reg.po_req,'Telephone' as type,telephone.id,telephone.intrpName,telephone.orgName,telephone.inchEmail,interpreter_reg.name,telephone.source,telephone.target,telephone.invoic_date,telephone.assignDate,telephone.assignTime,telephone.orgContact,telephone.submited, telephone.aloct_by,telephone.aloct_date,telephone.dated,telephone.hrsubmited,telephone.comp_hrsubmited,telephone.interp_hr_date,telephone.comp_hr_date, telephone.hoursWorkd,telephone.C_hoursWorkd,telephone.total_charges_comp,telephone.cur_vat,telephone.calCharges as C_otherexpns,telephone.credit_note,telephone.C_admnchargs, telephone.rAmount,telephone.rDate,telephone.sentemail,telephone.printed,telephone.printedby,telephone.deleted_flag,telephone.order_cancel_flag,telephone.nameRef,telephone.orgRef,telephone.invoiceNo,telephone.commit,telephone.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM telephone,interpreter_reg,comp_reg WHERE telephone.intrpName=interpreter_reg.id AND telephone.orgName=comp_reg.abrv $p_org_ad $append_multi_tp $append_over_dues_tp AND telephone.deleted_flag = 0 AND telephone.disposed_of = 0 and telephone.order_cancel_flag=0 and telephone.commit=1 $po_string_tp and (round(telephone.rAmount,2) < round((telephone.total_charges_comp+(telephone.total_charges_comp*telephone.cur_vat)),2) or telephone.total_charges_comp =0) and telephone.invoic_date like '$invoic_date%' and telephone.assignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and (  (telephone.source="' . $job . '" OR telephone.target="' . $job . '") OR (telephone.source="' . $job . '" AND telephone.target="English") OR (telephone.source="English" AND telephone.target="' . $job . '") ) ' : '') . " and interpreter_reg.name like '%$interp%' " . (!empty($org) && $org != "" ? '  and telephone.orgName IN (' . $all_cz["all_cz"] . ') ' : '') . " and telephone.invoiceNo like '%$inov%' 
                        UNION ALL SELECT translation.porder,comp_reg.po_req,'Translation' as type,translation.id,translation.intrpName,translation.orgName,translation.inchEmail,interpreter_reg.name,translation.source,translation.target,translation.invoic_date,translation.asignDate as assignDate,'00:00:00' as assignTime,translation.orgContact,translation.submited, translation.aloct_by,translation.aloct_date,translation.dated,translation.hrsubmited,translation.comp_hrsubmited,translation.interp_hr_date,translation.comp_hr_date, translation.numberUnit as hoursWorkd,translation.C_numberUnit as C_hoursWorkd,translation.total_charges_comp,translation.cur_vat,0 as C_otherexpns,translation.credit_note,translation.C_admnchargs, translation.rAmount,translation.rDate,translation.sentemail,translation.printed,translation.printedby,translation.deleted_flag,translation.order_cancel_flag,translation.nameRef,translation.orgRef,translation.invoiceNo,translation.commit,translation.new_comp_id,comp_reg.email,comp_reg.abrv as comp_abrv FROM translation,interpreter_reg,comp_reg WHERE translation.intrpName=interpreter_reg.id AND translation.orgName=comp_reg.abrv $p_org_ad $append_multi_tr $append_over_dues_tr AND translation.deleted_flag = 0 AND translation.disposed_of = 0 and translation.order_cancel_flag=0 and translation.commit=1 $po_string_tr and (round(translation.rAmount,2) < round((translation.total_charges_comp+(translation.total_charges_comp*translation.cur_vat)),2) or translation.total_charges_comp =0) and translation.invoic_date like '$invoic_date%' and translation.asignDate like '$assignDate%' " . (!empty($job) && $job != "" ? ' and ( (translation.source="' . $job . '" OR translation.target="' . $job . '") OR (translation.source="' . $job . '" AND translation.target="English") OR (translation.source="English" AND translation.target="' . $job . '") ) ' : '') . " and interpreter_reg.name like '%$interp%' " . (!empty($org) && $org != "" ? '  and translation.orgName IN (' . $all_cz["all_cz"] . ') ' : '') . " and translation.invoiceNo like '%$inov%') as grp 
