...
$action_view_job = $_SESSION['is_root'] == 1 || in_array(1, $get_actions);
$action_edit_job = $_SESSION['is_root'] == 1 || in_array(2, $get_actions);
$action_delete_job = $_SESSION['is_root'] == 1 || in_array(3, $get_actions);
$action_restore_job = $_SESSION['is_root'] == 1 || in_array(4, $get_actions);
$action_duplicate = $_SESSION['is_root'] == 1 || in_array(5, $get_actions);
$action_resume_job = $_SESSION['is_root'] == 1 || in_array(6, $get_actions);
$action_assign_interpreter = $_SESSION['is_root'] == 1 || in_array(7, $get_actions);
$action_view_applicants = $_SESSION['is_root'] == 1 || in_array(8, $get_actions);
$action_confirm_enquiry = $_SESSION['is_root'] == 1 || in_array(9, $get_actions);
$action_confirm_temporary = $_SESSION['is_root'] == 1 || in_array(10, $get_actions);
$action_reallocate_operator = $_SESSION['is_root'] == 1 || in_array(11, $get_actions);
$action_check_job = $_SESSION['is_root'] == 1 || in_array(12, $get_actions);
$action_job_note = $_SESSION['is_root'] == 1 || in_array(13, $get_actions);
$action_purchase_order = $_SESSION['is_root'] == 1 || in_array(14, $get_actions);
$action_force_assign_interpreter = $_SESSION['is_root'] == 1 || in_array(143, $get_actions);
$action_can_view_all_jobs = $_SESSION['is_root'] == 1 || in_array(144, $get_actions);
$action_dropdown_trashed_jobs_filter = $_SESSION['is_root'] == 1 || in_array(149, $get_actions);
$action_dropdown_cancelled_jobs_filter = $_SESSION['is_root'] == 1 || in_array(150, $get_actions);
$action_dropdown_enquiry_jobs_filter = $_SESSION['is_root'] == 1 || in_array(226, $get_actions);
$action_edited_history = $_SESSION['is_root'] == 1 || in_array(155, $get_actions);
$action_shift_order = $_SESSION['is_root'] == 1 || in_array(188, $get_actions);
//Access actions
$tp = @$_GET['tp'];
$table = 'interpreter';
                ...
                <table class="tablesorter table table-bordered tbl_data" cellspacing="0" cellpadding="0">
                    ...
                    <tbody>
                        ...
                        if (isset($_GET['tp']) && $_GET['tp'] == 'en'){
                            ...
                            $result = $acttObj->read_all("$table.*", "$table", "(intrpName='' OR intrpName IS NULL) " . $deleted_flag . $order_cancel_flag . " ORDER BY $table.dated DESC LIMIT 100");
                        }
                         else  {
                            $result = $acttObj->read_all("$table.*", "$table", "(intrpName='' OR intrpName IS NULL) " . $deleted_flag . $order_cancel_flag . " ORDER BY $table.assignDate ASC LIMIT 100");
                        }
                        $rowcount = 0;
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            $dbs_checked = GetMapped($row, 'dbs_checked', "");
                            ...
                            if (isset($row['bookedVia']) && $row['bookedVia'] == 'Online Portal') {
                                echo '<tr style="color:#F00;'.($get_mileage_enquiry_status==0?"background-color:#6d92b7;":"").'" ' . $urgent . '>';
                            } else {
                                echo '<tr '.($get_mileage_enquiry_status==0?"style='background-color:#6d92b7;'":"").' ' . $urgent . '>';
                            }
                            ...
                            <td <?php if ($row['is_temp'] == 1) { ?>title="This job is booked by Temporary Role. Kindly confirm to process." style="background-color:#cbda78;" <?php } ?>>
                                <div class="col-lg-4 col-md-4 col-sm-4 ">
                                    <ul class="w3-ul">
                                        <li>Language <?php echo '<span class="label label-default w3-medium w3-right">' . $row['source'] . ' <i class="fa fa-refresh"></i> ' . $row['target'] . '</span>'; ?></li>
...
$assignDuration = $parts ? implode(' ', $parts) : '0 minutes';
                                ...
                                <div class="col-lg-4 col-md-4 col-sm-4 ">
                                    <ul class="w3-ul">
                                        <li><?php $put_class = strlen($row['orgName']) > 19 ? 'w3-small' : 'w3-medium';
                                            echo '<span class="label ' . $put_class . ' w3-blue">' . $row['orgName'] . '</span>'; ?><span class="w3-medium w3-right" style="font-weight:bold"><?php echo date('d-m-Y', strtotime($row['assignDate'])) . ' ' . $row['assignTime']; ?></span></li>
                                        <li>Contact Number <span class="w3-right"><?php echo $row['inchContact']; ?></span></li>
                                ...
                                <div class="col-sm-3 col-xs-12">
                                    <ul class="w3-ul">
                                        <li>DBS Required <span class="w3-large w3-right"><?php echo $row['dbs_checked '] == 0 ? '<span class="label label-success"> Yes <i class="fa fa-check-circle"></i></span>' : '<span class="label label-danger"> No <i class="fa fa-remove"></i></span>'; ?></span></li>
                                        <li>Gender <span class="w3-right"><?php echo $row['gender']; ?></span></li>
                                        <li>Applicants <span class="w3-large w3-right">
                                                <?php
                                                if ($row['jobDisp'] == 1) {
                                ...
                                <div class="col-sm-12 text-center action_buttons">
                                    <?php if ($row['approve_portal_mngt']>0){ if ($action_view_job) { ?>
                                        <a data-toggle="tooltip" data-placement="top" href="javascript:void(0)" class="w3-button w3-small w3-circle w3-white w3-border w3-border-blue" title="View Job" onClick="popupwindow('order_view.php?view_id=<?php echo $row['id']; ?>&table=<?php echo $table; ?>', 'View Order', 1000, 1000);"><i class="fa fa-eye"></i></a>
                                    <?php }
                                    if ($row['deleted_flag'] == 0 && $row['order_cancel_flag'] == 0) {
                                        if ($action_confirm_temporary) {
                                            if ($row['is_temp'] == 1) { ?>
                                                <a data-toggle="tooltip" data-placement="top" href="javascript:void(0)" class="w3-button w3-small w3-circle w3-yellow w3-border w3-border-blue" title="This job must be confirmed by Manager or Supervisor" onClick="popupwindow('confirm_record.php?id=<?php echo $row['id']; ?>&table=<?php echo $table; ?>', 'Confirm Record', 520,350);"><i class="fa fa-check-circle"></i></a>
                                            <?php }
                                        }
                                        if ($action_duplicate) {
                                            if ($row['is_temp'] == 0) { ?>
                                                <a data-toggle="tooltip" data-placement="top" href="javascript:void(0)" class="w3-button w3-small w3-circle w3-white w3-border w3-border-blue" title="Create Duplicate" onClick="popupwindow('interp_edit.php?edit_id=<?php echo $row['id']; ?>&duplicate=<?php echo 'yes'; ?>','Duplicate Order',1250, 730)"><i class="fa fa-clone"></i></a>
                                            <?php }
                                        }
                                    }
                                    if ($action_job_note) {
                                        ...
                                        <a title="<b>JOB NOTES</b>" data-toggle="popover" data-trigger="hover" data-placement="top" style="text-decoration:none;" data-content="<?php echo 'Unread Job Notes are <b>' . $arr_n['unread'] . '</b> <br> Read Job Notes are <b>' . $arr_n['yes_read'] . '</b>'; ?>" href="javascript:void(0)" onclick="popupwindow('jobnote.php?fid=<?php echo $row['id']; ?>&table=<?php echo $table; ?>&orgName=<?php echo $row['orgName']; ?>','Job Notes',900,800)" <?php echo $arr_n['unread'] > 0 ? 'class="w3-button w3-small w3-circle w3-blue"' : 'class="w3-button w3-small w3-circle w3-grey"'; ?>>
                                            <?php echo $arr_n['unread'] > 0 ? $arr_n['unread'] : $arr_n['yes_read']; ?>
                                        </a>
                                    <?php }
                                    if ($row['deleted_flag'] == 0 && $row['order_cancel_flag'] == 0) {
                                        if ($action_edit_job) { ?>
                                            <a data-toggle="tooltip" data-placement="top" href="javascript:void(0)" class="w3-button w3-small w3-circle w3-white w3-border w3-border-blue" title="Edit Job" onClick="popupwindow('interp_edit.php?edit_id=<?= $row['id'] ?>&is_home=1','Edit Order',1250, 730)">
                                                <i class="fa fa-pencil-square-o"></i></a>
                                            <?php }
                                        if ($action_purchase_order) {
                                            if ($row['is_temp'] == 0) { ?>
                                                <a data-toggle="tooltip" data-placement="top" title="Update Purchase Order #" href="javascript:void(0)" class="w3-button w3-small w3-circle w3-white w3-border w3-border-blue" onClick="popupwindow('purch_update.php?purch_id=<?php echo $row['id']; ?>&table=<?php echo $table; ?>&orgName=<?php echo $row['orgName']; ?>&porder=<?php echo $row['porder']; ?>','Update Purchase Order',600,550)"><i class="fa fa-barcode"></i></a>
                                                <?php }
                                        }
                                    ...
                                    if ($row['deleted_flag'] == 0) {
                                        if ($action_delete_job) { ?>
                                            <a data-toggle="tooltip" data-placement="top" href="javascript:void(0)" class="w3-button w3-small w3-circle w3-red w3-border w3-border-red" title="Trash Job" onClick="popupwindow('del_trash.php?del_id=<?php echo $row['id']; ?>&table=<?= $table; ?>&is_home=1','Delete Record',520,350)"><i class="fa fa-trash-o"></i></a>
                                            <?php }
                                    } else {
                                        if ($action_restore_job) {
                                            if ($tp == 'tr') { ?>
                                                <a class="w3-button w3-small w3-circle w3-green w3-border w3-border-green" title="Restore Order" href="javascript:void(0)" onClick="popupwindow('trash_restore.php?del_id=<?php echo $row['id']; ?>&table=<?= $table; ?>','Restore this order',520,350)"><i class="fa fa-refresh"></i></a>
                                            <?php }
                                        }
                                    }
                                    if ($row['order_cancel_flag'] == 1 && $tp == 'c') {
                                        if ($action_resume_job) { ?>
                                            <a class="w3-button w3-small w3-circle w3-green w3-border w3-border-green" title="Resume this cancelled Order" href="javascript:void(0)" onClick="popupwindow('email_resume.php?email_id=<?php echo $row['id']; ?>&table=<?php echo $table; ?>&orgName=<?= $row['orgName'] ?>','Resume this cancelled order',620,450)"><i class="fa fa-undo"></i></a>
                                            <?php }
                                    }
                                    if ($row['deleted_flag'] == 0 && $row['order_cancel_flag'] == 0) {
                                        ...
                                        if ($action_shift_order) {?>
                                            <a data-toggle="tooltip" data-placement="top" href="javascript:void(0)" class="w3-button w3-small w3-circle w3-brown w3-border w3-border-black" title="Shift this order to Telephone" onClick="popupwindow('telep_edit.php?edit_id=<?php echo $row['id']; ?>&duplicate=yes&is_shift=1','Shift Order',1250, 730)"><i class="fa fa-refresh"></i></a>
                                        <?php }
                                    }
    ...
    <!--New changes alert modal-->
    <div class="modal" id="new_changes_modal" data-backdrop="static">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header"><h3 class="text-danger"><b>IMPORTANT NOTIFICATION!</b> <button type="button" class="btn btn-xs btn-danger pull-right" data-dismiss="modal">×</button></h3>
                </div>
                <div class="modal-body new_changes_modal_attach">
                    <p>Dear User! We have done some important updates regarding security and improvements. So if you don't find a necessary page or find some missing actions from your account. Please reach out to <b>Management</b> to fix it.<br>
                Thank you</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
    ...
    <script>
        $(document).ready(function() {
            ...
            //Alert for new changes if not OK
            <?php if ($_SESSION['is_root'] == 0 && date("Y-m-d") == "2023-10-02" && (date("H") >= 8 && date("H") <= 12) && !isset($_SESSION['update_seen'])) {?>
                $("#new_changes_modal").modal("show");
            <?php $_SESSION['update_seen'] = 1;
            } ?>
        });
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data['status'] == 1) {
                            ...
                            current_element.replaceWith(data['message']);
                        }
                    },
                    error: function(data) {
