...
function myFunction() {
    ...
    var p = document.getElementById("our").value;
    if (!p) {
        p = "<?php echo $our; ?>";
    }
    var q = document.getElementById("ur").value;
    if (!q) {
        q = "<?php echo $ur; ?>";
    }
    ...
    window.location.href = "interp_paid.php" + '?interp=' + x + '&org=' + y + '&job=' + z + '&assignDate=' + w +
        '&our=' + p + '&ur=' + q + '&inov=' + o;
...
<body>
    ...
    <section id="main" class="column">
        ...
        <article class="module width_full">
            <header>
                <h3 class="tabs_involved" style="width:300px;"><a href="<?php echo basename(__FILE__);?>">Interp Paid
                        List</a> Sum Received: £(<span id="lblTotalPrice"></span>)</h3>
                <div align="right" style=" width:75%; float:right;margin-top:3px;">
                    <input type="text" name="inov" id="inov" style="width:80px;height:19px;" placeholder="Invoice #"
                        onChange="myFunction()" value="<?php echo $inov; ?>" />
                    <input type="text" name="our" id="our" style="width:80px;height:19px;" placeholder="Our Ref"
                        onChange="myFunction()" value="<?php echo $our; ?>" />
                    <input type="text" name="ur" id="ur" style="width:80px;height:19px;" placeholder="Your Ref"
                        onChange="myFunction()" value="<?php echo $ur; ?>" />
                    <select id="interp" onChange="myFunction()" name="interp" style="width:100px;height:25px;">
                        <?php 			
$sql_opt=
"SELECT distinct interpreter_reg.name,interpreter_reg.id,interpreter_reg.gender, interpreter_reg.city 
FROM interpreter_reg
JOIN interp_lang ON interpreter_reg.code=interp_lang.code
JOIN interpreter ON interpreter.intrpName=interpreter_reg.id
where interpreter.multInv_flag=0 and interpreter.rAmount > 0 and (orgName like '%$_words%')
 ORDER BY name ASC";
...
where interpreter.multInv_flag=0 and interpreter.rAmount > 0
...
?>
                    ...
                    |
                    <input type="date" name="assignDate" id="assignDate" placeholder=''
                        style="width:100px;border-radius: 5px;" onChange="myFunction()"
                        value="<?php echo $assignDate; ?>" />
                </div>
            </header>
	   ...
	   where $table.deleted_flag = 0
		 ...
		 and $table.assignDate like '$assignDate%'
		 and source like '$job%'
		 ...
		 order by $table.assignDate LIMIT {$startpoint} , {$limit}";
		...
		while($row = mysqli_fetch_array($result)){
			...
			$counter++; ?>
                            <tr>
                                ...
                                <td><?php if($row['C_hoursWorkd']==0){ ?><span style="color:#F00"
                                        title="Comp Hours: <?php echo $row['C_hoursWorkd']; ?>"><?php echo $row['orgName']; ?></span><?php }else{ echo $row['orgName']; }?>
                                </td>
                                <td><?php if($row['hoursWorkd']==0){ ?><span style="color:#F00"
                                        title="Interp Hours: <?php echo $row['hoursWorkd']; ?>"><?php echo $row['name']; ?></span><?php }else{ echo $row['name']; }?>
                                </td>
                                ...
                                <td><?php echo $row['comp_hrsubmited'].'('.$misc->dated($row['comp_hr_date']).')'; ?>
                                </td>

                                <?php 
					//##gotcreditnote
					$totinvnow=$row['total_charges_comp']* $row["cur_vat"] + $row["C_admnchargs"]*$row["cur_vat"] + $row["C_admnchargs"] +
					...
					?>
                                ...
                                <td><?php echo round($totinvnow,2); ?></td>
                                <td class="totalprice"><?php echo round($row['rAmount'],2); ?></td>
                                <td><?php echo $row['rDate']; ?></td>

                                <td align="center"><a href="#"
                                        onClick="popupwindow('order_view.php?view_id=<?php echo $row['id']; ?>&table=<?php echo $table; ?>', 'title', 1000, 1000);">
                                        <input type="image" src="images/icn_new_article.png" title="View Order">
                                    </a>
                                    <a href="#"
                                        onClick="MM_openBrWindow('receive_amount.php?row_id=<?php echo $row['id']; ?>&table=<?php echo $table ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')">
                                        <input type="image" src="images/Cash.png" title="Payment Received">
                                        <a href="#"
                                            onClick="MM_openBrWindow('invoice.php?invoice_id=<?php echo $row['id']; ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')"><input
                                                type="image" src="images/invoice.png" title="Invoice"></a>
...
{ 
	?>
                                        <a href="#"
                                            onClick="MM_openBrWindow('credit_interp.php?invoice_id=<?php echo $row['id']; ?>','_blank','scrollbars=yes,resizable=yes,width=900,height=650')">
                                            <input style="background-color:red;" type="image"
                                                src="images/icn_categories.png" title="Credit Note">
                                        </a>
                                        <?php    
}
else
...
?>
                                        ...
                                        <a href="#"
                                            onClick="MM_openBrWindow('un_commit.php?com_id=<?php echo $row['id']; ?>&un_comit_paid=1&table=<?php echo $table; ?>','_blank','scrollbars=yes,resizable=yes,width=500,height=250')">
                                            <input type="image" src="images/icn_jump_back.png"
                                                title="Un-commit 'Received payment become 0'">
                                        </a>
