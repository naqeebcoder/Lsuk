...
<!-- end of sidebar -->
	...
	<section id="main" class="column"><!-- end of stats article -->
		...
		<header><h3 class="tabs_involved" style="width:200px;"><a href="<?php echo basename(__FILE__);?>">Translation Cancelled Orders list</a></h3>
...
?>
  ...
  <input type="date" name="asignDate" id="asignDate" placeholder='' style="width:100px;border-radius: 5px;" onChange="myFunction()" value="<?php echo $asignDate; ?>"/>
	  ...
	  switch($_SESSION['prv']){
	   ...
	   where  $table.deleted_flag=1 and  $table.asignDate like '$asignDate%' and $table.multInv_flag=0  and  $table.multInv_flag=0  and $table.commit=0  and source like '%$job%' and $table.orgName like '%$org%' and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	   order by asignDate LIMIT {$startpoint} , {$limit}";
		break; 
		case 'Finance':
		
	   $query="SELECT $table.* FROM $table	   
	   where  $table.deleted_flag=1 and  $table.numberUnit<>0 and $table.asignDate like '$asignDate%' and  $table.multInv_flag=0  and $table.commit=0  and source like '%$job%' and $table.orgName like '%$org%' order by asignDate and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	    LIMIT {$startpoint} , {$limit}";
		break; 
		case 'Operator':
		
	   $query="SELECT $table.* FROM $table	   
	   where  $table.deleted_flag=1 and  orderCancelatoin=0 and $table.numberUnit=0 and $table.asignDate like '$asignDate%' and  $table.multInv_flag=0  and $table.commit=0  and source like '%$job%' and $table.orgName like '%$org%' order by asignDate and $table.nameRef like '%$our%' and $table.orgRef like '%$ur%' and $table.invoiceNo like '%$inov%'
	    LIMIT {$startpoint} , {$limit}";
		break;  
	  }
