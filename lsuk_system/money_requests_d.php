...
//Access actions
$get_actions = explode(",", $obj->read_specific("GROUP_CONCAT(action_permissions.action_id) as actions", "action_permissions,route_actions", "action_permissions.action_id=route_actions.id AND route_actions.route_id=211 AND action_permissions.user_id=" . $_SESSION['userId'])['actions']);
$action_create = $_SESSION['is_root'] == 1 || in_array(200, $get_actions);
$action_check_request = $_SESSION['is_root'] == 1 || in_array(201, $get_actions);
...
$get_types = $obj->read_all("*", "loan_dropdowns", "is_payable=1 AND deleted_flag=0 ORDER BY title ASC");
...
<head>
  <title>Deduction Requests List</title>
...
<body>
      ...
      <header>
        <!-- <center>
          <div class="alert alert-info col-sm-3">
            <a href="<?php echo basename(__FILE__); ?>" class="alert-link">Deduction Requests list</a>
          </div>
        </center> -->
        <div class="row">
          ...
          <div class="form-group col-md-2"><br>
            <a href="javascript:void(0)" title="Click to Get Results" onclick="filter_list()"><span class="btn btn-danger">Filter List</span></a>
            <a href="money_requests_d.php" title="Click to reset filters"><span class="btn btn-warning">Clear</span></a>
          </div>
          <?php if ($action_create) { ?>
            <a style="color:white;margin: 5px 15px;" class="btn btn-danger" href="#" data-toggle="modal" data-target="#money_request_modal">Add New Deduction Request</a>
          <?php } ?>
            ...
            <tbody>
              ...
              while ($row = $result->fetch_assoc()) {
                ...
                <tr title="<?php echo 'Last updated at ' . $misc->dated($row['updated_date']); ?>">
                  ...
                  <td><?php 
                        if ($row['given_amount'] > 0) {
                          echo ($row['given_amount'] - $total_paid > 0) ? "<span class='text-danger'>" . number_format($total_paid, 2) . "</span>" : number_format($total_paid, 2) . " <i class='text-success fa fa-check-circle'></i>";
                        } else {
                          echo "<span class='text-danger'>0.00</span>";
                        } ?></td>
        ...
        <?php } else { ?>
          <h4 class="text-muted text-center">There are no Deduction Requests available currently in list!</h4>
        <?php } ?>
        <div>
        </div>
  </section>
    ...
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title" id="myModalLabel">Add New Deduction Request</h4>
        </div>
          ...
          <div class="row">
            <form action="process/money_requests.php" method="post">
              ...
              <div class="form-group col-md-12">
                <textarea id="reason" rows="12" name="reason" class="form-control" placeholder="Enter Deduction Request reason here ..."></textarea>
              </div>
              ...
              <div class="form-group col-md-8">
                <button type="submit" name="btn_add_money_request" class="btn btn-danger pull-left">Create Deduction Request</button>
              </div>
            </form>
          </div>
        </div>
  ...
  <div class="modal modal-info fade col-md-12" data-toggle="modal" data-target=".bs-example-modal-lg" id="view_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" style="display: none;">
    <div class="modal-dialog" role="document" style="width: 55%;">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
          <h4 class="modal-title" id="myModalLabel">Deduction Request Information</h4>
        </div>
  ...
  <script>
    ...
    function filter_list() {
      var append_url = "money_requests_d.php?";
