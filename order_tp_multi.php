...
<head>
    ...
    if (isset($_POST['submit']) && isset($captcha_flag) && $captcha_flag == 1) {
        ...
        foreach ($array_languages as $language) {
            ...
            $bookedTime = date('H:i:s');
            $assignTime = $_POST['assignTime'];
            $assignDur = $_POST['assignDur'];
            $orgContact = $_POST['orgContact'];
            $orgRef = $_POST['orgRef'];
            $assignBy = $_POST['assign_by'];
            $row = $acttObj->read_specific("count(id) as val", $table, "source='$source' and assignDate='$assignDate' and assignTime='$assignTime' and orgName='$orgName' and orgContact='$orgContact' and orgRef='$orgRef'");
            $val = $row['val'];
            if ($val == 0) {
                ...
                AND interpreter.order_company_id='" . $company_id . "' 
                ...
                if ($_POST['noClient'] != "") :
                    ...
                    $acttObj->editFun($table, $edit_id, 'noClient', $noClient);
                endif;
                if ($_POST['contactNo'] != "") :
                    $contactNo = $_POST['contactNo'];
                    $acttObj->editFun($table, $edit_id, 'contactNo', $contactNo);
                endif;
                if ($_POST['communication_link']) :
                ...
                if (isset($_SESSION['operator'])) {
                    if ($_SESSION['is_temp'] == 1) {
                        $acttObj->editFun($table, $edit_id, 'approve_portal_mngt', 0);
                    }
                }
                ...
                if (isset($_POST['submit']) && !empty($_POST['telep_cat']) && $_POST['telep_cat'] == '11') {
                ...
                $jobDisp =  $jobStatus_label == 'Enquiry' ? 0 : $_POST['jobDisp'];
                ...
                $message = "<p>Dear " . $inchPerson . "</p>
                    ...
                    <caption align='center' style='background: grey;color: white;padding: 5px;'>Order for " . $communication_type . "</caption>
                    <table class='myTable'>
                    <tr>
                ...
                $ack_message = 'Hi <b>Admin</b>
                    <p>This is an email acknowledgement for ' . $source . " " . $communication_type . ' Job of ' . $get_dur . ' requested by ' . $orgName . ' booked on ' . $misc->dated($bookedDate) . ' ' . $bookedTime . ' for assignment date ' . $assignDate . ' ' . $assignTime . '.</p>
                ...
                try {
                    ...
                    if ($mail->send()) {
                        ...
                        if (!isset($_GET['is_test'])) {
                            ...
                            $mail->addReplyTo($from_add, 'LSUK');
                            $mail->isHTML(true);
                            $mail->Subject = 'Acknowledgement for new ' . $communication_type . ' Online Portal Job';
                        ...
                        if ($jobDisp_req == '1' && $jobStatus_req == '1') {
                            ...
                            $subject_int = "New " . $communication_type . " Project " . $edit_id;
                                ...
                                if ($acttObj->read_specific("COUNT(*) as blacklisted", "interp_blacklist", "interpName='id-" . $row_emails['id'] . "' AND orgName='" . $orgName . "' AND deleted_flag=0 AND blocked_for=1")["blacklisted"] == 0) {
                                    ...
                                    if (!empty($array_tokens)) {
                                        ...
                                        foreach ($array_tokens as $token) {
                                            if (!empty($token)) {
                                                $acttObj->notify($token, $subject_int, $sub_title, array("type_key" => $type_key, "job_type" => "Telephone"));
                                            }
                                        }
                                    }
                ...
                if (isset($_SESSION['operator'])) {
                    $acttObj->editFun($table, $edit_id, 'submited', "Online Operator - " . $_SESSION['operator_name']);
                } else {
...
<body class="boxed">
    ...
    <div id="wrap">
        ...
        <a href="client_orders.php" class="btn btn-primary btn-lg">View All Orders</a></b></div>';
                ...
                <form class="sky-form" id="stepForm" action="" method="post">
                    <div class="panel panel-primary setup-content" id="step-1">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group col-md-4 col-sm-6" id="div_comunic">
                                <label class="control-label">Select Communication Type</label>
                                <select class="form-control" name="comunic" id="comunic" required>
                                    <?php $q_types = $acttObj->read_all("c_id,c_title,c_image", "comunic_types", "c_status=1 ORDER BY c_title");
                                    $options = "";
                            ...
                            <div class="form-group col-md-3 col-sm-6" id="div_tpc">
                                <label class="control-label">Select Telephone Category</label>
                            ...
                            <div class="form-group col-md-5 col-sm-6" id="div_assignIssue" style='display:none;'>
                                <textarea style='display:none;height: 60px;' title="Assignment Issue" placeholder="Write Assignment Issue Here ..." name="assignIssue" type="text" class="form-control" id="assignIssue"></textarea>
                            </div>
                            <div class="form-group col-md-3 col-sm-6">
                                <label class="control-label input2">Assignment Date*</label>
                                <input class="form-control date_picker" type="date" id="assignDate" name="assignDate" required='' value='' />
                            </div>
                            ...
                            <div class="form-group col-md-3 col-sm-6">
                                <label class="control-label">Assignment Duration * <small>(Hours:Minutes)</small></label>
                                <input id="assignDur" onkeyup="dur_finder();" name="assignDur" type="text" pattern="[0-9 :]{5}" maxlength="5" class="form-control" value="" required='' placeholder="Hours : Minutes" />
                            </div>
                            <div class="form-group col-md-3 col-sm-6">
                    ...
                    <div class="panel panel-primary setup-content" id="step-2">
                        ...
                        <div class="panel-body">
                            <div class="form-group col-md-6 col-sm-6 search-box">
                                ...
                                <input class="form-control" name="orgRef" id="orgRef" type="text" required='' autocomplete="off" placeholder="Type your reference" />
                                ...
                                <div class="col-sm-12 col-md-12">
                                    <div class="radio-inline ri">
                                        <label>
                                            ...
                                            <span class="label label-primary">Host By LSUK</span>
                                        </label>
                                    </div>
                                    <div class="radio-inline ri">
                                        <label>
                                            <input type="radio" name="assign_by" value="2" class="assign_by_client assign-type">
                                            <span class="label label-warning">Host By Client</span>
                                        </label>
                                    </div>
                                </div>
                                <div class="form-group col-md-6 col-sm-6 contact_info_div">
                                    <label class="control-label input">Telephone Number (Professional)*</label>
                                    <input type="text" class="form-control" id="noClient" name="noClient" required='' />
                                </div>
                                <div class="form-group col-md-6 col-sm-6 contact_info_div">
                                    <label class="control-label input">Telephone Number (Service User)<i class="fa fa-question-circle" title="IF Service User Is Not Present"></i></label>
                                    <input type="text" class="form-control" id="contactNo" name="contactNo" required=''>
                                </div>
                            </div>
                            <div class="assign_details2" style="display:none;">
                                <div class="form-group col-md-6 col-sm-">
                                    <label for="" class="control-lable inpou">Contact Link (Client)</label>
                                    <input name="communication_link" type="text" id="communication_link" class="form-control" value="" placeholder="Paste link here">
                                </div>
                            </div>
                            ...
                            <div class="form-group col-md-4 col-sm-6 <?php if ($po_req == 0) {
                                ...
                                <br><span class="col-md-offset-2">
                                    <label class="checkbox-inline" style="margin-top: 4px;border: 1px solid lightgrey;padding: 4px 10px;"><input style="transform: scale(1.2);" onchange="booking_purch_order();" type="radio" name="po_number" value="1" checked> Yes</label>
                                    <label class="checkbox-inline" style="border: 1px solid lightgrey;padding: 4px 10px;"><input style="transform: scale(1.2);" onchange="booking_purch_order();" type="radio" name="po_number" value="0"> No</label>
                                </span>
                            </div>
                            <div class="form-group col-md-4 col-sm-6 <?php if ($po_req == 0) {
                                                                            echo 'hidden';
                                ...
                                <input name="purchase_order_number" id="purchase_order_number" type="text" class="form-control" autocomplete="off" placeholder="Search purchase order number" <?php if ($po_req == 1) {
                                                                                                                                                                                                    echo 'required';
                                                                                                                                                                                                } ?> />
                                <i id="confirm_value" style="position: absolute; right: 25px; top: 35px; cursor: pointer;" onclick="$(this).hide();$(this).next('.result').empty();" class="glyphicon glyphicon-ok-sign text-success confirm_element" title="Confirm this purchase order number"></i>
                                <div class="result"></div>
                            </div>
                            <div id="div_po_req" class="form-group hidden col-md-4 col-sm-6">
                                <label>Purchase Order Email Address</label>
                                <input oninput="$('#write_po_email').html($(this).val());if($(this).val()){$('.tr_po_email').removeClass('hidden');}" name="po_req" id="po_req" type="text" class="long form-control bg-success" placeholder='Fill email for purchase order' />
                            </div>
                            <div class="row"></div>
                            <div class="form-group col-md-12">
                                <button class="btn btn-primary nextBtn pull-right" type="button">Next <i class="fa fa-angle-right"></i><i class="fa fa-angle-right"></i></button>
                    ...
                    <div class="panel panel-primary setup-content" id="step-3">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group col-md-4">
                                <label class="control-label input">Assignment Incharge Name&nbsp;* <i class="fa fa-question-circle" title="Assignment in-Charge"></i></label>
                    ...
                    <div class="panel panel-primary setup-content" id="step-4">
                        ...
                        <div class="panel-body">
                            <div class="form-group col-sm-12">
                                ...
                                <!-- 
                  <div class="radio-inline ri"><label><input name="gender" type="radio" value="Male" ?> />
                  ...
                  <span class="label label-default">No Preference</span></label></div> -->
                                <select name="gender" class="form-control" required="required">
                                    ...
                                    <option value="Female" <?php if (in_array($_GET['company_id'], $array)) {
                                                                echo 'selected';
                                                            } ?>>Female</option>
                            ...
                            <div class="form-group col-sm-12">
                                ...
                                <div class="radio-inline ri"><label><input name="jobStatus" type="radio" value="0" />
                                        <span class="label label-info">Ask for Quote <i class="fa fa-question"></i></span></label></div>
                                <div class="radio-inline ri"><label><input type="radio" name="jobStatus" value="1" checked />
                                        <span class="label label-primary">Confirm <i class="fa fa-check-circle"></i></span></label></div>
                            </div>
                            <div class="form-group col-sm-12 dJob">
                                <label class="control-label optional">Seek Approval ?</label><br>
                                <div class="radio-inline ri"><label><input name="jobDisp" type="radio" value="1" checked />
                                        <span class="label label-primary" style="font-size:100%;padding: .5em 0.6em 0.5em;">No Approval Required <i class="fa fa-check-circle"></i></span></label></div>
                                <div class="radio-inline ri"><label><input type="radio" name="jobDisp" value="0" />
                                        <span class="label label-info" style="font-size:100%;padding: .5em 0.6em 0.5em;">Approval Required <i class="fa fa-question"></i></span></label></div>
                            </div>
    ...
    btn.addEventListener("click", function () {
        ...
        inputs.forEach(el => {
            if (el.hasAttribute("required") && !el.disabled) {
                if (!el.value) {
                    el.classList.add("is-invalid"); // bootstrap red border
                    valid = false;
                } else {
        ...
        if (!valid) {
            // show error
            // 👇 Re-click current step tab after 300ms
            const currentNav = document.querySelector(`.stepwizard a[href="#${currentStep.id}"]`);
            if (currentNav) {
                setTimeout(() => currentNav.click(), 300);
            }
        ...
        if (nextStep) {
            ...
            const nextNav = document.querySelector(
                `.stepwizard a[href="#${nextStep.id}"]`
            );
            if (nextNav) {
                nextNav.classList.remove("disabled");
                nextNav.classList.add("btn-primary");
            }
        }
    });
});
    ...
    <script type="text/javascript">
        $('body').on('change', '#comunic', function() {
            ...
            if ($.inArray(val, list) > -1) {
                ...
                $('#contactNo').attr('required', false);
                $('#noClient').attr('required', false);
                // $('#communication_link').attr('required', true);
            } else {
                // alert(2);
                $('.assign_details1').show();
                $('.assign_details2').hide();
                $('#contactNo').attr('required', true);
                $('#noClient').attr('required', true);
                // $('#communication_link').attr('required', false);
            }
        });
        ...
        $('body').on('click', '.assign_by_client', function() {
            $('.contact_info_div').css('display', 'none');
            $('#contactNo').attr('required', false);
            $('#noClient').attr('required', false);
            // $('#communication_link').attr('required', false);
        });
        ...
        $('.nextBtn:eq(0)').click(function() {
            ...
            var assignment_category = $('#telep_cat').val();
            var comunic_type = $('#comunic').val();
            if (!check_languages && source_language && target_language && assignment_category && comunic_type) {
                set_language();
            }
        });
        $(document).ready(function() {
            ...
            $(document).on("change", "input[name=jobStatus]", function() {
                if ($("input[name=jobStatus]:checked").val() == 0) {
                    $(".dJob").addClass("hidden");
                } else {
                    $(".dJob").removeClass("hidden");
                }
            });
        });
        $(document).ready(function() {
