...
<?php include'source/db.php'; include'source/class.php'; if(isset($_POST['submit'])){$table='comp_login';$dbemail=$acttObj->uniqueFun($table,'email',$_POST['email']);
if($dbemail==0){$edit_id= $acttObj->get_id($table);}}?>
<?php if(empty($_SESSION['cust_UserName'])){echo '<script type="text/javascript">' . "\n"; 	
			echo 'window.location="index.php";'; echo '</script>';} ?>
<!DOCTYPE HTML>
...
<select id="orgName" name="orgName" required=''>
           ...
           <?php if(isset($_POST['submit']) && $dbemail==0){$c1=$_POST['orgName']; $acttObj->editFun($table,$edit_id,'orgName',$c1);} ?>
							</label>
						</section>	
						
						<section class="col col-6">
							<label class="input">Email 
      <input name="email" type="email" value="" required='' placeholder='' onBlur="uniqueFun(this.value,'login','email',$(this).attr('id') );" />
      ...
      <input name="paswrd" type="password" value="" placeholder='' required='' id="pass" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}"  onchange="form.repass.pattern = this.value;"/>
      <?php if(isset($_POST['submit'])  && $dbemail==0){$c2=$_POST['paswrd']; $acttObj->editFun($table,$edit_id,'paswrd',$c2);} ?>
							</label>
						</section>
                        <section class="col col-6">
							<label class="input">Re-Password 
      <input name="repass" type="password" value="" placeholder='' required=''/>
							</label>
						</section>
					</div>
